...
- `scripts/write-data-flow-storage-plan.ps1` and `.github/workflows/manual-data-flow-storage-plan-evidence.yml` record the sizing, query-window, target p95 query latency, no-object-key aggregate policy, backfill, rollback, dashboard cutover, retention budget, and query-plan/target-store evidence checklist for the future partition/time-series transition. MariaDB partition/dual-write candidates now consume the sanitized `scripts/write-mariadb-query-plan-evidence.ps1` JSON summary via `-QueryPlanEvidenceJsonPath` or workflow `query_plan_evidence_json_base64`, and dashboard readiness plus operations artifact import expose both `dataFlowStoragePlan.candidateDecision` and the sanitized `dataFlowStoragePlan.queryPlanEvidence` summary while checks remain pending.
- `scripts/write-data-flow-storage-transition-runbook-evidence.ps1` and `.github/workflows/manual-data-flow-storage-transition-runbook-evidence.yml` turn a passed storage plan into a target rehearsal record for backfill, dual-write or partition toggle, rollback, reconciliation, dashboard cutover, and retention dry-run evidence without storing raw SQL, raw EXPLAIN, object keys, raw event messages, or secrets; operations workflow run-id, artifact collection, and artifact import helpers can promote that runbook snapshot into the standard `.osmu-run/latest-data-flow-storage-transition-runbook-evidence.*` handoff path.
- `scripts/write-monitoring-threshold-evidence.ps1` and `.github/workflows/manual-monitoring-threshold-evidence.yml` record target Prometheus rule load, Grafana dashboard import, Alertmanager route review, incident routing review, and tenant baseline tuning evidence. Operations workflow run-id, artifact collection, artifact finalizer, and artifact import helpers can promote the passed snapshot into `.osmu-run/latest-monitoring-threshold-evidence.*` while rejecting credential-shaped receiver/secret content.
- `scripts/write-cluster-network-access-review-evidence.ps1`/`.github/workflows/manual-cluster-network-access-review-evidence.yml` and `scripts/write-helm-values-hardening-evidence.ps1`/`.github/workflows/manual-helm-values-hardening-evidence.yml` record target NetworkPolicy access review and Helm values hardening evidence with non-secret base64 reference payloads. Workflow run-id, artifact collection, artifact finalizer, and artifact import helpers promote the resulting `.osmu-run/latest-cluster-network-access-review-evidence.*` and `.osmu-run/latest-helm-values-hardening-evidence.*` reports while rejecting self-test target markers and credential-shaped content.
- `scripts/write-support-escalation-handoff-evidence.ps1` and `.github/workflows/manual-support-escalation-handoff-evidence.yml` record reviewed runbook, troubleshooting, rollback, support escalation, support SLA, known-gap, and operations handoff package references; workflow run-id, artifact collection, artifact finalizer, and artifact import helpers can promote the reduced evidence without storing credential-shaped references.
...
- Full B2B product current estimate: about 45% after Kubernetes HA safety rails, HA/DR live evidence script, dedicated Kubernetes HA/DR Readiness CI workflow, Kubernetes backup/restore namespace/backup artifact/isolated restore drill scripts, external DR bucket bootstrap and immutability preflight, bounded and hardened external DR artifact transfer helper, DR drill orchestration wrapper, restore smoke helper, DR evidence API request helper, Kubernetes DR finalization wrapper, Kubernetes DR Finalizer CI workflow, persisted restore drill evidence history, dashboard restore evidence visibility, admin dashboard operations readiness evidence API and frontend summary/remediation/evidence-plan/invocation/invocation-unblock/dispatch-preflight/workflow-run-id/artifact-collection/artifact-import/finalizer/evidence-handoff/convergence/Kubernetes report sync visibility, convergence-level Kubernetes report sync ready/source-result gate, Kubernetes report sync artifact collection/import support, Kubernetes report sync live dashboard polling verifier, Kubernetes operations report ConfigMap/backend Pod mount verifier, Kubernetes/Helm operations report mount plus ConfigMap sync helper that republishes sync evidence for dashboard visibility, CI workflow, storage expansion runner RBAC, dry-run evidence scripts, storage expansion finalization wrapper, Storage Expansion Finalizer CI workflow, operations readiness artifact gate, operations evidence plan helper, guarded operations evidence plan invocation helper, operations invocation unblock plan helper, operations dispatch preflight helper, operations workflow run id plan helper with manual target evidence workflow mapping, operations artifact collection plan helper with storage backend telemetry, monitoring threshold, secret rotation, commercial integration, commercial approval, support escalation handoff, and handoff package artifact input, operations evidence handoff helper with selected-action scope freshness for dispatch preflight/workflow run id/artifact collection plus finalizer-missing/pending/gap and readiness-ready/finalizer-missing/gap routing, no-execute operations readiness convergence helper, storage backend telemetry/monitoring threshold/secret rotation/commercial integration/commercial approval/support escalation handoff/operations handoff package evidence workflows and artifact import/finalizer support, Operations Readiness Finalizer CI workflow, Operations Readiness Artifact Finalizer CI workflow, IAM/RBAC finalizer evidence and dedicated IAM/RBAC Finalizer CI workflow, security evidence writer scripts for image signing plus container scan/SBOM evidence, a security evidence finalizer that promotes non-synthetic CI artifacts, Security Evidence Finalizer CI workflow, image digest capture, SBOM SHA256 hash evidence, enterprise auth smoke evidence helper and manual CI workflow, cross-platform PowerShell command propagation for finalizer CI workflows, and operations readiness remediation metadata for remaining live/security/monitoring/commercial-approval/enterprise-auth/package evidence gaps.
- Durable MVP demo evidence: `.osmu-run/latest-durable-demo-gate.json` and `.osmu-run/latest-demo-readiness.json` show `result=ready` / `currentDemoStatus=docker-durable-demo-verified`; `.osmu-run/latest-durable-mvp-finalize.json` is currently `result=planned` / `currentDemoStatus=durable-mvp-finalize-plan`, so MVP completion is `readiness-ready/finalizer-missing` until a Docker-ready finalize refresh rewrites ready finalizer evidence.
...
| 테스트/검증 자동화 | 79% | 프론트/문서/manifest/static gate, backend Gradle tests, Docker durable gate, Browser E2E, Docker integration smoke, Dockerized real S3 client smoke, operations readiness, operations evidence plan verifier, guarded operations evidence plan invocation verifier, operations invocation unblock plan verifier, operations dispatch preflight verifier, operations workflow run id plan verifier with manual target evidence workflows, operations artifact collection plan verifier with storage backend telemetry/data-flow storage plan/data-flow query-retention/data-flow transition runbook/monitoring threshold/secret rotation/cluster network access review/Helm values hardening/commercial integration/commercial approval/chargeback closeout/enterprise auth smoke/JIT rollback/support escalation handoff/handoff package artifact input, operations evidence handoff verifier with dispatch-preflight/workflow-run-id/artifact selected-action scope mismatch plus finalizer-missing/pending/gap and readiness-ready/finalizer-missing/gap fixtures, operations readiness convergence verifier with finalizer-required/sync-required/string-or-missing-count/not-ready-source fixtures, operations handoff package writer verifier, Kubernetes report sync live dashboard polling verifier, Kubernetes report ConfigMap/backend Pod mount verifier, pending evidence remediation metadata, operations readiness finalizer plan self-test, cross-platform finalizer `pwsh` command propagation check, operations readiness artifact importer self-test with storage backend telemetry, monitoring threshold, secret rotation, cluster network access review, Helm values hardening, commercial integration, commercial approval, support escalation handoff, handoff package, enterprise auth import, and data-flow query-plan summary rejection, operations readiness dashboard API/frontend invocation unblock, dispatch preflight, workflow run id, artifact collection, artifact import, finalizer, evidence handoff, convergence visibility test, chargeback preview/daily rollup service/controller tests, manual storage backend telemetry/monitoring threshold/secret rotation/cluster network access review/Helm values hardening/commercial integration/commercial approval/support escalation handoff/operations handoff package evidence workflow checks, Operations Readiness Artifact Finalizer CI draft, Kubernetes HA/DR Readiness CI draft, IAM/RBAC finalizer self-test와 전용 IAM/RBAC Finalizer CI draft, security evidence writer/finalizer self-test, enterprise auth smoke helper self-test와 Enterprise Auth Smoke CI draft, Security Evidence Finalizer CI draft, digest/hash evidence 검증은 통과, live Kubernetes/security/monitoring/enterprise-auth/package CI evidence는 환경 필요 |
...
- MariaDB object search/filter page가 prefix/search/tag/cursor/limit 조건을 SQL에 pushdown해 대량 metadata 후보를 애플리케이션 메모리로 모두 읽지 않는다. `scripts/verify-object-list-query-pushdown.ps1`가 회귀를 확인한다.
...
- target-scale MariaDB EXPLAIN/slow query evidence 확보
...
- Storage backend telemetry manual workflow: `.github/workflows/manual-storage-backend-telemetry-evidence.yml`가 준비된 `OSMU_MINIO_ADMIN_INFO_JSON_BASE64` admin-info JSON을 임시 파일로 디코드하는 `prepared_base64` mode와, `collection_mode=live`에서 `OSMU_MINIO_ACCESS_KEY`/`OSMU_MINIO_SECRET_KEY`로 target MinIO에 직접 `mc admin info --json`을 실행하는 mode를 모두 지원한다. 두 mode 모두 raw input 또는 mc config를 제거하고 요약 artifact만 업로드한다.
...
5. 운영/관리 기능 고도화
   ...
   - data-flow 장기 analytics와 chargeback 모델링. 현재 data-flow daily rollup API/UI와 daily rollup CSV export/materialized rollup refresh/read/export/retention cleanup, chargeback preview API, data-flow daily rollup 기반 chargeback trend API/UI/CSV export, pricing policy 저장과 proposal/internal approval/commercial price-list reference, Admin billing panel, scoped threshold alert, scoped notification payload preview/outbox/history와 configured generic webhook/Slack/EMAIL SMTP relay send/adapter retry state, private/local webhook URL과 SMTP relay host 기본 차단, outbound payload size cap, generic notification/payment webhook HMAC signature header 옵션, scoped preview CSV export, draft invoice CSV export/persistence/internal approval, final invoice/payment state workflow, payment provider handoff outbox/history와 native payment provider adapter SPI/composite dispatch, CARD/BANK/TAX/ERP 설정 기반 native API bridge 우선 handoff send, configured generic/CARD/BANK/TAX/ERP webhook fallback/adapter retry state, payment provider adapter readiness, payment-provider adapter readiness snapshot 포함 commercial integration evidence writer/workflow, billing pricing proposal approval snapshot 포함 commercial approval evidence writer/workflow는 구현됐고, 남은 것은 vendor별 고정 SDK/스키마 구현과 target commercial integration/commercial approval `result=passed` evidence이다.
   - 운영 패키징, troubleshooting, runbook 보강. Operations handoff package evidence writer/workflow는 readiness/convergence/data-flow plan/runbook/secret rotation/monitoring threshold/commercial integration/commercial approval/enterprise auth smoke-or-scope-out snapshot 요약, typed target snapshot confirmation gate, review confirmation까지 구현됐고, target commercial approval과 handoff package `result=passed` evidence는 실제 pilot/production handoff에서 확보해야 한다.
...
- Implemented adapter retry worker dry-run/run for due notification delivery and payment provider handoff rows; notification rows can attempt the configured generic notification webhook adapter, `SLACK` channel Slack webhook adapter, or `EMAIL` channel SMTP relay adapter, and payment handoff rows can attempt the configured CARD/BANK/TAX/ERP native payment-provider bridge before provider-specific or generic webhook fallback. ADMIN-only payment provider adapter readiness now shows generic/CARD/BANK/TAX/ERP webhook profile readiness plus native bridge support, configuration-required, and ready states before sending handoff rows.
- Remaining billing/chargeback scope is vendor-specific fixed SDK/schema adapters, target secret/certificate rotation `result=passed` evidence, target commercial integration `result=passed` evidence, and target commercial approval `result=passed` evidence; approved commercial price-list reference recording, data-flow daily rollup chargeback trend/CSV export, notification webhook/Slack/EMAIL SMTP relay send, generic/CARD/BANK/TAX/ERP payment webhook profile send, native payment provider adapter SPI/composite dispatch, CARD/BANK/TAX/ERP configurable native API bridge, private/local webhook/native URL and SMTP relay host blocking, outbound payload size caps, generic notification/payment webhook plus native bridge HMAC signature headers, secret rotation evidence writer/workflow, commercial integration evidence writer/workflow with sanitized payment-provider adapter readiness snapshot, and commercial approval evidence writer/workflow with sanitized billing pricing proposal approval snapshot are implemented.
