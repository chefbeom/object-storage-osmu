...
- Remaining billing/chargeback scope is card/bank/tax/ERP-specific provider adapters, broader external-send hardening, and production secret rotation evidence; approved commercial price-list reference recording, notification webhook/Slack/EMAIL SMTP relay send, payment webhook send, private/local webhook URL/SMTP relay host blocking, and outbound payload size caps are implemented.
