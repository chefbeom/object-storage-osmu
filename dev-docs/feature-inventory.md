...
- Remaining MVP polish blockers: GitHub-hosted durable/real S3/Browser/image-signing evidence and host-installed `aws` or `mc` client smoke outside Docker. Demo package notes are generated by `scripts/write-mvp-demo-package-notes.ps1`.
...
- host-installed `aws` 또는 `mc` client smoke
