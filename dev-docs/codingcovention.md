...
작업유형 태그는 1개만 사용하고, 영역 태그는 변경 범위에 따라 1개 이상 사용할 수 있다.

작업유형 태그:

| 태그 | 의미 | 적용 대상 |
| --- | --- | --- |
| `[Feat]` | 기능 추가 | 새 API, 새 화면, 새 기능, 새 설정 |
| `[Fix]` | 버그 수정 | 오작동 수정, 예외 처리, 회귀 수정 |
| `[Refactor]` | 리팩터링 | 동작 변경 없는 구조 개선 |
| `[Docs]` | 문서 변경 | 기획서, 명세서, README, Worklog |
| `[Test]` | 테스트 | 테스트 코드, 테스트 케이스, 검증 환경 |
| `[Build]` | 빌드 | Gradle, npm, 패키지, 빌드 설정 |
| `[CI]` | CI/CD | GitHub Actions, 자동화 파이프라인 |
| `[Chore]` | 잡무 | 포맷, 설정 정리, 의존성 정리, 기타 유지보수 |
| `[Style]` | 스타일 | 코드 포맷, UI 색상/간격 등 동작 영향이 작은 스타일 변경 |
| `[Perf]` | 성능 | 성능 개선, 메모리/속도 최적화 |
| `[Release]` | 릴리스 | 버전, 배포 산출물, 릴리스 노트 |
| `[Revert]` | 되돌림 | 이전 커밋 되돌림 |
...
| `[M]` | Management | 관리자 기능, 관리 정책, 관리 화면, 운영 관리 기능 |
...
[Feat][B]: 버킷 생성 API 구현
[Feat][F]: 버킷 목록 화면 구현
[Feat][B][F]: 버킷 생성 API와 화면 연결
[Style][F]: 프론트페이지 검정 색상 제거
[Chore][I]: MariaDB와 MinIO 로컬 환경 추가
[Test][B][T]: 버킷 API 테스트 케이스 추가
[Fix][S][B]: Access Key 노출 방지 처리
[Refactor][B]: Storage Adapter 책임 분리
```

여러 영역을 함께 수정한 경우, 관련된 영역 태그를 모두 붙인다.

예:

```text
[Feat][B][F]: 파일 업로드 API와 업로드 화면 구현
```

영역이 많아져 제목이 길어지면 핵심 영역만 제목에 넣고, 나머지 영향은 커밋 본문 또는 worklog에 기록한다.
