...
- `PUT /api/s3/{bucketName}?lifecycle` validates optional single `Content-MD5` and one explicit `x-amz-checksum-sha256`, `x-amz-checksum-sha1`, `x-amz-checksum-crc32`, `x-amz-checksum-crc32c`, or `x-amz-checksum-crc64nvme` header against the raw lifecycle XML body before configuration replacement. `x-amz-sdk-checksum-algorithm` must be single-valued and match the explicit checksum value header. Duplicate `Content-MD5` or explicit checksum value headers return `InvalidDigest`; duplicate SDK checksum algorithm headers return `InvalidRequest`; invalid checksum syntax returns `InvalidDigest`; mismatched checksum values or SDK/header algorithm shape return `BadDigest`.
...
- S3 object `PUT` validates one optional checksum value header among `x-amz-checksum-sha256`, `x-amz-checksum-sha1`, `x-amz-checksum-crc32`, `x-amz-checksum-crc32c`, and `x-amz-checksum-crc64nvme`. If a single `x-amz-sdk-checksum-algorithm` is present without an explicit checksum value, OSMU computes and stores the requested checksum while streaming the object body, matching AWS CLI `--checksum-algorithm` behavior for supported algorithms. Invalid base64/length returns `InvalidDigest`; duplicate or unsupported SDK algorithms return `InvalidRequest`; mismatched body checksum or SDK/value header algorithm shape returns `BadDigest`; matching checksum is stored in object metadata and returns the same `x-amz-checksum-*` response header.
...
- `x-amz-copy-source` copies source object body, content type, user metadata, tags, and stored checksum metadata into the target object and returns `CopyObjectResult` XML. A single `x-amz-checksum-algorithm` overrides the copied checksum metadata by recalculating the target checksum for `SHA256`, `SHA1`, `CRC32`, `CRC32C`, or `CRC64NVME`; duplicate or unsupported algorithms return S3 XML `InvalidRequest`. Stored checksums are emitted as `ChecksumSHA256`, `ChecksumSHA1`, `ChecksumCRC32`, `ChecksumCRC32C`, or `ChecksumCRC64NVME` elements when the copied target metadata has them. `?versionId={versionId}` can copy an OSMU-retained source version body/content type/tags/user metadata.
...
- S3 multipart initiate accepts a single `x-amz-checksum-algorithm` for `SHA256`, `SHA1`, `CRC32`, `CRC32C`, and `CRC64NVME`, validates a single optional `x-amz-checksum-type: COMPOSITE|FULL_OBJECT` combination, rejects duplicate or unsupported checksum negotiation as S3 XML `InvalidRequest`, persists accepted checksum algorithm/type on the upload session, and echoes accepted `x-amz-checksum-algorithm`/`x-amz-checksum-type` response headers.
...
- S3 multipart complete accepts one optional supported final object checksum value header (`x-amz-checksum-sha256`, `x-amz-checksum-sha1`, `x-amz-checksum-crc32`, `x-amz-checksum-crc32c`, or `x-amz-checksum-crc64nvme`), rejects multiple final checksum value headers or unsupported `x-amz-checksum-*` value headers before storage completion, validates the checksum against the completed object body, validates a single optional `x-amz-mp-object-size` against the actual completed object size and returns S3 XML `InvalidRequest` on duplicate, invalid, or mismatched values, accepts a single `x-amz-checksum-type: FULL_OBJECT` when a final object checksum header is present, rejects duplicate `x-amz-checksum-type` values as S3 XML `InvalidRequest`, requires the final checksum algorithm to match any stored initiate `FULL_OBJECT` negotiation and returns S3 XML `BadDigest` before storage completion on mismatch, stores matching checksum metadata, returns the same checksum response header, emits complete-result XML `Location`, `Bucket`, `Key`, `ETag`, matching checksum element plus `ChecksumType` when requested, persisted on initiate, or inferable from the returned checksum shape, and preserves the MinIO-backed S3 multipart ETag (`md5-of-part-md5s-partCount`) in the response XML/header and later `HEAD`.
- S3 multipart complete rejects unsupported CompleteMultipartUpload control headers before storage completion: `x-amz-checksum-algorithm`, `x-amz-request-payer`, `x-amz-expected-bucket-owner`, `x-amz-server-side-encryption-customer-algorithm`, `x-amz-server-side-encryption-customer-key`, and `x-amz-server-side-encryption-customer-key-MD5`.
- Missing or inactive S3 multipart upload IDs return S3 XML `NoSuchUpload`.
- S3 multipart complete request XML requires 1~10000 direct `Part` children under `CompleteMultipartUpload`, exactly one direct `PartNumber`, exactly one direct non-blank `ETag` for every part, and strictly ascending unique part numbers. Out-of-order or duplicate part numbers return S3 XML `InvalidPartOrder` before storage complete. Uploaded non-last parts smaller than 5 MiB return S3 XML `EntityTooSmall` before storage complete.
...
- AWS S3 전체 동작 호환은 API 목표가 아니다. The S3-compatible API targets replacement use for common S3 clients and SDKs, with supported/partial/unsupported behavior fixed in `s3-compatibility.md`. Additional conditional, versioning, checksum negotiation, CreateBucket/DeleteBucket/CreateMultipartUpload, or error-message edge cases are expanded only when a supported real-client smoke or target migration scenario shows product impact.
...
### GET /api/admin/billing/pricing-policy

Internal chargeback preview default pricing policy. `ADMIN` can read and update it. `ORG_ADMIN` can read it to calculate its scoped preview but cannot update it. This policy is a preview default, not a final invoice or approved commercial price list.
...
### PUT /api/admin/billing/pricing-policy

Save the default pricing inputs used by chargeback preview when request query rates are omitted. `ADMIN` only.
...
- `currency`: optional display currency code. When omitted, the saved pricing policy currency is used.
...
- `eventScanLimit`: bounded event scan limit. When omitted, the saved pricing policy limit is used. Maximum is `50000`.
...
- `reason` (optional): operator note for the outbox record. Defaults to `manual`.
...
Lists scoped chargeback notification delivery outbox/history rows. `ADMIN` sees all rows and `ORG_ADMIN` sees only rows for its organization.
...
### GET /api/admin/billing/chargeback-invoice-draft/export.csv

Exports scoped organization chargeback rows as draft invoice CSV for internal billing review. `ADMIN` exports all visible organizations and `ORG_ADMIN` exports only its own organization. The output is generated from the same pre-model as chargeback preview/export and is not a final invoice, approved price list, payment request, or persisted billing record.
...
- `Content-Type: text/csv`
- `Content-Disposition: attachment; filename="osmu-chargeback-invoice-draft.csv"`
...
- `includeMonthlyRollups`: optional boolean, default `true`. Runs stored `data_flow_monthly_rollups` retention.
...
- Passing `includeEvents=false`, `includeDailyRollups=false`, and `includeMonthlyRollups=false` returns `VALIDATION_ERROR`.
...
- This is an OSMU operations capacity probe, not AWS S3 parity work. MinIO Admin `mc admin info --json` pool/node telemetry is currently captured through `scripts/write-storage-backend-telemetry-evidence.ps1`, `.github/workflows/manual-storage-backend-telemetry-evidence.yml`, or the storage expansion finalizer telemetry option; when direct MinIO Prometheus metrics are unavailable in MinIO mode, the backend status endpoint can use a passed `.osmu-run/latest-storage-backend-telemetry.json` report as `capacitySource=storage_backend_telemetry_evidence` and `readiness=TELEMETRY_EVIDENCE_READY`. The manual workflow supports prepared base64 admin-info ingestion and live workflow-side `mc admin info --json` collection with target MinIO credentials stored only as GitHub secrets.
...
`operationsHandoffPackage.secretRotationSnapshot` includes the reduced target secret/certificate rotation summary so the API exposes core rotation coverage, failure/planned counts, and confirmation flags without raw secret values or credential-shaped payloads.
...
`operationsHandoffPackage.secretRotationSnapshot` is reduced summary only: it exposes secret/certificate rotation result, target labels, rotation window, external evidence refs, confirmation flags, core rotation counts, failure/planned counts, top rotations, and top failed checks, but not secret values, private keys, bearer tokens, kubeconfig values, provider credentials, or full secret-manager output.
...
```json
{
  "data": {
    ...
    "categorySummaries": [
      { "category": "OPERATIONS", "totalCount": 14, "blockerCount": 0, "warningCount": 14 }
    ],
    "items": [
      {
        "severity": "WARNING",
      ...
      },
      {
        ...
        "message": "Operations workflow run id plan is query-required: workflows=12, missingRuns=12.",
        "targetPage": "dashboard",
        "targetPanel": "dashboard-readiness-panel",
      ...
      },
      {
        ...
        "message": "Operations handoff package is failed: failures=2, planned=1, checks=30.",
        "targetPage": "dashboard",
        "targetPanel": "dashboard-readiness-panel",
        "actionLabel": "Handoff package",
        "evidencePath": ".osmu-run/latest-operations-handoff-package.json",
        "remediationCommand": "powershell -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\write-operations-handoff-package.ps1",
        "remediationNote": "Evidence stores references only and must not contain passwords, bearer tokens, kubeconfig values, private keys, provider credentials, raw provider responses, or customer payment data."
      },
      {
        "severity": "WARNING",
        "category": "OPERATIONS",
    ...
    "operationsHandoffPackage": {
      ...
      "evidenceRefs": {
        ...
        "secretRotation": "latest-secret-rotation-evidence-passed",
      ...
      "monitoringThresholdSnapshot": {
        ...
        "targetCluster": "customer-cluster-a",
        "operatorName": "ops-admin",
        "evidenceRef": "latest-monitoring-threshold-evidence-passed",
    ...
    "securityEvidence": {
      ...
      "source": {
        ...
        "containerSecurityArtifactName": "osmu-container-security-abc123"
      },
      "images": {
      ...
      "imageSigning": {
        ...
        "commitSha": "abc123",
        "backendDigest": "sha256:1111111111111111111111111111111111111111111111111111111111111111",
        "backendVersionSignatureVerified": true,
        "backendShaSignatureVerified": true,
        "frontendDigest": "sha256:2222222222222222222222222222222222222222222222222222222222222222",
        "frontendVersionSignatureVerified": true,
        "frontendShaSignatureVerified": true
      },
      "containerSecurity": {
        "result": "failed",
        "failureCount": 1,
        ...
        "backendSbomValid": true,
        "backendSbomPackageCount": 42,
        "backendSbomByteSize": 4096,
        "backendSbomSha256": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "frontendSbomValid": true,
        "frontendSbomPackageCount": 38,
        "frontendSbomByteSize": 3072,
        "frontendSbomSha256": "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
      },
    ...
    "dataFlowStorageTransitionRunbook": {
      ...
      "storagePlanResult": "plan-ready-execute-required",
      "candidateStore": "MARIADB_PARTITION",
      "targetP95QueryLatencyMs": 500,
      "failureCount": 2,
      "checkCount": 10,
    ...
    "operationsEvidenceHandoff": {
      ...
      "nextStep": {
        ...
        "note": "Run the ready subset plan command first without -Execute, then dispatch only after review and continue resolving the remaining blocked actions."
      },
      "stageCount": 8,
      "readyStageCount": 1,
