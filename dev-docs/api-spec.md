...
- This is an OSMU operations capacity probe, not AWS S3 parity work. MinIO Admin `mc admin info --json` pool/node telemetry is currently captured through `scripts/write-storage-backend-telemetry-evidence.ps1`; when direct MinIO Prometheus metrics are unavailable in MinIO mode, the backend status endpoint can use a passed `.osmu-run/latest-storage-backend-telemetry.json` report as `capacitySource=storage_backend_telemetry_evidence` and `readiness=TELEMETRY_EVIDENCE_READY`. Fully automatic live collection remains a future enhancement.
