...
- Errors under `/api/s3/**` return AWS-style XML `<Error><Code>...</Code><Message>...</Message><Resource>...</Resource><RequestId>...</RequestId><HostId>...</HostId></Error>`; when derivable, per-error details such as `BucketName`, `Key`, and `UploadId` are emitted between `Message` and `Resource`. `HostId` is an opaque deterministic value derived from request id plus resource. S3 error responses also expose `x-amz-request-id` with the same value as `RequestId` and `x-amz-id-2` with the same value as `HostId`.
