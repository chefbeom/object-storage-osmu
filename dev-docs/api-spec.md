...
- S3 multipart initiate accepts `x-amz-checksum-algorithm` for `SHA256`, `SHA1`, `CRC32`, `CRC32C`, and `CRC64NVME`, validates optional `x-amz-checksum-type: COMPOSITE|FULL_OBJECT` combinations, rejects unsupported checksum negotiation as S3 XML `InvalidRequest`, and echoes accepted `x-amz-checksum-algorithm`/`x-amz-checksum-type` response headers.
...
- S3 multipart complete accepts one optional final object `x-amz-checksum-*` value header, validates it against the completed object body, validates optional `x-amz-mp-object-size` against the actual completed object size and returns S3 XML `InvalidRequest` on mismatch, accepts `x-amz-checksum-type: FULL_OBJECT` when a final object checksum header is present, stores matching checksum metadata, returns the same checksum response header, emits matching complete-result XML checksum element plus `ChecksumType` when requested, and preserves the MinIO-backed S3 multipart ETag (`md5-of-part-md5s-partCount`) in the response XML/header and later `HEAD`.
...
- S3 multipart complete request XML accepts optional per-part `ChecksumSHA256`, `ChecksumSHA1`, `ChecksumCRC32`, `ChecksumCRC32C`, and `ChecksumCRC64NVME` elements and validates their checksum syntax before completing storage upload. If no final object checksum header is supplied and every completed part has the same `ChecksumSHA256`, `ChecksumSHA1`, `ChecksumCRC32`, or `ChecksumCRC32C`, the response stores and emits the AWS-style composite checksum calculated from ordered part checksum bytes and accepts `x-amz-checksum-type: COMPOSITE`. `CRC64NVME` is accepted as a full-object checksum only. Invalid checksum type values or mismatched checksum type/header shape return S3 XML `InvalidRequest` before storage completion. Full AWS multipart checksum negotiation parity beyond supported composite algorithms and explicit checksum-type validation remains future work.
