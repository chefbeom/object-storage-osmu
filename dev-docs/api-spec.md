...
- S3 multipart complete accepts one optional final object `x-amz-checksum-*` value header, validates it against the completed object body, validates optional `x-amz-mp-object-size` against the actual completed object size and returns S3 XML `InvalidRequest` on mismatch, stores matching checksum metadata, returns the same checksum response header, emits matching complete-result XML checksum element, and preserves the MinIO-backed S3 multipart ETag (`md5-of-part-md5s-partCount`) in the response XML/header and later `HEAD`.
