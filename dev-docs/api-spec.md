...
- `x-amz-copy-source` copies source object body, content type, user metadata, tags, and stored checksum metadata into the target object and returns `CopyObjectResult` XML. Stored checksums are emitted as `ChecksumSHA256`, `ChecksumSHA1`, `ChecksumCRC32`, `ChecksumCRC32C`, or `ChecksumCRC64NVME` elements when the selected source metadata has them. `?versionId={versionId}` can copy an OSMU-retained source version body/content type/tags/user metadata.
