...
- S3 multipart complete request XML requires 1~10000 direct `Part` children under `CompleteMultipartUpload`, strictly ascending unique direct `PartNumber` values, and a non-blank direct `ETag` for every part. Out-of-order or duplicate part numbers return S3 XML `InvalidPartOrder` before storage complete. Uploaded non-last parts smaller than 5 MiB return S3 XML `EntityTooSmall` before storage complete.
