...
- Internal chargeback preview: `GET /api/admin/billing/chargeback-preview` and the Admin billing panel can model organization storage, ingress, egress, internal copy, and operation costs from current usage and data-flow events; this is not a final invoice or approved pricing policy.
...
- Chargeback preview: API and Admin billing panel pre-model implemented; persistent pricing policy, invoice export, and threshold alerts remain pending.
