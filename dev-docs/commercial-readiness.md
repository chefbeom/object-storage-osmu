...
- Internal chargeback preview: `GET/PUT /api/admin/billing/pricing-policy`, `GET /api/admin/billing/chargeback-preview`, and the Admin billing panel can model organization storage, ingress, egress, internal copy, and operation costs from current usage and data-flow events; this is not a final invoice or approved commercial price list.
...
- Chargeback preview: API, persistent pricing policy, and Admin billing panel pre-model implemented; invoice export and threshold alerts remain pending.
