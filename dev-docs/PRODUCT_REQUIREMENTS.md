...
- Unknown-size S3 multipart initiate parity, full AWS multipart checksum aggregation parity, automatic DNS/proxy provisioning for virtual-hosted-style domains, remaining conditional edge parity, exact CopyObject arbitrary user-metadata/full AWS versioning/full-conditional parity, multipart ETag parity, remaining CreateBucket/DeleteBucket edge error parity beyond name/duplicate/non-empty buckets, and full AWS error behavior parity remain future work. `dev-docs/s3-compatibility.md` tracks the supported/partial/unsupported matrix.
