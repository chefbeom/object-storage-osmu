...
- S3 XML error bodies include `Code`, `Message`, `Resource`, `RequestId`, and `HostId` for client compatibility and request tracing. S3 error responses also expose `x-amz-request-id` and `x-amz-id-2`.
