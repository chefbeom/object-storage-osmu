...
- `../scripts/verify-prototype-status.ps1` - verify the prototype status document stays current on local durable MVP readiness, production/B2B target-evidence gaps, S3 replacement boundary, enterprise auth, data-flow, commercial, and operations evidence chain scope.
...
- `../scripts/write-enterprise-auth-smoke-plan.ps1` - record plan-only, live target, or commercial scope-out enterprise auth smoke evidence as `.osmu-run/latest-enterprise-auth-smoke.*`; it rejects token or credential-like OIDC claim/JIT JSON input fields before request execution and never writes admin passwords, LDAP passwords, OIDC code/state, tokens, client secrets, or raw claim JSON to evidence.
- `../scripts/verify-enterprise-auth-smoke-plan.ps1` - self-test the enterprise auth smoke writer, plan-only and scope-out shapes, secret redaction, credential-like scope-out rejection, and unsafe token-bearing claim input rejection.
...
- `../scripts/verify-commercial-integration-evidence.ps1` - self-test the commercial integration evidence writer, payment-provider adapter readiness snapshot parsing, passed evidence shape, credential-like reference rejection, and unsafe raw provider/customer snapshot rejection.
...
- `../scripts/verify-commercial-approval-evidence.ps1` - self-test the commercial approval evidence writer, pricing proposal approval snapshot parsing, passed approval shape, price/raw-contract exclusion, unsafe raw commercial snapshot rejection, and credential-like reference rejection.
- `../scripts/write-operations-handoff-package.ps1` - record target-environment operations handoff package evidence as `.osmu-run/latest-operations-handoff-package.*`, tying runbook, troubleshooting, rollback, support escalation, known gaps, commercial approval, enterprise auth smoke or scope-out, target evidence references, sanitized operations readiness/convergence snapshot summaries, sanitized data-flow storage plan/query-plan summaries, sanitized data-flow storage transition runbook summaries, sanitized commercial summaries, and sanitized enterprise auth smoke summaries together without executing external commands or storing secrets, raw SQL, raw EXPLAIN, object keys, raw event messages, raw identity claims, raw remediation, raw provider/customer, contract, license, or payment content.
- `../scripts/verify-operations-handoff-package.ps1` - self-test the operations handoff package writer, passed package shape, readiness/convergence/data-flow storage plan/data-flow storage transition runbook/commercial/enterprise-auth snapshot parsing, required snapshot failure, credential-like reference/snapshot rejection, unsafe raw operations snapshot rejection, unsafe raw SQL data-flow snapshot rejection, unsafe raw price-table commercial snapshot rejection, and unsafe raw identity-claim enterprise auth snapshot rejection.
...
- `../scripts/write-operations-artifact-collection-plan.ps1` - derive expected GitHub artifact names, `gh run download` commands, Security Evidence Finalizer dispatch input, Operations Readiness Artifact Finalizer dispatch input, storage backend telemetry/monitoring threshold/data-flow storage plan/secret rotation/commercial integration/commercial approval/enterprise auth smoke or scope-out/operations handoff package/Kubernetes operations report sync artifact input, data-flow query-plan summary handoff notes, and local artifact import commands from an invocation report plus workflow run ids.
...
- `../scripts/import-operations-readiness-artifacts.ps1` - import previously collected storage expansion, HA/DR, Kubernetes DR, IAM/RBAC, security evidence, storage backend telemetry, monitoring threshold, data-flow storage plan, secret rotation, commercial integration, commercial approval, enterprise auth smoke or scope-out, operations handoff package, and Kubernetes operations report sync artifacts into the standard `.osmu-run/latest-*` readiness paths after validating their expected result values, including enterprise auth `passed|scope-out`, sanitized monitoring threshold content, and sanitized data-flow query-plan summary contract.
- `../scripts/verify-operations-readiness-artifact-import.ps1` - self-test the operations readiness artifact importer, including rejection of invalid failed evidence and MariaDB/dual-write data-flow plans missing query-plan evidence summary or carrying unsafe raw SQL/raw EXPLAIN/credential-shaped summary content.
...
- `../scripts/verify-operations-evidence-handoff.ps1` - self-test handoff selection for missing reports, blocked invocation, artifact-finalizer-ready, operations-finalizer-missing, operations-finalizer-pending, and readiness-ready but finalizer-missing states.
- `../scripts/write-operations-readiness-convergence.ps1` - summarize the latest handoff, readiness, operations finalizer, and Kubernetes operations report sync reports into a no-execute convergence report with current bottleneck, recommended command chain, finalizer/sync gates, and ready decision.
- `../scripts/verify-operations-readiness-convergence.ps1` - self-test convergence reporting for missing handoff, action-required, finalizer-required, sync-required, and fully ready states.
- `../scripts/sync-kubernetes-operations-reports.ps1` - validate and sync the latest operations readiness convergence report plus generated sync evidence into the Kubernetes `osmu-operations-reports` ConfigMap with sanitized data-flow query-plan summary checks, plan-only, server-dry-run, and explicit apply modes.
- `../scripts/verify-kubernetes-operations-report-sync.ps1` - self-test Kubernetes operations report ConfigMap sync using a fake kubectl for plan, server dry-run, apply, data-flow query-plan summary, and unsafe summary rejection paths.
- `../scripts/verify-kubernetes-operations-report-sync-live.ps1` - poll deployed dashboard readiness until it reflects the applied Kubernetes operations report sync evidence plus mounted data-flow storage plan/query-plan summary without storing admin passwords or bearer tokens.
- `../scripts/verify-kubernetes-operations-report-sync-live-self-test.ps1` - self-test the live sync verifier with dashboard fixtures for applied sync/data-flow/query-plan summary and stale sync states.
- `../scripts/verify-kubernetes-operations-report-mount.ps1` - read-only live verifier that checks the operations report ConfigMap keys and confirms the backend Pod can read the mounted convergence, sync evidence, and data-flow storage plan/query-plan summary files.
...
- `../.github/workflows/kubernetes-operations-report-sync-ci.yml` - manual GitHub Actions workflow for operations convergence report generation plus optional data-flow storage plan restore/sanitized query-plan summary validation, Kubernetes ConfigMap sync plan, server-dry-run, and guarded apply evidence.
...
- `../scripts/write-monitoring-threshold-evidence.ps1` - record target Prometheus/Alertmanager/Grafana threshold review evidence from `infra/monitoring/alert-threshold-targets.yaml`, external rule/dashboard/route/baseline refs, and no-secret confirmations.
- `../scripts/verify-monitoring-threshold-evidence.ps1` - self-test the monitoring threshold evidence writer, including passed evidence, secret-shaped reference rejection, reversed review window rejection, and missing required alert target rejection.
- `../scripts/verify-ci-workflow.ps1` - verify GitHub Actions workflow drafts for lightweight CI, durable Docker MVP gate CI, real S3 client CI, container security CI, Browser E2E CI, storage expansion finalizer CI, Kubernetes HA/DR readiness CI, Kubernetes DR finalizer CI, operations readiness finalizer CI including data-flow storage plan/runbook evidence input, manual data-flow storage plan/runbook evidence CI, storage backend telemetry evidence CI, enterprise auth smoke CI, and finalizer `pwsh` command propagation.
...
- `../.github/workflows/operations-readiness-finalizer-ci.yml` - manual GitHub Actions workflow for combined operations readiness plan/live evidence collection. Live mode requires `OSMU_KUBECONFIG_BASE64`; optional `data_flow_storage_plan_json_base64` restores passed data-flow storage transition evidence after rejecting unsafe raw SQL/raw EXPLAIN/credential-shaped query-plan summaries, and the workflow uploads the combined readiness report plus selected storage, data-flow, HA/DR, DR, IAM/RBAC, and security evidence artifacts.
- `../.github/workflows/operations-readiness-artifact-finalizer-ci.yml` - manual GitHub Actions workflow that downloads evidence artifacts from previous storage expansion, HA/DR, Kubernetes DR, IAM/RBAC, security, storage backend telemetry, monitoring threshold, data-flow storage plan, data-flow storage transition runbook, secret rotation, commercial integration, commercial approval, enterprise auth smoke, operations handoff package, and optional Kubernetes operations report sync workflow runs; optional `data_flow_storage_plan_json_base64` and `data_flow_storage_transition_runbook_json_base64` import direct data-flow transition evidence through sanitized plan/query-plan/runbook guards, then the workflow writes the operations readiness report and uploads the resulting artifact set.
...
- `../.github/workflows/manual-operations-handoff-package.yml` - manual GitHub Actions workflow that wraps `write-operations-handoff-package.ps1`, accepts base64 operations readiness/convergence/data-flow storage plan/data-flow storage transition runbook/commercial/enterprise-auth evidence snapshots, records target operations handoff package references without running external systems, deletes decoded temporary snapshot files, and uploads `operations-handoff-package-<run_id>`.
...
- `../scripts/write-data-flow-storage-transition-runbook-evidence.ps1` - write target backfill, dual-write or partition toggle, rollback, reconciliation, dashboard cutover, and retention dry-run rehearsal evidence for a passed data-flow storage transition plan without storing raw SQL, raw EXPLAIN, object keys, raw event messages, or credentials.
- `../scripts/verify-data-flow-storage-transition-runbook-evidence.ps1` - self-test the storage transition runbook evidence writer, including passed evidence, secret-shaped reference rejection, reversed review window rejection, and unsafe raw SQL plan rejection.
...
- `../scripts/verify-browser-e2e-local-demo.ps1` - start the full Docker local demo, seed demo data, verify REST/S3 demo smoke, run Playwright Browser E2E against the Docker frontend, auto-select installed Chrome/Edge when needed, and stop the Docker stack. `-EnableRealMultipartFixture -TestGrep "real MinIO multipart"` rebuilds the frontend with small multipart threshold/5 MiB part size, uses the seeded writable demo bucket, delays real MinIO PUT requests through Playwright `route.continue()`, and verifies browser pause/resume against real MinIO rather than API mocks.
...
- `../scripts/verify-s3-compatibility-boundary.ps1` - verify S3 support stays scoped to replacement-layer compatibility, rejects overbroad English/Korean AWS S3 parity claims, and keeps README, API spec, feature inventory, status, and release gates tied to `dev-docs/s3-compatibility.md`.
