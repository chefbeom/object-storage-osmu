...
- `../.github/workflows/manual-data-flow-storage-plan-evidence.yml` - manual GitHub Actions workflow that wraps `write-data-flow-storage-plan.ps1`, accepts target sizing/retention/backfill/rollback/dashboard-cutover confirmations plus optional base64 sanitized MariaDB query-plan evidence, deletes the decoded query-plan input before upload, and uploads `data-flow-storage-plan-evidence-<run_id>`.
