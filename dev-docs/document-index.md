...
- `../scripts/verify-operations-readiness-artifact-import.ps1` - self-test the operations readiness artifact importer, including rejection of invalid failed evidence and MariaDB/dual-write data-flow plans missing query-plan evidence summary.
...
- `../scripts/verify-kubernetes-operations-report-sync.ps1` - self-test Kubernetes operations report ConfigMap sync using a fake kubectl for plan, server dry-run, apply, and data-flow query-plan summary paths.
...
- `../.github/workflows/kubernetes-operations-report-sync-ci.yml` - manual GitHub Actions workflow for operations convergence report generation plus optional data-flow storage plan restore/query-plan summary validation, Kubernetes ConfigMap sync plan, server-dry-run, and guarded apply evidence.
