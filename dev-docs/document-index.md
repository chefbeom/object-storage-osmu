...
- `../.github/workflows/operations-readiness-artifact-finalizer-ci.yml` - manual GitHub Actions workflow that downloads evidence artifacts from previous storage expansion, HA/DR, Kubernetes DR, IAM/RBAC, security, storage backend telemetry, data-flow storage plan, secret rotation, commercial integration, commercial approval, enterprise auth smoke, operations handoff package, and optional Kubernetes operations report sync workflow runs; optional `data_flow_storage_plan_json_base64` imports direct data-flow storage transition evidence through the same sanitized query-plan guard, then the workflow writes the operations readiness report and uploads the resulting artifact set.
