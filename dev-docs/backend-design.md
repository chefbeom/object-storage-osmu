...
- `BillingPricingPolicyService` owns the internal chargeback pricing policy and ADMIN-only pricing proposal workflow. Proposals start as `PENDING_APPROVAL`; internal approval changes the active calculation policy as `APPROVED_APPLIED`, and a later commercial approval records an external price-list reference as `PRICE_LIST_APPROVED`/`approvedPriceList=true` without calling payment providers, notification adapters, or storing credentials.
