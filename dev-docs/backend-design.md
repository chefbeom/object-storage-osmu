...
- `GlobalExceptionHandler` returns AWS-style XML error bodies for `/api/s3/**` with `Code`, `Message`, `Resource`, `RequestId`, and an opaque deterministic `HostId` derived from request id plus resource while normal REST API errors stay JSON. `Resource` uses the request path plus query string.
