...
- `DataFlowMonitoringService` exposes detailed data-flow snapshots, CSV export, an ADMIN-only UTC daily rollup, daily rollup CSV export, and a daily rollup materialization endpoint. The daily rollup aggregates by day, bucket, source, and operation from `data_flow_events` in MariaDB mode or runtime events in in-memory mode; materialization upserts aggregate-only rows into `data_flow_daily_rollups` so chargeback/operations analytics has a stable handoff point before table partitioning or a dedicated time-series repository is introduced.
