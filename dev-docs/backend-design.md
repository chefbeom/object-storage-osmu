...
- `ObjectLifecycleS3XmlService` exports/imports AWS S3 LifecycleConfiguration XML subset for lifecycle rule interoperability and validates the lifecycle root, 1000-rule limit, 255-character `Rule/ID` limit, required `Rule/Status` values (`Enabled` or `Disabled`), direct `Filter` predicate shape, lifecycle tag restrictions, unsupported object-size predicates, and unsupported lifecycle action combinations.
