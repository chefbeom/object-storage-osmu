...
- `DataFlowMonitoringService` exposes detailed data-flow snapshots, CSV export, and an ADMIN-only UTC daily rollup. The daily rollup aggregates by day, bucket, source, and operation from `data_flow_events` in MariaDB mode or runtime events in in-memory mode, giving chargeback/operations analytics a stable API before table partitioning or a dedicated time-series repository is introduced.
