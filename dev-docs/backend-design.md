...
- `DataFlowMonitoringService` exposes detailed data-flow snapshots, CSV export, an ADMIN-only UTC daily rollup, daily rollup CSV export, daily rollup materialization, materialized rollup readback, materialized rollup CSV export, and UTC monthly rollup JSON/CSV from either live events or materialized daily rows. The daily rollup aggregates by day, bucket, source, and operation from `data_flow_events` in MariaDB mode or runtime events in in-memory mode; materialization upserts aggregate-only rows into `data_flow_daily_rollups` with actor/status filter dimensions so chargeback/operations analytics has a stable read/write handoff point before table partitioning or a dedicated time-series repository is introduced. The monthly rollup groups aggregate-only rows by month for longer-term operations analytics without storing object keys or raw messages.
