...
- S3 multipart complete request XML requires a non-empty, strictly ascending, unique part list with `PartNumber` in 1~10000 and non-blank `ETag`, parses optional per-part checksum elements into `CompletedMultipartUploadPart.checksums()`, and validates checksum syntax before delegating to storage. The MinIO completion call still uses part number and ETag only in the MVP.
