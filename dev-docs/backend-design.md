...
- `AdminController` exposes `GET /api/admin/storage/backend-status` for `ADMIN` and `AUDITOR` as a read-only OSMU operations status surface. It combines object storage health, access-key provisioner health, bucket/object metadata counts, and optional direct MinIO Prometheus capacity metrics. When `OSMU_STORAGE_METRICS_ENABLED=true`, `MinioPrometheusStorageMetricsProvider` reads the configured MinIO metrics endpoint and maps total/free capacity into `capacitySource=minio_prometheus_metrics` and `readiness=DIRECT_METRICS_READY`; otherwise the status keeps the metadata-usage fallback and reports the direct metrics gate as disabled/unavailable.
