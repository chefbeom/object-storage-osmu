...
- `S3ErrorCodeMapper` normalizes generic non-bucket S3 XML messages for `BadDigest`/`InvalidDigest` Content-MD5 failures, `EntityTooLarge`, `OperationAborted`, `InternalError`, missing content length, and incomplete body length; non-MD5 checksum failures keep checksum-specific detail. `MissingContentLength` uses HTTP `411`, while `IncompleteBody` remains HTTP `400`.
