...
- S3 multipart complete accepts one final object `x-amz-checksum-*` value header, validates it against the completed storage object before metadata commit, stores the checksum metadata, echoes the checksum response header, and writes matching complete-result XML checksum elements. When no final object checksum header is supplied and every completed part has the same `ChecksumSHA256` or `ChecksumSHA1`, `ObjectService` stores the AWS-style composite checksum by digesting the ordered part checksum bytes.
