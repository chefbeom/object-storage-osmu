...
- `AdminController` exposes `GET /api/admin/storage/backend-status` for `ADMIN` and `AUDITOR` as a read-only OSMU operations status surface. It combines object storage health, access-key provisioner health, and bucket metadata usage totals; direct MinIO Admin capacity metrics are intentionally disabled in the MVP (`minioAdminMetricsEnabled=false`).
