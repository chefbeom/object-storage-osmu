...
- `S3BucketLifecycleController` exposes `/api/s3/{bucketName}?lifecycle` as a path-style S3 lifecycle alias backed by the same bucket lifecycle service and maps missing/blank lifecycle XML to S3 `MissingRequestBodyError`.
