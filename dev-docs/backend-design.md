...
- S3 multipart complete request XML requires a non-empty, strictly ascending, unique part list with `PartNumber` in 1~10000 and non-blank `ETag`, parses optional per-part checksum elements into `CompletedMultipartUploadPart.checksums()`, validates checksum syntax, derives SHA1/SHA256/CRC32/CRC32C composite metadata when every part supplies the same algorithm, and checks uploaded part existence plus ETag match before delegating to storage. The MinIO completion call still uses part number and ETag only in the MVP.
