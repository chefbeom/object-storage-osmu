...
- `ChargebackPreviewService` builds an API-first tenant cost pre-model from current organization-owned bucket usage plus bounded `data_flow_events`. `ADMIN` receives every organization; `ORG_ADMIN` receives only the caller organization. It can export preview CSV, draft invoice CSV, persist internal draft invoice review records, approve those records as `APPROVED_INTERNAL`, finalize approved drafts as final chargeback invoices, move them through `PAYMENT_REQUESTED` and `PAID`, build no-send payment provider handoff payloads, record payment handoff outbox/history rows with `PENDING_PAYMENT_PROVIDER_ADAPTER`, record handoff adapter result/retry states, build threshold notification payload previews, record scoped notification outbox/history rows with `PENDING_DELIVERY_ADAPTER`, record notification adapter result/retry states, and let `ADMIN` attempt configured webhook delivery through `POST /api/admin/billing/chargeback-alert-notifications/outbox/{deliveryId}/adapter-send`. It still does not call external payment processors, tax invoice providers, email/Slack-specific adapters, or store credentials/raw provider responses.
- `ChargebackAdapterRetryWorkerService` scans due notification delivery rows and payment provider handoff rows through repository `findDueAdapterRetries` methods. The ADMIN status API is dry-run only; the run API and disabled-by-default scheduled job attempt configured notification webhook delivery for due notification rows, otherwise block them for missing configuration, while payment provider rows still move to credential/configuration blocked states without external calls. It increments attempts, clears or schedules `nextAttemptAt`, emits counters, and audits the action without storing secrets/raw provider responses.
