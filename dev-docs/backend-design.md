...
- `ChargebackPreviewService` builds an API-first tenant cost pre-model from current organization-owned bucket usage plus bounded `data_flow_events`. `ADMIN` receives every organization; `ORG_ADMIN` receives only the caller organization. It can export preview CSV, draft invoice CSV, build no-send threshold notification payload previews, and record scoped notification outbox/history rows with `PENDING_DELIVERY_ADAPTER`, but it does not persist final invoices or deliver external notifications.
