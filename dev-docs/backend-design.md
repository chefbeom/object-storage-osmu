...
- `S3ErrorCodeMapper` normalizes generic non-bucket S3 XML messages for `BadDigest`/`InvalidDigest` Content-MD5 failures, `EntityTooLarge`, `OperationAborted`, `InternalError`, and missing content length; non-MD5 checksum failures keep checksum-specific detail. `MissingContentLength` uses HTTP `411`.
