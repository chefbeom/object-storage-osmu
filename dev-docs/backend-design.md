...
- `ChargebackPreviewService` builds an API-first tenant cost pre-model from current organization-owned bucket usage plus bounded `data_flow_events`. `ADMIN` receives every organization; `ORG_ADMIN` receives only the caller organization. It can export preview CSV and draft invoice CSV, but it does not persist final invoices.
