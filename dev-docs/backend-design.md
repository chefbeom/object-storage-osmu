...
- `GlobalExceptionHandler` returns AWS-style XML error bodies for `/api/s3/**` with `Code`, `Message`, `Resource`, `RequestId`, and `HostId` while normal REST API errors stay JSON. `Resource` uses the request path plus query string, and MVP `HostId` mirrors the request id.
