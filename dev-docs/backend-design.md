...
- S3 object `PUT` validates non-streaming signed `x-amz-content-sha256` against the actual request body. `UNSIGNED-PAYLOAD` is accepted without body hash validation. AWS `aws-chunked` request bodies are decoded when `x-amz-decoded-content-length` is present and mismatched decoded length is rejected, but the per-chunk `chunk-signature` chain is not cryptographically verified yet.
