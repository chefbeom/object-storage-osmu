...
- `AdminEnterpriseAuthPlanController` exposes `POST /api/admin/security/enterprise-auth/jit-provision` as an admin-only JIT apply endpoint. `OidcJitProvisioningService` reuses the preview result, rejects disallowed domains and missing required claims, requires explicit approval for privileged roles, validates organization assignment, creates an `ACTIVE` local user with a random non-disclosed password hash, and records `OIDC_JIT_PROVISION` audit evidence without storing raw claims. `write-enterprise-auth-jit-rollback-evidence.ps1` records the matching production rollback/runbook evidence for disabling or locking JIT users, correcting role/org/team mappings, validating local-login fallback, and reviewing JIT audit events without storing raw claims or secrets, and the dashboard readiness API exposes a reduced JIT rollback summary when `.osmu-run/latest-enterprise-auth-jit-rollback-evidence.json` exists.
...
- `DataFlowMonitoringService` exposes detailed data-flow snapshots, CSV export, an ADMIN-only UTC daily rollup, daily rollup CSV export, daily rollup materialization, materialized rollup readback, materialized rollup CSV export, UTC monthly rollup JSON/CSV from either live events or materialized daily rows, stored monthly rollup materialize/read/export APIs, and a storage-status readiness view. The daily rollup aggregates by day, bucket, source, and operation from `data_flow_events` in MariaDB mode or runtime events in in-memory mode; materialization upserts aggregate-only rows into `data_flow_daily_rollups` with actor/status filter dimensions. Monthly materialization compacts those aggregate-only daily rows into `data_flow_monthly_rollups` so long-window operations analytics has a dedicated read/write handoff point before table partitioning or an external time-series repository is introduced. The storage-status view reports repository health, detailed/daily/monthly row counts, scan/window limits, and `partitionedOrTimeSeriesStoreEnabled=false` for the current MVP boundary. The monthly surfaces do not store object keys or raw messages. `write-data-flow-storage-plan.ps1` captures the target sizing, query window, target p95 query latency budget, backfill, rollback, dashboard-cutover, retention-budget, and no-object-key policy evidence required before that boundary can move to partitioned MariaDB, external time-series storage, or dual-write mode. `GET /api/admin/dashboard/readiness` now surfaces `.osmu-run/latest-data-flow-storage-plan.json` as `dataFlowStoragePlan` plus a `DATA_FLOW_STORAGE_PLAN` warning when target evidence is still pending.
- `write-data-flow-storage-plan.ps1` links future MariaDB partition or dual-write candidates to the sanitized `.osmu-run/latest-mariadb-query-plan-evidence.json` summary from `write-mariadb-query-plan-evidence.ps1`. The readiness DTO preserves the query-plan result/count/failed-check summary but not raw SQL, EXPLAIN JSON, or credentials; external time-series candidates still satisfy the same gate through target-store benchmark evidence.
- `write-data-flow-query-retention-budget-evidence.ps1` sits after a passed storage plan and records target benchmark evidence for observed p95/p99 query latency, sample count, query-window coverage, and detailed/daily/monthly retention job durations against a shared budget. It stores only aggregate metrics, reduced storage-plan summary, confirmations, and external references, then rejects raw SQL, raw EXPLAIN, object keys, raw event messages, and credential-shaped values.
- `write-data-flow-storage-transition-runbook-evidence.ps1` sits after a passed storage plan and records target backfill rehearsal, dual-write or partition toggle review, rollback rehearsal, row/count reconciliation, dashboard cutover review, and retention dry-run references. It stores only reduced plan summary fields and external references, then rejects raw SQL, raw EXPLAIN, object keys, raw event messages, and credential-shaped values so the handoff package can reference transition rehearsal evidence without copying sensitive operational material.
- `AdminController` exposes `GET /api/admin/storage/backend-status` for `ADMIN` and `AUDITOR` as a read-only OSMU operations status surface. It combines object storage health, access-key provisioner health, bucket/object metadata counts, optional direct MinIO Prometheus capacity metrics, and the latest passed storage backend telemetry evidence. When `OSMU_STORAGE_METRICS_ENABLED=true`, `MinioPrometheusStorageMetricsProvider` reads the configured MinIO metrics endpoint and maps total/free capacity into `capacitySource=minio_prometheus_metrics` and `readiness=DIRECT_METRICS_READY`; if direct metrics are unavailable in MinIO mode but `.osmu-run/latest-storage-backend-telemetry.json` has `result=passed` and capacity totals, the status uses `capacitySource=storage_backend_telemetry_evidence` and `readiness=TELEMETRY_EVIDENCE_READY`; otherwise it keeps the metadata-usage fallback and reports the direct metrics or telemetry evidence gate as disabled/unavailable.
...
- `DataFlowDailyRollupRetentionJob` deletes old `data_flow_daily_rollups` rows in bounded batches and records `DATA_FLOW_DAILY_ROLLUP_RETENTION` audit plus `osmu.data.flow.daily.rollup.retention.rows{result=success}` and `osmu.data.flow.daily.rollup.retention.runs{result=failure}` metrics. Default retention is longer than detailed event retention (`1095` days) so aggregate analytics can survive event cleanup.
- `DataFlowMonthlyRollupRetentionJob` deletes old `data_flow_monthly_rollups` rows in bounded batches and records `DATA_FLOW_MONTHLY_ROLLUP_RETENTION` audit plus `osmu.data.flow.monthly.rollup.retention.rows{result=success}` and `osmu.data.flow.monthly.rollup.retention.runs{result=failure}` metrics. Default retention is `1825` days so long-window monthly analytics survives event and daily-row cleanup.
- `AdminDataFlowRetentionController` exposes ADMIN-only status/manual run endpoints for event, daily-rollup, and monthly-rollup retention jobs, returning configured days/batch size, job availability, accumulated retention counters, and manual run deletion counts without changing the analytics storage contract.
- `BillingPricingPolicyService` owns the internal chargeback pricing policy and ADMIN-only pricing proposal workflow. Proposals start as `PENDING_APPROVAL`; internal approval changes the active calculation policy as `APPROVED_APPLIED`, and a later commercial approval records an external price-list reference as `PRICE_LIST_APPROVED`/`approvedPriceList=true` without calling payment providers, notification adapters, or storing credentials. The paired commercial approval summary endpoint returns a reduced evidence snapshot and omits rates, terms, approval notes, raw price tables, contracts, customer data, license keys, payment data, and secrets.
- `ChargebackPreviewService` builds an API-first tenant cost pre-model from current organization-owned bucket usage plus bounded `data_flow_events`, and also exposes a scoped daily chargeback trend plus CSV export from live or materialized data-flow daily rollups. `ADMIN` receives every organization; `ORG_ADMIN` receives only the caller organization. It can export preview CSV, daily rollup trend CSV, draft invoice CSV, persist internal draft invoice review records, approve those records as `APPROVED_INTERNAL`, finalize approved drafts as final chargeback invoices, move them through `PAYMENT_REQUESTED` and `PAID`, build payment provider handoff payloads with a `providerProfile` of `CARD`, `BANK`, `TAX`, `ERP`, or `GENERIC`, record payment handoff outbox/history rows with `PENDING_PAYMENT_PROVIDER_ADAPTER`, record handoff adapter result/retry states, expose an ADMIN-only payment-provider adapter readiness view for generic/CARD/BANK/TAX/ERP profiles, expose an ADMIN-only reduced chargeback closeout summary for evidence collection, let `ADMIN` attempt configured payment-provider adapter delivery through `POST /api/admin/billing/chargeback-payment-provider-handoffs/{handoffId}/adapter-send`, build threshold notification payload previews, record scoped notification outbox/history rows with `PENDING_DELIVERY_ADAPTER`, record notification adapter result/retry states, and let `ADMIN` attempt configured generic notification webhook delivery, `SLACK` channel Slack webhook delivery, or `EMAIL` channel SMTP relay delivery through `POST /api/admin/billing/chargeback-alert-notifications/outbox/{deliveryId}/adapter-send`. `CompositeChargebackPaymentProviderAdapter` first routes to a configured `ChargebackNativePaymentProviderAdapter` for the provider profile, then falls back to `WebhookChargebackPaymentProviderAdapter`; the built-in `ConfigurableChargebackNativePaymentProviderAdapter` provides CARD/BANK/TAX/ERP native API bridge profiles with per-provider endpoint URL/auth-header value settings, shared timeout/payload/private-network policy, and optional HMAC-SHA256 signature headers. Webhook, native bridge, and SMTP relay endpoint policy rejects private/local hosts by default unless the matching `allow-private-network=true` setting is explicitly set, outbound notification/payment/native adapter payloads are size-capped before external calls, and generic notification/payment webhooks plus the native bridge can add timestamped HMAC-SHA256 signature headers without storing signing secrets. The readiness view reports webhook-ready and native-ready profile counts and never exposes webhook/native endpoint URLs, secret/auth header values, signing secrets, credentials, or raw provider responses.
- `ChargebackAdapterRetryWorkerService` scans due notification delivery rows and payment provider handoff rows through repository `findDueAdapterRetries` methods. The ADMIN status API is dry-run only; the run API and disabled-by-default scheduled job attempt configured generic notification webhook delivery, `SLACK` channel Slack webhook delivery, `EMAIL` channel SMTP relay delivery, and configured payment-provider native-bridge-or-webhook handoff delivery for due rows, otherwise block rows for missing adapter configuration. It increments attempts, clears or schedules `nextAttemptAt`, emits counters, and audits the action without storing secrets/raw provider responses.
...
- `BucketService` exposes explicit bucket versioning management through storage adapter calls. `MinioObjectStorageAdapter` maps OSMU `ENABLED`/`SUSPENDED` to MinIO bucket versioning, while `InMemoryObjectStorageAdapter` stores runtime status for tests and lightweight demos. This is an OSMU operations control surface, not AWS S3 versioning parity.
- Browser multipart upload는 part PUT 응답의 `ETag`를 읽어야 하므로 `MinioBucketCorsProvisioner`와 local Compose CORS JSON이 `ExposeHeaders`에 `ETag`, `x-amz-request-id`, `x-amz-id-2`, `x-amz-version-id`를 포함한다. 운영 검증은 `scripts/verify-minio-bucket-cors.ps1`가 `mc cors info <alias>/<bucket>` 또는 operator-provided CORS XML을 읽어 `.osmu-run/latest-minio-bucket-cors-verification.*`에 요약한다. Docker local demo의 real browser pause/resume path는 `scripts/verify-browser-e2e-local-demo.ps1 -EnableRealMultipartFixture -TestGrep "real MinIO multipart"`로 seed demo bucket과 real MinIO presigned part PUT을 사용해 검증한다. 이는 browser upload 운영 조건 검증이며 AWS S3 세부 parity 확장이 아니다.
...
- MinIO와 MariaDB는 path별 best-effort 보상으로 맞춘다. 원래 실패 원인을 사용자에게 유지하고, cleanup/rollback 실패는 운영 sync/retention 작업으로 재조정한다.
...
- 파일 업로드: storage PUT 성공 후 metadata 저장이 실패하면 새 active object를 삭제하거나 이전 snapshot을 active object로 복원하고, 반영된 bucket usage/object count delta를 best-effort로 되돌린다. 감사 로그 실패는 파일 업로드 성공으로 처리하고 로그 에러만 기록.
