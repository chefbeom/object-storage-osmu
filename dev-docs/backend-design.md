...
- `AdminDataFlowRetentionController` exposes ADMIN-only status/manual run endpoints for both data-flow retention jobs, returning configured days/batch size, job availability, accumulated retention counters, and manual run deletion counts without changing the analytics storage contract.
