...
- Storage Expansion finalization wrapper 구현 완료: `scripts/finalize-storage-expansion.ps1`는 RBAC auth evidence, server-side dry-run evidence, optional backend dry-run runner, optional backend apply runner 호출을 하나의 `.osmu-run/latest-storage-expansion-finalize.json`/`.md` 보고서로 묶는다. 실제 apply는 `-RunBackendApply -ConfirmApply`가 함께 있고 같은 실행에서 RBAC/server-side dry-run evidence를 스킵하지 않을 때만 진행되며 admin password와 bearer token은 evidence에 기록하지 않는다.
