...
- Storage backend telemetry evidence writer 구현 완료: `scripts/write-storage-backend-telemetry-evidence.ps1`는 `mc admin info --json` file input 또는 명시적 `-Execute` 결과를 pool/server/drive summary, online/offline state, total/used/free bytes, input SHA-256으로 요약해 `.osmu-run/latest-storage-backend-telemetry.*`에 남긴다. `.github/workflows/manual-storage-backend-telemetry-evidence.yml`는 준비된 `OSMU_MINIO_ADMIN_INFO_JSON_BASE64` 입력을 임시 파일로 디코드해 같은 writer를 실행하고 요약 artifact만 업로드한다. raw admin output, credential, bearer token, MinIO root credential은 저장하지 않는다.
