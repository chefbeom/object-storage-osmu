...
| S3 XML error envelope | MVP supported | Global `/api/s3/**` errors include `Code`, `Message`, per-error `BucketName`/`Key`/`UploadId` details when derivable, `Resource`, `RequestId`, and an opaque deterministic `HostId` derived from request id plus resource. `AccessDenied`, `InvalidRange`, `NoSuchBucket`, `NoSuchKey`, `NoSuchUpload`, and `PreconditionFailed` messages are normalized toward AWS-style text. | `S3ObjectControllerTest.missingS3MultipartUploadReturnsNoSuchUploadXml`, `S3ObjectControllerTest.accessKeyScopeControlsS3StyleObjectActions`, `S3ObjectControllerTest.accessKeyCanUseRangeGetThroughS3StylePath`, `S3ObjectControllerTest.accessKeyCanPutHeadGetAndDeleteObjectThroughS3StylePath` |
