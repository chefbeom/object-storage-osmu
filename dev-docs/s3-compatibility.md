...
| Multipart initiate/upload/list/complete/abort | MVP supported | S3 initiate accepts AWS-style unknown-size sessions when the OSMU expected-size header is omitted; these sessions have no precomputed part URL/byte plan and quota is checked on complete using actual completed object size. Initiate validates, persists, and echoes supported `x-amz-checksum-algorithm`/`x-amz-checksum-type` checksum negotiation headers, rejects unsupported ACL/grant/Object Lock/SSE/non-standard storage class/website redirect/requester-pays controls, and accepts safe no-op `private` ACL plus `STANDARD` storage class. Missing uploads return `NoSuchUpload`. ListParts supports `max-parts`/`part-number-marker` pagination. Complete rejects unsupported `x-amz-checksum-algorithm`, requester-pays, expected-bucket-owner, and SSE-C customer headers before storage completion. Complete XML validates the `CompleteMultipartUpload` root, destination `If-Match`/`If-None-Match`, non-empty ascending unique part numbers, 1~10000 range, required ETag, stored initiate checksum algorithm/type match, uploaded part existence, ETag match, non-last part size, and optional `x-amz-mp-object-size` actual-size match before metadata commit. Unexpected complete XML roots return `MalformedXML`; out-of-order/duplicate part numbers return `InvalidPartOrder`; missing uploaded parts or stale part ETags return `InvalidPart`; uploaded non-last parts below 5 MiB return `EntityTooSmall`. MinIO-backed complete preserves the AWS-style multipart ETag (`md5-of-part-md5s-partCount`) and smoke scripts verify it in complete response XML/header and later `HEAD`. |
