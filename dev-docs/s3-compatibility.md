...
| `GET/PUT/DELETE Bucket lifecycle` | MVP supported | S3 XML lifecycle subset mapped to OSMU lifecycle rules. Exported XML emits `ID`, `Filter`, `Status`, action child order. Missing configuration returns `NoSuchLifecycleConfiguration`; missing or blank XML body returns `MissingRequestBodyError`; unexpected roots, missing/unsupported `Rule/Status` values, and invalid direct `Filter` shapes return `MalformedXML`; too many rules, overlong `Rule/ID`, lifecycle tag restriction violations, unsupported object-size predicates, unsupported lifecycle actions, multiple target actions in one rule, and `x-amz-transition-default-minimum-object-size` return `InvalidRequest`. |
