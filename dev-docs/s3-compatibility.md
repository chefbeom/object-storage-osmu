...
| S3 XML error envelope | MVP supported | Global `/api/s3/**` errors include `Code`, `Message`, per-error `BucketName`/`Key`/`UploadId` details when derivable, `Resource`, `RequestId`, and an opaque deterministic `HostId` derived from request id plus resource. `AccessDenied`, `InvalidRange`, `NoSuchBucket`, `NoSuchKey`, `NoSuchUpload`, `NoSuchLifecycleConfiguration`, `PreconditionFailed`, `InvalidBucketName`, `BucketAlreadyOwnedByYou`, `BucketAlreadyExists`, `BucketNotEmpty`, `InvalidPart`, `InvalidPartOrder`, `EntityTooSmall`, `EntityTooLarge`, `OperationAborted`, `InternalError`, `MissingContentLength`, `IncompleteBody`, `MalformedXML`, and `MissingRequestBodyError` messages/statuses are normalized toward AWS-style behavior. | `S3ObjectControllerTest.missingS3MultipartUploadReturnsNoSuchUploadXml`, `S3ObjectControllerTest.missingS3ObjectContentLengthReturnsMissingContentLengthXml`, `S3ObjectControllerTest.mismatchedS3ObjectContentLengthReturnsIncompleteBodyXml`, `S3ObjectControllerTest.createBucketRejectsInvalidCreateBucketConfigurationXml`, `BucketTaggingControllerTest.malformedS3BucketTaggingXmlReturnsMalformedXml`, `BucketTaggingControllerTest.missingS3BucketTaggingBodyReturnsMissingRequestBodyError`, `BucketLifecycleControllerTest.adminCanUseS3StyleLifecycleQueryAlias`, `S3ObjectControllerTest.completeMultipartUploadInvalidPartOrderReturnsInvalidPartOrderXml`, `S3ObjectControllerTest.createBucketReturnsS3DuplicateBucketCodes`, `S3ObjectControllerTest.bucketLevelRequestsRejectInvalidS3BucketNames`, `S3ObjectControllerTest.deleteBucketReturnsBucketNotEmptyForNonEmptyBucket`, `S3ObjectControllerTest.accessKeyCanPutHeadGetAndDeleteObjectThroughS3StylePath` |
...
| `GET/PUT/DELETE Bucket lifecycle` | MVP supported | S3 XML lifecycle subset mapped to OSMU lifecycle rules. Exported XML emits `ID`, `Filter`, `Status`, action child order. PUT validates optional `Content-MD5`, one explicit `x-amz-checksum-*` header, and matching `x-amz-sdk-checksum-algorithm` against the raw lifecycle XML before replacement. Missing configuration returns `NoSuchLifecycleConfiguration`; missing or blank XML body returns `MissingRequestBodyError`; unexpected roots, missing/unsupported `Rule/Status` values, and invalid direct `Filter` shapes return `MalformedXML`; invalid checksum syntax returns `InvalidDigest`; checksum mismatch returns `BadDigest`; too many rules, overlong `Rule/ID`, lifecycle tag restriction violations, unsupported object-size predicates, unsupported lifecycle actions, multiple target actions in one rule, and `x-amz-transition-default-minimum-object-size` return `InvalidRequest`. |
...
| Object tagging | Supported | S3 XML `Tagging/TagSet` subset backed by OSMU object tags. Missing or blank XML body returns `MissingRequestBodyError`; unexpected roots return `MalformedXML`. |
| ListObjects V1/V2 | Supported | Prefix, delimiter, marker/continuation token, max keys, URL encoding, owner field. |
| Multi-object delete | MVP supported | Uses soft-delete; requires the S3 `Delete` XML root, supports quiet mode, and reports per-key errors. Unexpected roots return `MalformedXML` before delete execution. |
| Multipart initiate/upload/list/complete/abort | MVP supported | S3 initiate accepts AWS-style unknown-size sessions when the OSMU expected-size header is omitted; these sessions have no precomputed part URL/byte plan and quota is checked on complete using actual completed object size. Initiate validates, persists, and echoes supported `x-amz-checksum-algorithm`/`x-amz-checksum-type` checksum negotiation headers, rejects unsupported ACL/grant/Object Lock/SSE/non-standard storage class/website redirect/requester-pays controls, and accepts safe no-op `private` ACL plus `STANDARD` storage class. Missing uploads return `NoSuchUpload`. ListParts supports `max-parts`/`part-number-marker` pagination. Complete rejects unsupported `x-amz-checksum-algorithm`, requester-pays, expected-bucket-owner, and SSE-C customer headers before storage completion. Complete XML validates the `CompleteMultipartUpload` root, direct root `Part` children, destination `If-Match`/`If-None-Match`, non-empty ascending unique direct part numbers, 1~10000 range, exactly one direct ETag, stored initiate checksum algorithm/type match, uploaded part existence, ETag match, non-last part size, and optional `x-amz-mp-object-size` actual-size match before metadata commit. Unexpected complete XML roots, non-`Part` root children, nested required part fields, or duplicate direct `PartNumber`/`ETag` fields return `MalformedXML`; out-of-order/duplicate part numbers return `InvalidPartOrder`; missing uploaded parts or stale part ETags return `InvalidPart`; uploaded non-last parts below 5 MiB return `EntityTooSmall`. MinIO-backed complete preserves the AWS-style multipart ETag (`md5-of-part-md5s-partCount`) and smoke scripts verify it in complete response XML/header and later `HEAD`. |
