...
| `aws-chunked` body decoding | MVP supported | Decodes AWS chunked transfer bodies and stores the decoded object. Requires `x-amz-decoded-content-length` and rejects decoded length mismatch. | `S3ObjectControllerTest.accessKeyCanUploadAwsChunkedStreamingPayload` |
| Chunk signature chain | Not fully supported | `chunk-signature` fields are parsed as chunk extensions but not cryptographically chained or verified. Treat this as real-client compatibility, not full AWS streaming signature parity. | tracked as gap |
...
| Multipart initiate/upload/list/complete/abort | MVP supported | Requires OSMU expected-size header for quota/session planning. Complete XML validates non-empty ascending unique part numbers, 1~10000 range, and required ETag. Unknown-size AWS initiate parity is not supported. |
...
| `x-amz-checksum-sha256` | Supported | PUT, multipart part, final object metadata/header/XML. |
| `x-amz-checksum-sha1` | Supported | PUT, multipart part, final object metadata/header/XML. |
| `x-amz-checksum-crc32` | Supported | PUT, multipart part, final object metadata/header/XML. |
| `x-amz-checksum-crc32c` | Supported | PUT, multipart part, final object metadata/header/XML. |
