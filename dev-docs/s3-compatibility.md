...
| `GET/PUT/DELETE Bucket lifecycle` | MVP supported | S3 XML lifecycle subset mapped to OSMU lifecycle rules. Missing configuration returns `NoSuchLifecycleConfiguration`; missing or blank XML body returns `MissingRequestBodyError`; unexpected roots, missing/unsupported `Rule/Status` values, and invalid direct `Filter` shapes return `MalformedXML`; lifecycle tag restriction violations, unsupported object-size predicates, unsupported lifecycle actions, and multiple target actions in one rule return `InvalidRequest`. |
