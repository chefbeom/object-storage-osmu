...
| S3 XML error envelope | MVP supported | Global `/api/s3/**` errors include `Code`, `Message`, per-error `BucketName`/`Key`/`UploadId` details when derivable, `Resource`, `RequestId`, and an opaque deterministic `HostId` derived from request id plus resource. `AccessDenied`, `InvalidRange`, `NoSuchBucket`, `NoSuchKey`, `NoSuchUpload`, `PreconditionFailed`, `InvalidBucketName`, `BucketAlreadyOwnedByYou`, `BucketAlreadyExists`, `BucketNotEmpty`, `InvalidPart`, `InvalidPartOrder`, `EntityTooSmall`, `EntityTooLarge`, `OperationAborted`, `InternalError`, `MissingContentLength`, `IncompleteBody`, and `MalformedXML` messages/statuses are normalized toward AWS-style behavior. | `S3ObjectControllerTest.missingS3MultipartUploadReturnsNoSuchUploadXml`, `S3ObjectControllerTest.missingS3ObjectContentLengthReturnsMissingContentLengthXml`, `S3ObjectControllerTest.mismatchedS3ObjectContentLengthReturnsIncompleteBodyXml`, `S3ObjectControllerTest.createBucketRejectsInvalidCreateBucketConfigurationXml`, `BucketTaggingControllerTest.malformedS3BucketTaggingXmlReturnsMalformedXml`, `S3ObjectControllerTest.completeMultipartUploadInvalidPartOrderReturnsInvalidPartOrderXml`, `S3ObjectControllerTest.createBucketReturnsS3DuplicateBucketCodes`, `S3ObjectControllerTest.bucketLevelRequestsRejectInvalidS3BucketNames`, `S3ObjectControllerTest.deleteBucketReturnsBucketNotEmptyForNonEmptyBucket`, `S3ObjectControllerTest.accessKeyCanPutHeadGetAndDeleteObjectThroughS3StylePath` |
