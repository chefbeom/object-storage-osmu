...
| Chargeback alert notification outbox | `GET/POST /api/admin/billing/chargeback-alert-notifications/outbox`, `POST /api/admin/billing/chargeback-alert-notifications/outbox/{deliveryId}/adapter-result` | Allow | Queue/List only | Deny | Deny | threshold alert와 같은 scope로 delivery outbox/history를 기록/조회하며 adapter result/retry state 기록은 `ADMIN`만 수행. 실제 외부 전송은 adapter 구성 전까지 수행하지 않음 | `AdminRbacPolicy`, `AdminBillingController`, `ChargebackPreviewService` |
