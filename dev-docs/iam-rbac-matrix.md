...
| Chargeback preview/trend | `GET /api/admin/billing/chargeback-preview`, `GET /api/admin/billing/chargeback-daily-rollup` | Allow | Allow | Deny | Deny | `ORG_ADMIN`은 자기 조직 chargeback preview와 data-flow daily rollup 기반 일별 trend만 조회 | `AdminRbacPolicy`, `AdminBillingController`, `ChargebackPreviewService` |
