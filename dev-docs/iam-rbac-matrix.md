...
| Chargeback alert notification outbox | `GET/POST /api/admin/billing/chargeback-alert-notifications/outbox` | Allow | Allow | Deny | Deny | threshold alert와 같은 scope로 delivery outbox/history를 기록/조회하며 실제 외부 전송은 adapter 구성 전까지 수행하지 않음 | `AdminRbacPolicy`, `AdminBillingController`, `ChargebackPreviewService` |
...
- `AdminBillingControllerTest`, `ChargebackPreviewServiceTest`: `ADMIN`/`ORG_ADMIN` chargeback preview/export/draft invoice export/threshold alert/notification preview/outbox scope, ADMIN-only invoice draft persistence/internal approval/final invoice/payment workflow and pricing policy proposal approval, pricing policy save/defaulting, data-flow event cost pre-model, unsupported role denial 검증.
