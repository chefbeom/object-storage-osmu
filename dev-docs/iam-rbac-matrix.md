...
| `ORG_ADMIN` | 조직 관리자 | 자기 조직 사용자/조직 usage/chargeback preview, 자기 조직 bucket 관리, 일반 사용자 생성/비활성화 |
...
- `AdminBillingControllerTest`, `ChargebackPreviewServiceTest`: `ADMIN`/`ORG_ADMIN` chargeback preview scope, data-flow event cost pre-model, unsupported role denial 검증.
