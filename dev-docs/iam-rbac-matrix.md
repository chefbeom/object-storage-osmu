...
| `ORG_ADMIN` | 조직 관리자 | 자기 조직 사용자/조직 usage/chargeback preview/daily trend/export/alerts/notification preview/outbox/draft invoice export, pricing policy 조회, 자기 조직 bucket 관리, 일반 사용자 생성/비활성화 |
...
| Chargeback preview/trend | `GET /api/admin/billing/chargeback-preview`, `GET /api/admin/billing/chargeback-daily-rollup`, `GET /api/admin/billing/chargeback-daily-rollup/export.csv` | Allow | Allow | Deny | Deny | `ORG_ADMIN`은 자기 조직 chargeback preview와 data-flow daily rollup 기반 일별 trend/export만 조회 | `AdminRbacPolicy`, `AdminBillingController`, `ChargebackPreviewService` |
...
| Chargeback final invoice/payment workflow | `POST /api/admin/billing/chargeback-invoice-drafts/{invoiceId}/finalize`, `GET /api/admin/billing/chargeback-invoices`, `POST /api/admin/billing/chargeback-invoices/{invoiceId}/payment-request`, `GET/POST /api/admin/billing/chargeback-invoices/{invoiceId}/payment-provider-handoff`, `GET /api/admin/billing/chargeback-payment-provider-handoffs`, `GET /api/admin/billing/payment-provider-adapter-readiness`, `POST /api/admin/billing/chargeback-payment-provider-handoffs/{handoffId}/adapter-result`, `POST /api/admin/billing/chargeback-payment-provider-handoffs/{handoffId}/adapter-send`, `POST /api/admin/billing/chargeback-invoices/{invoiceId}/payment-record` | Allow | Deny | Deny | Deny | ADMIN-only final invoice, payment state workflow, payment provider handoff outbox, configured CARD/BANK/TAX/ERP native bridge before provider-specific/generic webhook fallback, payment provider adapter readiness, and adapter result/retry state. It stores product-side legal billing state and manual payment reference without storing provider secrets/raw responses | `AdminRbacPolicy`, `AdminBillingController`, `ChargebackPreviewService` |
...
| Data-flow monitoring | `GET /api/admin/monitoring/data-flow`, `GET /api/admin/monitoring/data-flow/daily-rollup`, `POST /api/admin/monitoring/data-flow/daily-rollup/materialize`, `GET /api/admin/monitoring/data-flow/daily-rollup/materialized`, `GET /api/admin/monitoring/data-flow/monthly-rollup`, `POST /api/admin/monitoring/data-flow/monthly-rollup/materialize`, `GET /api/admin/monitoring/data-flow/monthly-rollup/materialized`, `GET /api/admin/monitoring/data-flow/export.csv`, `GET /api/admin/monitoring/data-flow/daily-rollup/export.csv`, `GET /api/admin/monitoring/data-flow/daily-rollup/materialized/export.csv`, `GET /api/admin/monitoring/data-flow/monthly-rollup/export.csv`, `GET /api/admin/monitoring/data-flow/monthly-rollup/materialized/export.csv`, `GET /api/admin/monitoring/data-flow/storage-status`, `GET /api/admin/monitoring/data-flow/retention/status`, `POST /api/admin/monitoring/data-flow/retention/run` | Allow | Deny | Deny | Deny | ADMIN-only object data-flow detail, daily rollup analytics, materialized daily rollup refresh/read, monthly aggregate analytics, materialized monthly rollup refresh/read, detailed CSV export, daily rollup CSV export, materialized daily/monthly rollup CSV export, repository row-count/readiness status, and event/daily-rollup/monthly-rollup retention status/manual run. The rollups aggregate day/month bucket/source/operation rows for operations and chargeback planning without object keys or raw event messages | `AdminRbacPolicy`, `AdminController`, `AdminDataFlowRetentionController`, `DataFlowMonitoringService` |
| Usage/status | `GET /api/admin/usage`, `GET /api/admin/system/status`, `GET /api/admin/storage/backend-status`, `GET /api/admin/security/enterprise-auth-plan`, `GET /api/admin/dashboard/*` | Allow | Deny | Allow | Deny | 전역 운영/보안 plan, object storage backend readiness, metadata usage 상태 read-only | `AdminRbacPolicy`, `AdminEnterpriseAuthPlanController`, `AdminController` |
...
- Chargeback adapter retry worker: `GET /api/admin/billing/chargeback-adapter-retry-worker/status`, `POST /api/admin/billing/chargeback-adapter-retry-worker/run`, and `GET /api/admin/billing/payment-provider-adapter-readiness` are `ADMIN` only. `ORG_ADMIN`, `AUDITOR`, and `USER` are denied. `ChargebackAdapterRetryWorkerService` performs dry-run state inspection and attempts configured generic notification webhook, `SLACK` notification webhook, `EMAIL` SMTP relay, and provider-matched CARD/BANK/TAX/ERP native bridge delivery before provider-specific or generic payment webhook fallback on run without storing secrets, signing secrets, webhook URLs, native auth header values, SMTP passwords, or raw provider responses. The payment provider adapter readiness endpoint is read-only and reports generic/CARD/BANK/TAX/ERP webhook readiness plus native bridge supported, configuration-required, and ready states without external calls.
