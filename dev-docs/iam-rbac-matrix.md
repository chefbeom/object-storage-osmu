...
| Chargeback invoice draft persistence | `GET/POST /api/admin/billing/chargeback-invoice-drafts`, `POST /api/admin/billing/chargeback-invoice-drafts/{invoiceId}/approve` | Allow | Deny | Deny | Deny | ADMIN-only internal invoice draft persistence/approval. `APPROVED_INTERNAL` still is not a legal final invoice or payment request | `AdminRbacPolicy`, `AdminBillingController`, `ChargebackPreviewService` |
