...
| Data-flow monitoring | `GET /api/admin/monitoring/data-flow`, `GET /api/admin/monitoring/data-flow/daily-rollup`, `GET /api/admin/monitoring/data-flow/export.csv` | Allow | Deny | Deny | Deny | ADMIN-only object data-flow detail, daily rollup analytics, and CSV export. The rollup aggregates day/bucket/source/operation rows for operations and chargeback planning | `AdminRbacPolicy`, `AdminController`, `DataFlowMonitoringService` |
