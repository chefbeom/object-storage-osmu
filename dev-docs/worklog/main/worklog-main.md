...
- Request analysis:
  ...
  - S3 object, CopyObject, and multipart overwrite guards mapped to `PreconditionFailed`, but XML `Message` still came from internal guard text.
- Execution:
  - Extended `S3ErrorCodeMapper.messageFor` to normalize `PreconditionFailed` to AWS-style precondition text.
  - Added mapper assertion and object overwrite guard regression assertion.
...
- Modified files:
  ...
  - `dev-docs/worklog/main/worklog-main.md`
- Tests:
  - `./gradlew.bat test --no-daemon --tests com.example.osmu.common.error.S3ErrorCodeMapperTest --tests com.example.osmu.object.S3ObjectControllerTest.accessKeyCanPutHeadGetAndDeleteObjectThroughS3StylePath`: passed.
  - `git diff --check`: passed.
- Review:
  - S3 precondition failures now keep HTTP `412` while using the normalized AWS-style message across shared XML error rendering.

### 2026-06-18 - S3 not-found message parity
...
- Request analysis:
  - `dev-docs` still tracks full AWS error behavior parity as future work.
  - After `AccessDenied` and `InvalidRange`, S3 not-found errors still used internal messages like `Object not found.` and `Upload session not found.`.
- Execution:
  - Extended `S3ErrorCodeMapper.messageFor` to normalize `NoSuchBucket`, `NoSuchKey`, and `NoSuchUpload` messages.
  - Added mapper assertions and S3 object/multipart regression assertions for `NoSuchKey` and `NoSuchUpload`.
  - Updated API/backend/S3 compatibility docs, product requirements, and test cases.
- Modified files:
  - `osmu-backend/src/main/java/com/example/osmu/common/error/S3ErrorCodeMapper.java`
  - `osmu-backend/src/test/java/com/example/osmu/common/error/S3ErrorCodeMapperTest.java`
  ...
  - `dev-docs/worklog/main/worklog-main.md`
- Tests:
  - `./gradlew.bat test --no-daemon --tests com.example.osmu.common.error.S3ErrorCodeMapperTest --tests com.example.osmu.object.S3ObjectControllerTest.accessKeyCanPutHeadGetAndDeleteObjectThroughS3StylePath --tests com.example.osmu.object.S3ObjectControllerTest.missingS3MultipartUploadReturnsNoSuchUploadXml`: passed.
  - `git diff --check`: passed.
- Review:
  - S3 not-found XML now returns AWS-style generic messages while preserving derived `BucketName`, `Key`, and `UploadId` details.
...
- User command:
  - Active goal: continue development from `dev-docs`.
- Request analysis:
  - S3 XML errors exposed AWS trace headers, but successful S3 responses still relied only on `X-Request-Id`.
  - AWS SDK/client diagnostics expect `x-amz-request-id` and `x-amz-id-2` on normal S3 operations too.
- Execution:
  - Added `S3TraceHeaders` helper for S3 request detection, resource rendering, and opaque host id generation.
  - `RequestIdFilter` now adds `x-amz-request-id` and `x-amz-id-2` to S3 responses.
  - `GlobalExceptionHandler` reuses the helper so XML `HostId` and header `x-amz-id-2` stay aligned.
  - Backend CORS now exposes the S3 trace headers with `X-Request-Id`.
  - Added success-response trace header assertions to `S3ObjectControllerTest.accessKeyCanPutHeadGetAndDeleteObjectThroughS3StylePath`.
  - Updated API/backend/S3 compatibility docs, product requirements, feature inventory, and test cases.
- Modified files:
  - `osmu-backend/src/main/java/com/example/osmu/common/web/S3TraceHeaders.java`
...
- Tests:
  ...
  - `git diff --check`: passed.
- Review:
  - Successful and error S3 responses now share the same trace id/header generation path.
  - Browser clients can read the S3 trace headers through backend CORS.
...
### 2026-06-19 - S3 CompleteMultipartUpload XML root 검증
...
- 요청 분석:
  - AWS S3 CompleteMultipartUpload 요청 body는 `CompleteMultipartUpload` 루트를 사용한다.
  - 기존 OSMU XML parser는 루트 이름을 확인하지 않고 내부 `Part` 노드를 검색했기 때문에, 다른 XML 문서에 `Part`가 들어 있으면 잘못된 complete XML을 통과시킬 수 있었다.
- 실행:
  - `S3ObjectController.completedPartsFromXml`에서 root local name이 `CompleteMultipartUpload`인지 먼저 검증하도록 수정했다.
  - 잘못된 루트는 기존 S3 error mapper 경로를 통해 `MalformedXML`과 AWS-style XML message로 응답한다.
  - MockMvc 테스트 `completeMultipartUploadRejectsUnexpectedRootXml`를 추가했다.
  - `api-spec.md`, `s3-compatibility.md`, `test-cases.md`에 complete XML root validation을 반영했다.
- 수정한 파일:
  - `osmu-backend/src/main/java/com/example/osmu/object/S3ObjectController.java`
  - `osmu-backend/src/test/java/com/example/osmu/object/S3ObjectControllerTest.java`
  - `dev-docs/api-spec.md`
