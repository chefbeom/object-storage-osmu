...
- Steps: In plan-only mode, write the planned durable finalization commands without replacing latest ready finalizer evidence. In full mode, run durable preflight, backend Gradle tests, durable demo gate, durable release artifact generation, and hard MVP readiness verification with durable pending gates treated as failure.
- Expected: Default plan-only mode writes `.osmu-run/latest-durable-mvp-finalize-plan.json` and `.osmu-run/latest-durable-mvp-finalize-plan.md` while preserving `.osmu-run/latest-durable-mvp-finalize.*` ready evidence. Explicit plan-only `-ReportPath`/`-SummaryPath` writes to the requested paths and records default-path flags as false. Full mode exits successfully only after preflight, backend tests, Docker/MariaDB/MinIO/Browser/S3 durable gate, synchronized release artifacts, and readiness hard gate all pass.
- Priority: P0
- Automated: `scripts/finalize-durable-mvp-demo.ps1`, `scripts/verify-durable-mvp-finalize-plan.ps1`

### TC-DEMO-008

- Feature: MVP completion report.
- Preconditions: Durable MVP evidence artifacts exist under `.osmu-run`, including latest demo readiness, durable demo gate, durable finalizer, durable preflight, release report, audit, decision, release notes, and demo package notes.
- Input: `powershell -ExecutionPolicy Bypass -File .\scripts\verify-mvp-completion.ps1`.
- Steps: Read the latest durable MVP evidence, durable preflight handoff, release artifacts, operations readiness, and status documents. Verify they agree on the current local durable MVP state, preserve the production/B2B readiness split, and reserve `-FailIfLocalMvpNotReady` for the post-Docker-ready finalizer hard gate.
- Expected: Before Docker-ready finalizer refresh, plan-only durable finalizer keeps completion pending and the script writes `.osmu-run/latest-mvp-completion.json`/`.md` with `classification=local-durable-mvp-pending`, `localDurableMvpReady=false`, `durablePreflightNextActionCommand`, and `durablePreflightBlockingActions`. After the Docker-ready finalizer refresh writes ready evidence, the same verifier can report `classification=local-durable-mvp-ready`, `localDurableMvpReady=true`, zero local pending checks, and production operations readiness tracked separately.
- Priority: P0
...
- Expected: The script exits successfully only when the status document says `docker-durable-demo-verified`, `MVP demo estimate: 90-95%`, `Production/B2B readiness: pending target evidence`, `S3 compatibility role: replacement layer, not AWS edge parity`, the 2026-06-30 snapshot with MVP completion `local-durable-mvp-pending`, operations readiness `passed=82, pending=20, total=102`, operations evidence handoff browser dispatch checklist, S3 boundary verifier pass, and monitoring threshold target evidence in the production operations chain, while rejecting stale 2026-06-18/2026-06-21 stale snapshot wording and old SSO/LDAP pending wording. If latest evidence JSON is present, stale snapshot numbers fail the verifier.
- Priority: P1
...
- Steps: Generate a fixture operations readiness convergence report, run the sync helper in plan-only mode, run it with fake `kubectl` in server dry-run mode, and run it with fake `kubectl` in apply mode. Verify the evidence report records the ConfigMap name/key, sync evidence ConfigMap key, source report format, source byte count, SHA256, optional data-flow storage plan key/hash, optional data-flow transition runbook key/hash, server dry-run command, apply command, publish-evidence apply command, safety policy, and zero failed checks. Verify raw SQL/raw EXPLAIN/credential-shaped data-flow plan summaries and raw SQL/raw EXPLAIN/object key/raw event message/credential-shaped runbook snapshots are rejected before ConfigMap publish. In apply mode, verify the helper renders and applies a second ConfigMap manifest that includes `latest-kubernetes-operations-report-sync.json`.
...
- Steps: Verify the ConfigMap contains `latest-operations-readiness-convergence.json` and `latest-kubernetes-operations-report-sync.json`, plus `latest-data-flow-storage-plan.json` and `latest-data-flow-storage-transition-runbook-evidence.json` when local evidence exists. Parse the JSON documents, compare expected result/configMap/runbook metadata with local evidence when present, select a ready backend Pod by `app.kubernetes.io/name=osmu-backend`, and read the mounted files through `kubectl exec cat`.
...
- Steps: Verify the workflow is manual-only, runs on Ubuntu with `pwsh`, writes the operations readiness convergence report, restores and validates optional `data_flow_storage_plan_json_base64` and `data_flow_storage_transition_runbook_json_base64` inputs, runs `sync-kubernetes-operations-reports.ps1 -PlanOnly` by default with runbook path wiring, prepares kubeconfig only when `run_live=true`, runs `-ServerDryRunOnly` before live apply, requires `run_live=true` when `apply=true`, and uploads convergence/sync/data-flow plan/runbook evidence artifacts.
...
- Steps: Verify `dev-docs/development-roadmap.md` keeps the B2B operations evidence priority, current durable MVP baseline, 2026-06-30 snapshot, Kubernetes DR finalizer `result=ready` contract, security/IAM finalizer `result=passed` contracts, data-flow storage transition plan, enterprise auth target smoke or scope-out path, operations evidence handoff browser dispatch checklist, convergence bottleneck/Kubernetes sync stale snapshot, and the rule that S3 work is limited to replacement-use smoke failures, migration blockers, or OSMU control-plane needs. When latest `.osmu-run` MVP completion, operations readiness, evidence handoff, or readiness convergence JSON exists, compare the roadmap snapshot lines against the JSON result/classification/count fields.
...
- Expected: The report remains honest about missing live/CI/storage/data-flow/monitoring evidence, but every remaining production/B2B operations gap points to an executable script command and, where applicable, a `gh workflow run` template so operators know the next evidence run to perform. Monitoring threshold target evidence is a top-level readiness gate and points to `write-monitoring-threshold-evidence.ps1` plus `manual-monitoring-threshold-evidence.yml`. The operations readiness finalizer preserves the selected `DataFlowStoragePlanPath` and `DataFlowStorageTransitionRunbookEvidencePath` when regenerating the final readiness report, and the operations readiness finalizer plus artifact finalizer CI workflows can restore data-flow storage plan/runbook snapshots from base64 only through the same unsafe raw SQL/raw EXPLAIN/object key/raw event message/credential-shaped rejection path. Storage expansion finalizer artifacts are promoted only when the finalizer has typed zero failures, `impersonateRunner=true`, RBAC/server-dry-run evidence paths, passed RBAC and server dry-run steps, required paired RBAC/server-dry-run JSON artifacts, and no skipped required evidence or credential-shaped content. Kubernetes HA/DR readiness artifacts are promoted only when every deployment/statefulset/PDB/PVC/CronJob/restore dry-run check is typed passing with exit code 0, and Kubernetes DR finalizer artifacts are promoted only when confirmed restore, non-server-dry-run execution, concrete backup timestamp, DR bucket bootstrap/immutability/transfer, restore smoke, evidence request/submit, S3 smoke, and gapless finalizer steps are present. IAM/RBAC finalizer artifacts are promoted only when the finalizer has an allowed passed status, typed zero failures, backend/live flags matching that status, required IAM/RBAC and Kubernetes RBAC verifier commands and passed steps, and no credential-shaped content. Storage backend telemetry artifacts are promoted only when `result=passed`, target environment/cluster/operator metadata is present, source mode and admin-info SHA-256 are valid, all required target check rows are typed passing, typed pool/server/drive/capacity/failure counts prove online capacity, `rawAdminInfoStored=false` is a typed boolean, and raw admin-info or credential-shaped content is absent. Secret rotation artifacts are promoted only when `result=passed`, target environment/cluster/operator metadata is present, the rotation window is chronological, required evidence refs exist, required core rotations and check rows are typed passing, required confirmations are true, typed failure/planned counts are zero, and credential-shaped content is absent. Security finalizer artifacts are promoted only when the finalizer format is correct, typed zero failures are present, `allowSyntheticEvidence=false` is typed, promotion actions/source URLs/image refs/digests exist, all finalizer checks are typed passing, and no synthetic/self-test or credential-shaped content is present. Image signing artifacts are promoted only when non-synthetic passed evidence includes typed zero failures, source run URL, nonzero commit SHA, backend/frontend refs, nonzero digests, and typed signature verification booleans; container security artifacts require typed zero failures, source run URL, nonzero commit SHA, image names, typed scan booleans, and backend/frontend SPDX SBOM package/hash/byte metadata. Kubernetes operations report sync artifacts are promoted only when `result=applied`, typed `failedCount=0`, ready convergence source metadata, valid source SHA-256, and no kubeconfig or credential-shaped content are present. Direct data-flow target plan artifacts are promoted only when `result=passed`, typed `pendingCount=0`, candidate store is valid, and MariaDB partition/dual-write query-plan summaries have `result=passed` with typed `failedCount=0`; planned or string-count evidence cannot satisfy the target import path. Data-flow transition runbook artifacts are promoted only when `result=passed`, target metadata, chronological review window, evidence refs, the storage-plan snapshot is passed with a valid candidate store, typed `pendingCount=0`, positive plan/check counts, required query-plan summary result for MariaDB or dual-write candidates, typed `failureCount=0`, all rehearsal/no-object/no-secret confirmations are typed boolean true values, and complete PASS check rows are present. Monitoring threshold artifacts are promoted only when `result=passed`, target metadata, chronological review window, evidence refs, required alert mappings, typed alert/route/Grafana/tuning/failure/check counts, complete PASS check rows, typed confirmations, and no credential-shaped receiver/secret content are present. Commercial integration and commercial approval artifacts are promoted only when their typed summary counts, target review confirmations, no-secret/no-raw-content gates, and required snapshot summaries prove the target evidence rather than just claiming `result=passed`. Enterprise auth `scope-out` artifacts are promoted only when `accepted` is typed boolean true with a non-secret reference and reason. Operations handoff package artifacts are promoted only when `result=passed`, every required no-secret/runbook/troubleshooting/rollback/support/known-gap/readiness/convergence/data-flow/secret-rotation/commercial/enterprise-auth/monitoring/production/snapshot confirmation is true, evidence refs are non-empty, typed `targetEvidenceSnapshots` prove data-flow storage plan/runbook, secret rotation, commercial integration/approval, enterprise auth passed or scope-out, and monitoring threshold readiness, summary counts match complete PASS check rows, and nested readiness/convergence snapshots prove readiness, typed integer finalizer failed/gap counts at zero, typed boolean Kubernetes report sync readiness, failed sync count zero, and Kubernetes report sync `sourceReportResult=ready`; the final readiness gate applies the same direct handoff package validation, so stale pre-review, stale convergence, or weak target-snapshot packages cannot satisfy the final import or readiness chain.
- Artifact import detail: Kubernetes DR finalizer artifacts are promoted only when generated/start/complete timestamps are chronological, each required finalizer command row matches its required script and flag, each required finalizer step row matches its required script and flag, and every finalizer step row is `passed` with typed integer `exitCode=0`; failed extra steps cannot be hidden behind the required step rows.
- Artifact import detail: Image signing artifacts are promoted only when GitHub Actions OIDC keyless signing metadata is present, backend/frontend version refs are tagged with the release version, backend/frontend SHA refs are tagged with the exact commit SHA, digests are nonzero `sha256:` values, and version/SHA signature verification flags are typed boolean true. Container security artifacts are promoted only when backend/frontend images are tagged with the exact commit SHA, the artifact name carries that commit, Trivy high/critical scan scope and `ignoreUnfixed=true` are typed, scan pass flags are typed boolean true, and backend/frontend SPDX SBOM metadata includes the expected format, label, version, positive package/byte counts, and nonzero SHA-256 hashes.
- Remediation detail: Security evidence remediation must point image-signing operators to `publish=true`, a release version input, GitHub OIDC keyless Cosign verification, and commit-SHA tag verification, while container security remediation must point operators to Trivy `CRITICAL,HIGH`, commit-SHA image tag capture, typed ignore-unfixed policy, and backend/frontend SPDX SBOM metadata.
- Artifact import detail: Commercial integration artifacts are promoted only when `result=passed`, target environment/cluster/operator metadata is present, the verification window is chronological, required evidence refs exist, all eight implemented integration rows and target check rows are typed passing, payment-provider adapter readiness snapshot metadata is typed and reviewed, no-secret/no-raw-provider confirmations are true, typed failure/planned counts are zero, and raw provider/customer/payment or credential-shaped content is absent.
- Artifact import detail: Commercial approval artifacts are promoted only when `result=passed`, product version/approver/approval timestamp metadata is present, required approval evidence refs exist, all target check rows are typed passing, pricing policy proposal approval snapshot metadata is typed and reviewed, at least one approved price-list proposal row exists, approval confirmations are true, typed failure counts are zero, and raw contract/customer/license/payment/price-table or credential-shaped content is absent.
- Artifact import detail: Enterprise auth artifacts are promoted only when format/generated timestamp/execution mode metadata is present, passed target smoke evidence uses `executionMode=execute` with selected OIDC or LDAP target checks and typed zero fail/block/planned counts, scope-out evidence uses `executionMode=scope-out` with typed confirmed/accepted approval plus all scope-out check rows passing, and raw passwords, tokens, OIDC claims/codes/states, or credential-shaped content are absent.
- Priority: P1
...
- Steps: Verify backend readiness items with `code=OPERATIONS_READINESS_CHECK` include `evidencePath`, `remediationCommand`, `remediationWorkflow`, `remediationWorkflowCommand`, and `remediationNote` when the source report provides them. Verify `OPERATIONS_EVIDENCE_PLAN` appears when `.osmu-run/latest-operations-evidence-plan.json` exists, including plan path and regeneration command, and verify `operationsEvidencePlan.actions` carries ordered action detail, recommended command, operator inputs, approval flag, and kubeconfig flag. Verify `OPERATIONS_EVIDENCE_PLAN_INVOCATION` appears when `.osmu-run/latest-operations-evidence-plan-invocation.json` exists, including invocation path, regeneration command, planned/blocked counts, action command, block reasons, unresolved/invalid placeholders, and status. Verify `OPERATIONS_INVOCATION_UNBLOCK_PLAN` appears when `.osmu-run/latest-operations-invocation-unblock-plan.json` exists, including required confirmation flags, unresolved/invalid placeholder input mapping, repeated-placeholder warning counts, blocked/planned action order lists, confirmed plan command, blocked-only command, and per-action plan command detail. Verify `OPERATIONS_DISPATCH_PREFLIGHT` appears when `.osmu-run/latest-operations-dispatch-preflight.json` exists, including failed check count, missing/unsafe/invalid input counts, required GitHub secret names, workflow file detail, preflight checks, required input safe/valid flags, ready plan command, and execute command when ready. Verify `OPERATIONS_WORKFLOW_RUN_ID_PLAN` appears when `.osmu-run/latest-operations-workflow-run-ids.json` exists, including top-level `selectedActionOrders`, browser workflow runs URLs, workflow run-id inputs, GitHub REST API run-id collection command/query URLs, workflow count, missing run count, artifact collection follow-up command, security evidence finalizer command, optional MinIO bucket CORS run-id handoff, Kubernetes operations report sync run-id handoff when present, and workflow query command detail. Verify `OPERATIONS_ARTIFACT_COLLECTION_PLAN` appears when `.osmu-run/latest-operations-artifact-collection-plan.json` exists, including artifact count, missing required count, `securityEvidenceFinalizerInputs[]`, finalizer command, local import command, artifact download command detail, storage backend telemetry artifact detail, optional MinIO bucket CORS artifact detail with `requiredForReadiness=false`, monitoring threshold artifact detail, secret rotation artifact detail, commercial integration artifact detail, commercial approval artifact detail, operations handoff package artifact detail, and optional Kubernetes operations report sync artifact detail. Verify `OPERATIONS_READINESS_ARTIFACT_IMPORT` appears when `.osmu-run/latest-operations-readiness-artifact-import.json` exists, including import status, imported/failed counts, no-secret import policy, entry file names, source paths, destination paths, storage backend telemetry, MinIO bucket CORS dashboard import entries when selected, monitoring threshold, secret rotation, commercial integration, commercial approval, operations handoff package, and Kubernetes operations report sync import entries when selected, and failure details. Verify `OPERATIONS_READINESS_FINALIZER` appears when `.osmu-run/latest-operations-readiness-finalize.json` exists, including result, readiness result, failed count, selected steps, command list, step results, gaps, and secret masking policy. Verify `OPERATIONS_EVIDENCE_HANDOFF` appears when `.osmu-run/latest-operations-evidence-handoff.json` exists, including structured current bottleneck, next command, dispatch preflight/run-id/artifact selected action orders, input-free blocked review report exists/result/generated/action orders/counts/stale/scope fields, browser-dispatch `browserDispatchChecklist[]`, GitHub REST API workflow-run-id stage command when available, post-dispatch saved-run-list/GitHub REST API/GitHub CLI run-id and artifact collection commands, stage count, blocked action count, missing run count, missing artifact count, finalizer failed count, finalizer gap count, and stage summary detail. Verify `operationsHandoffPackage.dataFlowStoragePlanSnapshot`, `operationsHandoffPackage.dataFlowStorageTransitionRunbookSnapshot`, `operationsHandoffPackage.secretRotationSnapshot`, `operationsHandoffPackage.monitoringThresholdSnapshot`, `operationsHandoffPackage.commercialIntegrationSnapshot`, `operationsHandoffPackage.commercialApprovalSnapshot`, and `operationsHandoffPackage.enterpriseAuthSmokeSnapshot` appear when `.osmu-run/latest-operations-handoff-package.json` contains reduced target evidence. Verify `storageBackendTelemetryEvidence` appears when `.osmu-run/latest-storage-backend-telemetry.json` exists, including result, target environment/cluster/operator, source mode, non-secret evidence reference, admin-info SHA-256, pool/server/drive counts, offline count, capacity totals, and no raw admin output. Verify `monitoringThresholdEvidence` appears when `.osmu-run/latest-monitoring-threshold-evidence.json` exists, including result, target labels, review window, alert/route/Grafana/tuning counts, mapping coverage booleans, external evidence refs, confirmation flags, failed/check counts, check rows, and no-secret policy. Verify `OPERATIONS_READINESS_CONVERGENCE` appears when `.osmu-run/latest-operations-readiness-convergence.json` exists, including ready/action-required result, current bottleneck dispatch URLs, readiness summary/count fields, recommended commands, handoff post-dispatch saved-run-list/GitHub REST API/GitHub CLI commands, handoff stale/timestamp fields, mirrored handoff input-free blocked review report fields, mirrored handoff required GitHub secret count/list/summaries, finalizer status, stage counts, Kubernetes report sync ready/result/source report result/failed count/ConfigMap target, runbook workflow input guidance, and no-execute safety policy. Verify `KUBERNETES_OPERATIONS_REPORT_SYNC` appears when `.osmu-run/latest-kubernetes-operations-report-sync.json` exists, including namespace, ConfigMap name/key, source report result/hash, data-flow plan/runbook ConfigMap keys, runbook result/failure summary, check results, failed count, server dry-run command, apply command, workflow path, and safety policy. Verify the frontend readiness panel renders a compact remediation block with local command, workflow path, workflow command, evidence path, note, copy controls, a plan summary line, an invocation summary line, an invocation unblock summary line, a dispatch preflight summary line with unsafe/invalid counts, a workflow run id summary line, an artifact collection summary line with security finalizer input rows, an artifact import summary line, a finalizer summary line, a handoff summary line, handoff package data-flow plan/runbook/secret rotation/monitoring threshold/commercial/chargeback/enterprise-auth/cluster-network/Helm snapshot summary lines, a storage backend telemetry summary line, a monitoring threshold evidence summary line, a convergence summary line with readiness counts, handoff freshness, required GitHub secrets, operator input values non-ready action counts, bottleneck browser dispatch links, and Kubernetes sync source readiness, a Kubernetes report sync summary line, a Kubernetes sync runbook summary line, a plan action list, an invocation action list with invalid placeholder detail, unblock action input rows, dispatch preflight check/input/workflow rows with safe/valid status, workflow query command rows, artifact download command rows, artifact import entry rows, finalizer command rows, finalizer step rows, handoff stage rows, monitoring threshold check rows, convergence command rows, and Kubernetes report sync check rows under the related operations readiness warning.
- Steps extension: Verify `DATA_FLOW_STORAGE_TRANSITION_RUNBOOK` appears when `.osmu-run/latest-data-flow-storage-transition-runbook-evidence.json` exists, and verify `dataFlowStorageTransitionRunbook` exposes result, target labels, storage-plan result, candidate store, target p95 latency, failure/check counts, confirmation flags, top failed checks, and no-raw-SQL/no-secret scope policy. Verify the frontend renders the data-flow transition runbook summary, confirmation summary, and failed-check rows.
...
- Expected: Operators can see and copy the next evidence collection script command, GitHub Actions dispatch command, invocation unblock command, dispatch preflight command, workflow run query command, GitHub REST API run-id query command, artifact download command, artifact finalizer command, local import command, operations finalizer command, handoff-selected next command, convergence-selected recommended command, or Kubernetes report sync server dry-run/apply command from the dashboard without opening CLI-generated Markdown files, and they can see the ordered evidence plan, guarded planned/blocked invocation action list, blocker-unblock input handoff, dispatch preflight no-execute gate, workflow run id handoff, artifact collection handoff, artifact import result handoff, operations finalizer handoff, storage backend telemetry handoff, monitoring threshold handoff, current bottleneck handoff, selected-action scope mismatch handoff, handoff/convergence run-id query executed/success/error/candidate summaries, handoff/convergence input-free blocked review summaries, final operations convergence handoff, convergence-level deployed-report sync gate, and deployed-report sync handoff before live Kubernetes/security workflow execution while non-operations readiness items remain unchanged.
- Priority: P1
- Automated: `AdminDashboardSummaryControllerTest.dashboardReadinessIncludesOperationsEvidenceReportWarnings`, `npm run test:unit`, `npm run mock:api:self-test` verifies the mock dashboard operations chain keeps selected action 6, `operationsArtifactCollectionPlan.securityEvidenceFinalizerInputs[]`, `operationsEvidenceHandoff.browserDispatchChecklist[]`, `operationsReadinessConvergence.currentBottleneck.dispatchUrls[]`, `readiness-evidence-handoff-input-free-review`, `readiness-evidence-handoff-operator-input-values`, `readiness-convergence-input-free-review`, `readiness-convergence-operator-input-values`, and `dispatch-ready-subset-browser`; `scripts/verify-browser-e2e-mock-demo.ps1`, `scripts/verify-browser-e2e-local-demo.ps1`, and `scripts/verify-browser-e2e-prototype.ps1` cover the dashboard convergence fixture/command rows with the same action 6 browser-dispatch handoff, including the `container-security-ci.yml` bottleneck dispatch link, GitHub REST API run-id copy command, and post-dispatch run-id copy commands.
...
- Steps: Generate a fixture operations readiness report with passed, remediable pending, unplanned pending, Kubernetes live, and enterprise auth live workflow checks. Run `scripts/write-operations-evidence-plan.ps1`, then verify the JSON and Markdown plan include ordered remediation actions, local commands, workflow paths, `gh workflow run` commands, placeholder inputs, operator approval flags for `run_live=true` workflow dispatches, kubeconfig-secret requirements for Kubernetes-scoped live actions, and an unplanned-check section.
- Expected: Operators get a deterministic evidence execution plan from readiness gaps before touching a live cluster or GitHub Actions, live workflow dispatches require explicit operator approval, the plan does not hide pending checks that lack remediation metadata, and non-Kubernetes workflows such as enterprise auth smoke are not blocked on kubeconfig just because their workflow uses `run_live=true`.
- Priority: P1
...
- Steps: Generate a fixture invocation report containing storage expansion, Kubernetes HA/DR readiness, Kubernetes DR finalizer, image signing, container security, security evidence finalizer, manual storage backend telemetry evidence, optional MinIO bucket CORS verification, manual data-flow storage transition runbook evidence, manual secret rotation evidence, manual commercial integration evidence, manual commercial approval evidence, enterprise auth smoke, manual operations handoff package evidence, and Kubernetes operations report sync workflow commands. Run the helper in plan-only mode and verify it emits `gh run list --workflow ... --json ...` commands, GitHub REST API query URLs plus the `-UseGitHubApi` collection command, browser workflow runs URLs, workflow run-id input names, and saved run-list JSON path hints for every workflow and marks run ids as query-required. Rerun with fixture `gh run list` JSON for successful workflow runs and verify recommended run ids, commit SHA, Security Evidence Finalizer command, manual target evidence run ids, optional MinIO bucket CORS run id, Kubernetes operations report sync run id, and `write-operations-artifact-collection-plan.ps1` command are concrete.
- Expected: Operators can move from workflow invocation to run id discovery without manually reconstructing `gh run list` or GitHub REST API commands, and the artifact collection plan command is generated from latest successful workflow runs only after run evidence is available, including manual target evidence workflows, optional MinIO bucket CORS dashboard evidence, and the optional deployed-report sync workflow.
- Priority: P1
...
- Steps: Generate fixture report sets for missing handoff, action-required handoff, fully ready handoff/readiness/finalizer but missing Kubernetes report sync evidence, finalizer `ready` strings that still carry failed/gap counts, finalizer or Kubernetes report sync `failedCount` values supplied as strings or missing fields, applied Kubernetes report sync evidence whose `sourceReportResult` is not ready, and fully ready handoff/readiness/finalizer/sync states. Run the convergence writer for each set, then verify the JSON and Markdown report select the current bottleneck with dispatch URLs, ready/action-required result, readiness summary and passed/pending/total/check counts, recommended command chain with GitHub REST API workflow-run-id collection when available, handoff post-dispatch commands, stage counts, finalizer result, finalizer readiness result, finalizer failed/gap counts, Kubernetes report sync result/readiness/source report result, sync stale/timestamp/freshness reason, and no-execute safety policy.
- Expected: Operators can open one convergence report and see whether the operations workflow has reached ready or which single command chain remains, including the current browser dispatch bottleneck and final ConfigMap sync gate, without executing `kubectl`, `gh`, workflow dispatch, finalizer, or ConfigMap sync commands from the report writer itself. The convergence result is not ready unless the finalizer is ready with zero failed/gap counts and the applied ConfigMap sync evidence was generated from a ready source report.
- Priority: P1
