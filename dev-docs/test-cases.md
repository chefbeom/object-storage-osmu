...
- Adapter retry worker and adapter readiness: `GET /api/admin/billing/chargeback-adapter-retry-worker/status` returns `mode=ADAPTER_RETRY_WORKER`, `dryRun=true`, `externalAdaptersEnabled` reflecting notification/payment webhook configuration, candidate counts, and `updatedCount=0`; `POST /api/admin/billing/chargeback-adapter-retry-worker/run?dryRun=false` attempts configured generic notification webhook, `SLACK` notification webhook, `EMAIL` SMTP relay, and matching payment webhook profile rows, blocks unconfigured rows without external calls, and is `ADMIN` only. `GET /api/admin/billing/payment-provider-adapter-readiness` returns `mode=PAYMENT_PROVIDER_ADAPTER_READINESS`, generic/CARD/BANK/TAX/ERP rows, webhook-ready counts, `nativeApiSupported=false`, and no-secret policy without external calls. The Admin billing panel renders `chargeback-adapter-retry-worker-refresh-button`, `chargeback-adapter-retry-worker-run-button`, `chargeback-adapter-retry-worker-list`, `chargeback-payment-adapter-readiness-metrics`, and `chargeback-payment-adapter-readiness-list`.
- Priority: P1
...
- Steps: Generate synthetic passed target-environment commercial integration evidence, verify `osmu.commercial-integration-evidence.v1`, `result=passed`, notification webhook, Slack, EMAIL SMTP relay, generic payment webhook, CARD/BANK/TAX/ERP payment webhook profile handoff entries, adapter retry worker confirmation, payload cap confirmation, private/local endpoint blocking confirmation, HMAC signature confirmation, chronological verification window, no-secret policy, no-raw-provider-response policy, and native processor scope disclaimer. Verify credential-shaped evidence references such as `Bearer ...` and reversed `VerificationStartedAt`/`VerificationCompletedAt` windows are rejected.
...
- Input: `GET /api/admin/monitoring/data-flow?from=...&to=...&bucketName=...&actorId=...&source=...&operation=...&status=...&limit=50`, `GET /api/admin/monitoring/data-flow/daily-rollup?days=30&limit=200&...`, `POST /api/admin/monitoring/data-flow/daily-rollup/materialize?days=30&limit=200&...`, `GET /api/admin/monitoring/data-flow/daily-rollup/materialized?days=30&limit=200&...`, `GET /api/admin/monitoring/data-flow/retention/status`, `POST /api/admin/monitoring/data-flow/retention/run?includeEvents=true&includeDailyRollups=true`, `GET /api/admin/monitoring/data-flow/export.csv?...`, `GET /api/admin/monitoring/data-flow/daily-rollup/export.csv?...`, `GET /api/admin/monitoring/data-flow/daily-rollup/materialized/export.csv?...`, `GET /api/admin/dashboard/summary`, then render the admin dashboard.
- Steps: Upload or seed at least one object, list objects, download an object, copy an object, optionally abort a multipart upload or trigger a failed download. Verify the focused monitoring endpoint returns `traffic`, `operations`, `topBuckets`, `trendPoints`, `recentEvents`, and `generatedAt`, including `copiedBytes`, `internalBytes`, and `copyCount`, and that bucket/actor/source/operation/status/time filters narrow the result. Verify daily rollup returns `mode=DATA_FLOW_DAILY_ROLLUP`, `granularity=UTC_DAY`, day/bucket/source/operation points, success/failure/cancel counts, upload/download/copy bytes, and the no-AWS-billing note. Verify materialize returns `mode=DATA_FLOW_DAILY_ROLLUP_MATERIALIZATION`, `storedPointCount`, aggregate-only points, and `storagePolicy` referencing `data_flow_daily_rollups`. Verify materialized read returns `mode=DATA_FLOW_DAILY_ROLLUP_MATERIALIZED`, stored aggregate-only points, and actor/status scoped rows do not appear in an unscoped materialized read. Verify detailed CSV export returns `text/csv`, `osmu-data-flow.csv`, and the same filtered newest-first event window. Verify daily rollup CSV export returns `text/csv`, `osmu-data-flow-daily-rollup.csv`, rollup count/byte columns, and no object key or raw message columns. Verify materialized daily rollup CSV export returns `text/csv`, `osmu-data-flow-daily-rollup-materialized.csv`, stored aggregate count/byte columns, and no object key or raw message columns. Verify dashboard summary includes the same unfiltered `dataFlow` object. Verify the admin dashboard renders `dashboard-widget-io`, `data-flow-monitoring-panel`, `data-flow-filter-form`, `data-flow-export-button`, `data-flow-daily-rollup-export-button`, `data-flow-daily-rollup-materialize-button`, `data-flow-daily-rollup-materialized-load-button`, `data-flow-daily-rollup-materialized-export-button`, `data-flow-retention-panel`, `data-flow-retention-refresh-button`, `data-flow-retention-run-button`, `data-flow-total-bytes`, `data-flow-failed-cancelled`, `data-flow-trend-chart`, `data-flow-daily-rollup`, `data-flow-top-buckets`, and `data-flow-recent-events`.
- Expected: Operators can see, filter, aggregate, materialize, read materialized aggregates, inspect retention status, run selected retention targets, and export total upload/download/copy traffic, I/O operation counts, failed/cancelled transfer counts, source/operation trend, daily rollup, top bucket flow, recent data-flow events, and daily rollup CSV from both focused API and dashboard summary. CopyObject traffic is internal and stays separate from external ingress/egress. MariaDB mode persists the event history in `data_flow_events`, materializes daily aggregate rows into `data_flow_daily_rollups`, reads materialized rows without re-scanning detail rows, daily rollup aggregates the persisted rows by day/bucket/source/operation, scheduled retention deletes old event rows in bounded batches with audit/metrics, manual retention records `DATA_FLOW_RETENTION_RUN`, and scheduled materialized rollup retention deletes old `data_flow_daily_rollups` rows in bounded batches with `DATA_FLOW_DAILY_ROLLUP_RETENTION` audit and `osmu.data.flow.daily.rollup.retention.*` metrics.
- Priority: P1
...
- Steps: Generate a fixture invocation report containing storage expansion, Kubernetes HA/DR readiness, Kubernetes DR finalizer, image signing, container security, security evidence finalizer, commercial approval evidence, operations handoff package evidence, and Kubernetes operations report sync workflow commands. Run the collection plan helper without run ids and verify required readiness/convergence artifacts are marked missing while artifact names and download/import/finalizer commands use placeholders. Rerun with concrete run ids, image signing version, and commit SHA, then verify expected artifact names, `gh run download` commands, Security Evidence Finalizer command, Operations Readiness Artifact Finalizer command, commercial approval artifact inputs, operations handoff package artifact inputs, Kubernetes operations report sync artifact inputs, and local import command are concrete.
