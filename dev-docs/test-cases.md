...
- Input: `PUT /api/admin/billing/pricing-policy`, then `GET /api/admin/billing/chargeback-preview?storageGbMonthRate=...&ingressGbRate=...&egressGbRate=...&operationThousandRate=...`, `GET /api/admin/billing/chargeback-alerts?...`, and `GET /api/admin/billing/chargeback-preview/export.csv?...`
- Steps: Create two organizations, upload data to an ORG-owned bucket, optionally generate download/copy/failure events, save a pricing policy with warning/critical thresholds as `ADMIN`, call the chargeback preview, threshold alert API, and CSV export as `ADMIN` and `ORG_ADMIN`, then open Admin billing panel, save the policy, export CSV, and refresh the same preview/alert view.
- Expected: `ADMIN` can save pricing policy. `ORG_ADMIN` can read pricing policy for preview/alerts/export but cannot save it. `ADMIN` sees every organization. `ORG_ADMIN` sees only the caller organization in JSON alert rows and CSV. Preview uses saved policy values when query rates are omitted and query rates override saved values when present. The response includes current `usedBytes`, ingress/egress/internal bytes, billable successful operation count, failed/cancelled counts, per-organization cost components, and total estimated cost. Alert response includes warning/critical threshold amounts, severity, alert counts, and scoped organization rows. CSV export returns `text/csv`, `osmu-chargeback-preview.csv`, a `TOTAL` row, and scoped `ORGANIZATION` rows. User-owned or unknown buckets are excluded. The Admin billing panel renders `billing-chargeback-panel`, editable rate/date/event-limit/threshold inputs, `chargeback-save-policy-button` for ADMIN, `chargeback-alert-list`, `chargeback-export-button`, preview totals, and organization rows. The response, alerts, CSV, and UI are preview only and do not persist invoices.
- Priority: P1
