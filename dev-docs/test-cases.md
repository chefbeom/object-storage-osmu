...
- Expected: The report remains honest about missing live/CI/storage/data-flow evidence, but every remaining production/B2B operations gap points to an executable script command and, where applicable, a `gh workflow run` template so operators know the next evidence run to perform. The operations readiness finalizer preserves the selected `DataFlowStoragePlanPath` when regenerating the final readiness report.
- Priority: P1
