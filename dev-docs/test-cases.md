...
- Expected: The script exits successfully only when the status document says `docker-durable-demo-verified`, `MVP demo estimate: 90-95%`, `Production/B2B readiness: pending target evidence`, and `S3 compatibility role: replacement layer, not AWS edge parity`, while rejecting stale 2026-06-18 and old SSO/LDAP pending wording.
- Priority: P1
...
- Expected: Default output is `plan-ready-execute-required` and does not claim partition/time-series storage is enabled. Confirmed MariaDB partition/dual-write fixture can pass only when all sizing/confirmation switches are supplied and the linked `osmu.mariadb-query-plan-evidence.v1` summary is `result=passed` with zero failed checks. Failed query-plan evidence blocks `-FailIfNotPassed`, credential-shaped references such as `password=...` are rejected, and raw SQL/EXPLAIN payloads are not copied into the storage plan summary. Aggregate scope remains OSMU operations analytics, not AWS billing parity, and excludes object keys/raw event messages. `GET /api/admin/dashboard/readiness` exposes the latest plan as `dataFlowStoragePlan`, includes `dataFlowStoragePlan.queryPlanEvidence`, and adds `DATA_FLOW_STORAGE_PLAN` while pending checks remain.
- Priority: P1
