...
- Steps: Create a bucket through the S3-style path, create a bucket with matching `LocationConstraint`, reject a mismatched `LocationConstraint`, reject invalid CreateBucket XML root/duplicate `LocationConstraint`, reject unsupported CreateBucket ACL/grant/object-lock/object-ownership/bucket-namespace controls, reject AWS-invalid bucket names, create an access key with `ADMIN` scope for the bucket, verify deleting a non-empty bucket returns `BucketNotEmpty`, reject invalid bucket names on bucket delete, delete an empty bucket through the S3-style path, then HEAD the bucket again.
