...
- Steps: Put raw LifecycleConfiguration XML through query alias, get raw XML through query alias, delete through query alias, get lifecycle again, then submit missing, unexpected-root, missing/unsupported `Rule/Status`, invalid direct `Filter`, invalid lifecycle tag, over-limit lifecycle rule/ID, unsupported transition default header, valid and invalid checksum headers, unsupported object-size filter, unsupported action, and multiple-action lifecycle XML bodies.
- Expected: Alias uses the same bucket lifecycle rules. GET returns XML content type with exported rule children ordered as `ID`, `Filter`, `Status`, action while configuration exists. DELETE removes the bucket-scoped lifecycle config. GET after delete returns S3 XML `NoSuchLifecycleConfiguration` with HTTP `404`, message `The lifecycle configuration does not exist.`, and `BucketName`. Missing or blank lifecycle XML returns S3 XML `MissingRequestBodyError` with message `Request body is empty.`. Unexpected lifecycle XML roots, missing/unsupported `Rule/Status` values, and invalid direct `Filter` shapes return S3 XML `MalformedXML` and do not create a lifecycle configuration. Valid `Content-MD5` or one valid explicit `x-amz-checksum-*` header is checked before replacement. Invalid checksum syntax returns `InvalidDigest`; checksum mismatches or SDK/checksum algorithm mismatches return `BadDigest` and do not replace the configuration. Too many lifecycle rules, overlong `Rule/ID`, lifecycle tag restriction violations, unsupported transition default header, unsupported object-size lifecycle filters, unsupported lifecycle actions, and multiple target actions in one rule return S3 XML `InvalidRequest` and do not create a lifecycle configuration.
- Priority: P1
