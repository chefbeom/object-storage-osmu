...
- Preconditions: Node dependencies are installed.
- Input: `npm run test:unit`
- Steps: Run frontend unit tests that inspect `HomeView.vue` for stable `data-testid` selectors covering login, status, bucket list, object upload, object detail, bucket lifecycle, bucket tags, audit, and confirm modal flows.
- Expected: All required selectors exist so future Browser/Chrome E2E can target stable UI hooks instead of brittle text or layout selectors.
- Priority: P1
- Automated: `npm run test:unit`
...
- Preconditions: Backend API is running and reachable. AWS CLI (`aws`) or MinIO Client (`mc`) is installed on PATH. Admin login is available.
- Input: `.\scripts\verify-s3-client-smoke.ps1 -Client auto` or `.\scripts\verify-s3-client-smoke.ps1 -Client all -RequireClient`. Optional: `-SkipManualSigV4`, `-SkipVirtualHostedSmoke`.
- Steps: Script logs in through OSMU REST API, creates a smoke bucket and scoped access key, runs built-in SigV4 probes for root `HEAD`, root list, checksum object PUT/HEAD/GET, bucket sync plus another checksum `HEAD`, S3 multipart checksum complete, S3 multi-delete Quiet/Content-MD5 with mismatch guard, mismatched payload hash `BadDigest`, mismatched checksum `BadDigest`, and virtual-hosted-style object PUT/GET. Then it uses installed real S3 clients against `http://localhost:8080/api/s3` to list buckets, upload, head/stat, list objects, download/cat, delete object, and cleanup bucket.
- Expected: Built-in SigV4 probes pass without `X-OSMU-Secret-Key`; checksum PUT echoes `x-amz-checksum-sha256`; object HEAD/GET expose stored checksum; bucket sync preserves the checksum header; multipart checksum complete returns checksum header/XML and later HEAD exposes it; multi-delete Quiet suppresses `Deleted` entries; mismatched multi-delete `Content-MD5` returns `BadDigest` without deleting the protected object; mismatched payload hash and mismatched checksum return `BadDigest`. Installed clients complete all operations using S3 SigV4. If no real client is installed and `-RequireClient` is not set, the script skips external client checks with a clear warning.
- Priority: P1
...
- Steps: Create organization, delete it, list organizations.
- Expected: HTTP 204, organization disappears from list, `ORGANIZATION_DELETE` audit event is recorded.
- Priority: P1
...
- 기대 결과: 지정한 bucket별 scope와 사용자가 가진 범위 안의 `READ`, `WRITE`, `DELETE` permission이 저장/조회되고 S3 policy document가 생성된다. 사용자가 가진 권한을 초과한 permission은 `403 AUTHORIZATION_FAILED`로 차단된다.
- 우선순위: P1
- 자동화 여부: Automated
...
- 기대 결과: 생성 응답에 `targetType`, `targetId`, `quotaBytes`, `usedBytes`, `remainingBytes`가 포함되고, 삭제 후 목록에서 제거된다.
- 우선순위: P1
- 자동화 여부: Automated
...
- Steps: Verify `infra/k8s` contains namespace, configmap, secret example, MariaDB, MinIO, backend, frontend, ingress, kustomization, and README files. Check expected kinds, service/deployment/statefulset names, probes, storage images, secret references, ingress routing, and that `secret.example.yaml` is not included in `kustomization.yaml`.
- Expected: Manifest draft is complete enough for Kubernetes productization work to start, while real secrets remain outside the kustomize resource list.
- Priority: P2
...
- Steps: Verify `infra/helm/osmu` contains Chart.yaml, values.yaml, README, helper templates, configmap, guarded secret template, MariaDB, MinIO, backend, frontend, and ingress templates. Check expected chart metadata, image value indirection, secret creation guard, probes, service names, ingress routing, and required config keys.
- Expected: Helm chart draft is complete enough for customer/environment value separation work to start, while rendered Secret creation remains disabled by default.
- Priority: P2
