...
- Expected: Operators can see, filter, aggregate, materialize, read materialized aggregates, and export total upload/download/copy traffic, I/O operation counts, failed/cancelled transfer counts, source/operation trend, daily rollup, top bucket flow, recent data-flow events, and daily rollup CSV from both focused API and dashboard summary. CopyObject traffic is internal and stays separate from external ingress/egress. MariaDB mode persists the event history in `data_flow_events`, materializes daily aggregate rows into `data_flow_daily_rollups`, reads materialized rows without re-scanning detail rows, daily rollup aggregates the persisted rows by day/bucket/source/operation, scheduled retention deletes old event rows in bounded batches with audit/metrics, and scheduled materialized rollup retention deletes old `data_flow_daily_rollups` rows in bounded batches with `DATA_FLOW_DAILY_ROLLUP_RETENTION` audit and `osmu.data.flow.daily.rollup.retention.*` metrics.
- Priority: P1
