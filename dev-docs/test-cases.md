...
- Preconditions: PowerShell is available.
- Input: `powershell -ExecutionPolicy Bypass -File .\scripts\verify-data-flow-storage-plan.ps1`; target planning uses `powershell -ExecutionPolicy Bypass -File .\scripts\write-data-flow-storage-plan.ps1 -CandidateStore MARIADB_PARTITION|EXTERNAL_TIME_SERIES|DUAL_WRITE -ExpectedPeakEventsPerDay <n> -ExpectedQueryWindowDays <days> ...`.
- Steps: Generate default plan evidence, generate a fully confirmed fixture, and try a credential-shaped evidence reference. Confirm the report records candidate store, sizing, retention windows, no-object-key aggregate policy, backfill plan, rollback plan, dashboard cutover plan, retention job budget, and EXPLAIN/target-store evidence requirement.
- Expected: Default output is `plan-ready-execute-required` and does not claim partition/time-series storage is enabled. Confirmed fixture can pass only when all sizing and confirmation switches are supplied. Credential-shaped references such as `password=...` are rejected. Aggregate scope remains OSMU operations analytics, not AWS billing parity, and excludes object keys/raw event messages. `GET /api/admin/dashboard/readiness` exposes the latest plan as `dataFlowStoragePlan` and adds `DATA_FLOW_STORAGE_PLAN` while pending checks remain.
- Priority: P1
...
- Steps: Regenerate `.osmu-run/latest-operations-readiness.json` and `.osmu-run/latest-operations-readiness.md`, then verify the required live/security/storage pending checks exist for storage expansion, storage backend telemetry, Kubernetes HA/DR readiness, Kubernetes DR finalizer, security evidence finalizer, image signing, and container scan/SBOM evidence. Confirm those checks include executable remediation metadata, with workflow/workflow-command metadata when a workflow exists, and the Markdown report includes remediation command, workflow, and workflow command lines from the workflow-backed checks.
- Expected: The report remains honest about missing live/CI/storage evidence, but every remaining production/B2B operations gap points to an executable script command and, where applicable, a `gh workflow run` template so operators know the next evidence run to perform.
- Priority: P1
...
- Expected: Storage backend telemetry evidence can move through workflow run id planning, artifact collection, artifact finalizer import, readiness report, and dashboard handoff without storing raw MinIO admin output or expanding AWS S3 parity scope.
- Priority: P1
