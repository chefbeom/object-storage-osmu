...
- Preconditions: GitHub Actions can run manual workflows and repository secret `OSMU_MINIO_ADMIN_INFO_JSON_BASE64` contains base64-encoded target `mc admin info --json` output.
- Input: `gh workflow run manual-storage-backend-telemetry-evidence.yml -f environment_name=<env> -f target_cluster=<cluster> -f operator=<operator> -f minio_alias=<alias> -f evidence_ref=<run-ref> -f fail_if_not_passed=true`, then run the operations workflow run id and artifact collection helpers with the resulting run id.
- Steps: Verify the workflow writes `storage-backend-telemetry-evidence-<run_id>` with `.osmu-run/latest-storage-backend-telemetry.json` and `.md` only, removes the decoded raw admin-info input before upload, maps the run id to `StorageBackendTelemetryRunId`, and passes `-StorageBackendTelemetryArtifactPath` into `scripts/import-operations-readiness-artifacts.ps1`.
