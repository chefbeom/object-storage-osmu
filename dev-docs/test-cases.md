...
- Input: `PUT /api/admin/billing/pricing-policy`, then `GET /api/admin/billing/chargeback-preview?storageGbMonthRate=...&ingressGbRate=...&egressGbRate=...&operationThousandRate=...`, and `GET /api/admin/billing/chargeback-preview/export.csv?...`
- Steps: Create two organizations, upload data to an ORG-owned bucket, optionally generate download/copy/failure events, save a pricing policy as `ADMIN`, call the chargeback preview API and CSV export as `ADMIN` and `ORG_ADMIN`, then open Admin billing panel, save the policy, export CSV, and refresh the same preview.
- Expected: `ADMIN` can save pricing policy. `ORG_ADMIN` can read pricing policy for preview/export but cannot save it. `ADMIN` sees every organization. `ORG_ADMIN` sees only the caller organization in JSON and CSV. Preview uses saved policy values when query rates are omitted and query rates override saved values when present. The response includes current `usedBytes`, ingress/egress/internal bytes, billable successful operation count, failed/cancelled counts, per-organization cost components, and total estimated cost. CSV export returns `text/csv`, `osmu-chargeback-preview.csv`, a `TOTAL` row, and scoped `ORGANIZATION` rows. User-owned or unknown buckets are excluded. The Admin billing panel renders `billing-chargeback-panel`, editable rate/date/event-limit inputs, `chargeback-save-policy-button` for ADMIN, `chargeback-export-button`, preview totals, and organization rows. The response, CSV, and UI are preview only and do not persist invoices.
- Priority: P1
