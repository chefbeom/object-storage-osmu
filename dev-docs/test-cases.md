...
- Steps: Create two organizations, upload data to an ORG-owned bucket, optionally generate download/copy/failure events, save a pricing policy with warning/critical thresholds as `ADMIN`, call the chargeback preview, threshold alert API, alert notification preview, preview CSV export, and invoice draft CSV export as `ADMIN` and `ORG_ADMIN`, then open Admin billing panel, save the policy, preview notification payloads, export CSV/draft CSV, and refresh the same preview/alert view.
- Expected: `ADMIN` can save pricing policy. `ORG_ADMIN` can read pricing policy for preview/alerts/export but cannot save it. `ADMIN` sees every organization. `ORG_ADMIN` sees only the caller organization in JSON alert rows, notification payload rows, and CSV. Preview uses saved policy values when query rates are omitted and query rates override saved values when present. The response includes current `usedBytes`, ingress/egress/internal bytes, billable successful operation count, failed/cancelled counts, per-organization cost components, and total estimated cost. Alert response includes warning/critical threshold amounts, severity, alert counts, and scoped organization rows. Notification preview returns `mode=PREVIEW`, `externalDeliveryEnabled=false`, channel/target metadata, scoped notification subjects/messages, and machine-readable `chargeback.threshold` payloads without sending webhooks. Preview CSV export returns `text/csv`, `osmu-chargeback-preview.csv`, a `TOTAL` row, and scoped `ORGANIZATION` rows. Invoice draft CSV export returns `text/csv`, `osmu-chargeback-invoice-draft.csv`, scoped `DRAFT_INVOICE` rows, `DRAFT` invoice status, and `OSMU-DRAFT-YYYYMMDD-{organizationId}` invoice numbers. User-owned or unknown buckets are excluded. The Admin billing panel renders `billing-chargeback-panel`, editable rate/date/event-limit/threshold/notification inputs, `chargeback-save-policy-button` for ADMIN, `chargeback-alert-list`, `chargeback-notification-list`, `chargeback-export-button`, `chargeback-invoice-draft-export-button`, preview totals, and organization rows. The response, alerts, notification payloads, CSV, draft CSV, and UI are preview only and do not persist invoices or send external notifications.
- Priority: P1
