...
- Admin data-flow monitoring API/UI: `GET /api/admin/monitoring/data-flow`, dashboard summary `dataFlow`, and admin data flow panel expose upload/download/internal-copy traffic, operation counts, failure/cancel counts, source/operation trend chart, top buckets, recent flow events, filter controls, MariaDB `data_flow_events` persistence, and scheduled retention cleanup.
