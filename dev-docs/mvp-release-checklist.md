...
- Admin data-flow monitoring API/UI: `GET /api/admin/monitoring/data-flow`, dashboard summary `dataFlow`, and admin data flow panel expose upload/download traffic, operation counts, failure/cancel counts, top buckets, recent flow events, filter controls, and MariaDB `data_flow_events` persistence.
...
- Monitoring artifacts draft: Prometheus alert rules and Grafana overview dashboard verified, including data-flow failure/cancel/egress/bucket anomaly starter alerts; release report `scope.monitoringArtifacts=included`.
