...
- Operations Readiness Artifact Finalizer CI workflow draft: `.github/workflows/operations-readiness-artifact-finalizer-ci.yml` verified. The manual workflow downloads previous evidence workflow artifacts by run id and artifact name, including optional Kubernetes operations report sync evidence, imports only passing evidence to standard latest paths, writes operations readiness, and uploads the combined artifact set.
...
- Operations readiness artifact importer: `scripts/import-operations-readiness-artifacts.ps1` promotes previously collected workflow evidence only when required JSON results are passing or ready, then writes `.osmu-run/latest-operations-readiness-artifact-import.json` plus `.md`.
