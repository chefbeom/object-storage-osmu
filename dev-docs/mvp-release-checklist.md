...
- Operations artifact collection plan: `scripts/write-operations-artifact-collection-plan.ps1` converts an invocation report plus GitHub workflow run ids into expected artifact names, `gh run download` commands, Security Evidence Finalizer dispatch input, Operations Readiness Artifact Finalizer dispatch input, manual storage backend telemetry, manual data-flow storage plan, manual secret rotation, manual commercial integration, manual commercial approval, manual operations handoff package, and optional Kubernetes operations report sync artifact input, and a local import command. Its Kubernetes sync artifact note preserves the data-flow plan query-plan summary requirement for MariaDB partition/dual-write candidates. `scripts/verify-operations-artifact-collection-plan.ps1` covers missing-run-id and ready collection behavior.
