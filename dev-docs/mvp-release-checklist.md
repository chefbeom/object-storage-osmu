...
- Operations handoff package nested visibility also includes `operationsHandoffPackage.secretRotationSnapshot` and `operationsHandoffPackage.enterpriseAuthSmokeSnapshot`, so the dashboard can show core secret/certificate rotation status and enterprise auth `passed` smoke or accepted `scope-out` evidence inside the final handoff bundle.
