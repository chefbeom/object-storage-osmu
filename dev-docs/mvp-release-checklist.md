...
- Operations handoff package nested visibility also includes `operationsHandoffPackage.enterpriseAuthSmokeSnapshot`, so the dashboard can show enterprise auth `passed` smoke or accepted `scope-out` evidence inside the final handoff bundle.
