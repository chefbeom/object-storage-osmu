...
- Admin data-flow monitoring API/UI: `GET /api/admin/monitoring/data-flow`, `GET /api/admin/monitoring/data-flow/daily-rollup`, `GET /api/admin/monitoring/data-flow/export.csv`, `GET /api/admin/monitoring/data-flow/daily-rollup/export.csv`, dashboard summary `dataFlow`, and admin data flow panel expose upload/download/internal-copy traffic, operation counts, failure/cancel counts, source/operation trend chart, daily rollup rows, detailed CSV export, daily rollup CSV export, top buckets, recent flow events, filter controls, MariaDB `data_flow_events` persistence, and scheduled retention cleanup.
