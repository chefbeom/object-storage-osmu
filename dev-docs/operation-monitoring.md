...
- `scripts/write-operations-artifact-collection-plan.ps1` reads `.osmu-run/latest-operations-evidence-plan-invocation.json` and writes `.osmu-run/latest-operations-artifact-collection-plan.json` plus `.md`. Operators fill in GitHub workflow run ids to derive expected artifact names, `gh run download` commands, the Security Evidence Finalizer dispatch command, the Operations Readiness Artifact Finalizer dispatch command, and the equivalent local `import-operations-readiness-artifacts.ps1` command. The plan can also carry commercial approval and Kubernetes operations report sync artifacts so approval evidence and convergence-level deployed dashboard sync evidence are promoted with the rest of the operations bundle.
...
- `scripts/import-operations-readiness-artifacts.ps1` imports previously collected workflow or manual evidence artifacts into standard `.osmu-run/latest-*` readiness paths only after their expected result values pass. It validates commercial approval evidence with `result=passed` and selected Kubernetes operations report sync artifacts with `result=applied` before promotion. It writes `.osmu-run/latest-operations-readiness-artifact-import.json` plus `.md`.
...
- `.github/workflows/operations-readiness-artifact-finalizer-ci.yml` is the artifact aggregation workflow for this gate. Operators provide run ids and artifact names from the storage expansion, HA/DR readiness, Kubernetes DR, IAM/RBAC, security evidence, commercial approval, enterprise auth smoke, and optional Kubernetes operations report sync artifacts; it downloads available artifacts, imports passing evidence, writes the operations readiness report, and uploads the combined artifact set.
