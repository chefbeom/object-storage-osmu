...
- `scripts/write-storage-backend-telemetry-evidence.ps1 -AdminInfoJsonPath <path> -FailIfNotPassed` captures post-expansion MinIO pool/node health and capacity evidence from `mc admin info --json` output. This closes the first operator-run evidence path; backend/dashboard automatic ingestion remains future work.
