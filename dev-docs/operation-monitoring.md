...
- `scripts/write-storage-backend-telemetry-evidence.ps1 -AdminInfoJsonPath <path> -FailIfNotPassed` captures post-expansion MinIO pool/node health and capacity evidence from `mc admin info --json` output. `.github/workflows/manual-storage-backend-telemetry-evidence.yml` runs the same writer from prepared `OSMU_MINIO_ADMIN_INFO_JSON_BASE64`, deletes the decoded raw input, and uploads only `.osmu-run/latest-storage-backend-telemetry.*`. `GET /api/admin/storage/backend-status` can use the latest passed telemetry evidence as `capacitySource=storage_backend_telemetry_evidence` when direct MinIO Prometheus metrics are unavailable in MinIO mode, and the dashboard labels that value as `used from telemetry evidence`. Fully automatic workflow-side live `mc admin info` collection remains future work; the current path is prepared/operator-captured evidence ingestion.
