...
- `.github/workflows/operations-readiness-artifact-finalizer-ci.yml` is the artifact aggregation workflow for this gate. Operators provide run ids and artifact names from the storage expansion, HA/DR readiness, Kubernetes DR, IAM/RBAC, security evidence, secret rotation, commercial integration, commercial approval, enterprise auth smoke, operations handoff package, and optional Kubernetes operations report sync artifacts; it downloads available artifacts, imports passing evidence, writes the operations readiness report, and uploads the combined artifact set.
