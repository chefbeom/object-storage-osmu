...
- `scripts/import-operations-readiness-artifacts.ps1` imports previously collected workflow or manual evidence artifacts into standard `.osmu-run/latest-*` readiness paths only after their expected result values pass. It validates secret rotation, commercial integration, commercial approval, and operations handoff package evidence with `result=passed` and selected Kubernetes operations report sync artifacts with `result=applied` before promotion. Storage backend telemetry evidence is currently local target evidence and is read directly from `.osmu-run/latest-storage-backend-telemetry.json` by the readiness gate. It writes `.osmu-run/latest-operations-readiness-artifact-import.json` plus `.md`.
...
- `.github/workflows/manual-secret-rotation-evidence.yml`, `.github/workflows/manual-commercial-integration-evidence.yml`, `.github/workflows/manual-commercial-approval-evidence.yml`, and `.github/workflows/manual-operations-handoff-package.yml` wrap the corresponding no-external-call evidence writers and upload `secret-rotation-evidence-<run_id>`, `commercial-integration-evidence-<run_id>`, `commercial-approval-evidence-<run_id>`, and `operations-handoff-package-<run_id>` artifacts for later import.
- `scripts/write-storage-backend-telemetry-evidence.ps1` records MinIO pool/server/drive telemetry evidence from `mc admin info --json` file input or explicit `-Execute` mode. It stores summary counts, byte totals, server state, input SHA-256, and external evidence refs only.
...
- `scripts/write-storage-backend-telemetry-evidence.ps1 -AdminInfoJsonPath <path> -FailIfNotPassed` captures post-expansion MinIO pool/node health and capacity evidence from `mc admin info --json` output. `GET /api/admin/storage/backend-status` can use the latest passed telemetry evidence as `capacitySource=storage_backend_telemetry_evidence` when direct MinIO Prometheus metrics are unavailable in MinIO mode, and the dashboard labels that value as `used from telemetry evidence`. Fully automatic live backend collection remains future work; the current path is operator-captured evidence ingestion.
