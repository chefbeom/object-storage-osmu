...
- `.github/workflows/operations-readiness-artifact-finalizer-ci.yml` is the artifact aggregation workflow for this gate. Operators provide run ids and artifact names from the storage expansion, HA/DR readiness, Kubernetes DR, IAM/RBAC, security evidence, manual storage backend telemetry, manual data-flow storage plan, manual secret rotation, manual commercial integration, manual commercial approval, enterprise auth smoke, manual operations handoff package, and optional Kubernetes operations report sync artifacts; optional `data_flow_storage_plan_json_base64` restores direct data-flow storage transition evidence into the importer path, where unsafe raw SQL/raw EXPLAIN/credential-shaped query-plan summaries are rejected before the operations readiness report is written and the combined artifact set is uploaded.
