...
`scripts/write-storage-backend-telemetry-evidence.ps1` summarizes `mc admin info --json` output into `.osmu-run/latest-storage-backend-telemetry.json` and `.md` without storing raw admin output or credentials. Use it after storage expansion, before operations handoff, or during capacity incident review to capture pool, server, drive, health, and raw/used/free byte evidence. The writer is plan/file-input first; `-Execute` is opt-in for operators who explicitly want the script to call `mc admin info`. `.github/workflows/manual-storage-backend-telemetry-evidence.yml` provides the matching manual workflow in two modes: `prepared_base64` decodes `OSMU_MINIO_ADMIN_INFO_JSON_BASE64`, while `live` downloads `mc`, uses a temporary `MC_CONFIG_DIR`, configures the target alias from `minio_endpoint`, `OSMU_MINIO_ACCESS_KEY`, and `OSMU_MINIO_SECRET_KEY`, then runs `mc admin info --json` through the writer. Both modes upload only the summarized telemetry artifact.
...
- `GET /api/admin/monitoring/data-flow/monthly-rollup` exposes a UTC-month rollup from live events or materialized daily rows.
- `POST /api/admin/monitoring/data-flow/monthly-rollup/materialize` compacts refreshed daily rows into `data_flow_monthly_rollups` for long-window aggregate reads.
...
- `GET /api/admin/dashboard/readiness` also reads `.osmu-run/latest-data-flow-storage-plan.json`; when the plan result is not `passed`, the dashboard shows a `DATA_FLOW_STORAGE_PLAN` operations warning with sizing, candidate store, pending check count, query-plan evidence summary, and scope policy.
...
- `GET /api/admin/monitoring/data-flow/monthly-rollup/export.csv` and `GET /api/admin/monitoring/data-flow/monthly-rollup/materialized/export.csv` export live/daily-derived and stored monthly aggregate rows without object keys or raw event messages.
...
- `OSMU_DATA_FLOW_MONTHLY_ROLLUP_RETENTION_ENABLED=true`
- `OSMU_DATA_FLOW_MONTHLY_ROLLUP_RETENTION_DAYS=1825`
- `OSMU_DATA_FLOW_MONTHLY_ROLLUP_RETENTION_BATCH_SIZE=1000`
...
- Tune data-flow alert thresholds and Alertmanager routes from `infra/monitoring/alert-threshold-targets.yaml` against target tenant baselines before production SLO claims, then record the review with `scripts/write-monitoring-threshold-evidence.ps1` or `.github/workflows/manual-monitoring-threshold-evidence.yml` and feed both the evidence ref and reduced passed snapshot into `write-operations-handoff-package.ps1 -MonitoringEvidenceRef -MonitoringThresholdJsonPath`. The handoff package stores alert/route/Grafana/tuning/confirmation counts only and rejects raw Alertmanager receiver secrets or credential-shaped threshold snapshots.
...
- data-flow event, daily rollup, or monthly rollup retention cleanup failure
...
- Pending live/security/commercial/package/storage telemetry/data-flow evidence checks include structured `remediation` metadata in the JSON report and remediation command/workflow/workflow-command lines in the Markdown report when a workflow path exists. Operators can use those entries to run the exact storage expansion, storage backend telemetry, data-flow storage transition plan, HA/DR, DR, image signing, container security, secret rotation evidence, commercial integration evidence, commercial approval evidence, operations handoff package evidence, or security finalizer path needed to close each gap. The data-flow transition check accepts `.osmu-run/latest-data-flow-storage-plan.json` only when it is `result=passed`; MariaDB partition and dual-write candidates must carry the sanitized `osmu.mariadb-query-plan-evidence.v1` summary with zero failed checks.
...
- `scripts/write-operations-workflow-run-id-plan.ps1` reads `.osmu-run/latest-operations-evidence-plan-invocation.json` and writes `.osmu-run/latest-operations-workflow-run-ids.json` plus `.md`. In plan-only mode it emits the exact `gh run list --workflow ... --json ...` commands operators need after workflow dispatch. With fixture JSON or `-Execute`, it selects latest successful run ids, derives image/container security artifact names from run `headSha`, and emits follow-up commands for `security-evidence-finalizer-ci.yml` and `write-operations-artifact-collection-plan.ps1`. Local target evidence writer commands are mapped to `manual-storage-backend-telemetry-evidence.yml`, `manual-monitoring-threshold-evidence.yml`, `manual-data-flow-storage-plan-evidence.yml`, `manual-data-flow-storage-transition-runbook-evidence.yml`, `manual-secret-rotation-evidence.yml`, `manual-commercial-integration-evidence.yml`, `manual-commercial-approval-evidence.yml`, and `manual-operations-handoff-package.yml` so their uploaded artifacts can be collected by run id. When `kubernetes-operations-report-sync-ci.yml` is present in the invocation, the generated artifact collection command also carries `-KubernetesOperationsReportSyncRunId`.
- `scripts/write-operations-artifact-collection-plan.ps1` reads `.osmu-run/latest-operations-evidence-plan-invocation.json` and writes `.osmu-run/latest-operations-artifact-collection-plan.json` plus `.md`. Operators fill in GitHub workflow run ids to derive expected artifact names, `gh run download` commands, the Security Evidence Finalizer dispatch command, the Operations Readiness Artifact Finalizer dispatch command, and the equivalent local `import-operations-readiness-artifacts.ps1` command. The plan can also carry storage backend telemetry, monitoring threshold, data-flow storage plan, data-flow storage transition runbook, secret rotation, commercial integration, commercial approval, enterprise auth smoke or scope-out, operations handoff package, and Kubernetes operations report sync artifacts from either local evidence writer commands or direct `gh workflow run manual-*.yml` dispatches, so target environment evidence and convergence-level deployed dashboard sync evidence are promoted with the rest of the operations bundle.
- `scripts/finalize-operations-readiness.ps1` is the operator wrapper for this gate. It can plan or run the selected storage expansion finalizer, Kubernetes HA/DR readiness check, Kubernetes DR finalizer, IAM/RBAC finalizer, security evidence finalizer, and final operations readiness report, then writes `.osmu-run/latest-operations-readiness-finalize.json` plus `.md`. It passes `-DataFlowStoragePlanPath` through to `write-operations-readiness.ps1` so imported or target-specific data-flow transition evidence is evaluated by the same final gate.
...
- `.github/workflows/operations-readiness-finalizer-ci.yml` exposes the same wrapper as a manual workflow. It runs plan-only by default, accepts optional `data_flow_storage_plan_json_base64` for passed `.osmu-run/latest-data-flow-storage-plan.json` target evidence, rejects MariaDB partition/dual-write query-plan summaries with raw SQL/raw EXPLAIN/credential-shaped content, and requires `OSMU_KUBECONFIG_BASE64` for live Kubernetes evidence.
- `scripts/import-operations-readiness-artifacts.ps1` imports previously collected workflow or manual evidence artifacts into standard `.osmu-run/latest-*` readiness paths only after their expected result values pass. It validates storage backend telemetry, monitoring threshold, secret rotation, commercial integration, commercial approval, and operations handoff package evidence with `result=passed`, enterprise auth smoke or contracted deferral evidence with `result=passed|scope-out`, and selected Kubernetes operations report sync artifacts with `result=applied` before promotion. Monitoring threshold artifacts must include complete alert target mapping and Prometheus/Grafana/Alertmanager/baseline/routing confirmations, and credential-shaped receiver/secret content is rejected. Data-flow storage plans can be imported either from the Kubernetes operations report sync artifact or a direct `DataFlowStoragePlanArtifactPath`; MariaDB partition/dual-write plans must include sanitized query-plan summary evidence, and raw SQL/raw EXPLAIN/credential-shaped summary content is rejected. Data-flow storage transition runbook artifacts can be imported through `DataFlowStorageTransitionRunbookArtifactPath` only when `result=passed`, the embedded storage plan snapshot is passed, and raw SQL/raw EXPLAIN/object key/raw event message/credential-shaped content is absent. It writes `.osmu-run/latest-operations-readiness-artifact-import.json` plus `.md`.
...
- `scripts/write-operations-handoff-package.ps1` records the pilot or production handoff package as `.osmu-run/latest-operations-handoff-package.json` plus `.md`. It wraps already-collected readiness, convergence, data-flow storage transition plan/runbook, secret rotation, monitoring threshold, commercial integration, commercial approval, enterprise auth, backup/restore, HA/DR, monitoring, security, IAM/RBAC, runbook, troubleshooting, support escalation, and known-gap references into one target evidence file. When `-RequireOperationsSnapshotEvidence` is used, it rejects supplied operations readiness/convergence snapshots with raw remediation/provider/customer/contract/license/payment/identity fields, parses sanitized `.osmu-run/latest-operations-readiness.json` and `.osmu-run/latest-operations-readiness-convergence.json`, then stores only result/count/top-check/sync summary fields. When `-RequireProductionEvidence` is used, it also requires `-DataFlowStoragePlanEvidenceRef`, `-DataFlowStorageTransitionRunbookEvidenceRef`, `-CommercialIntegrationEvidenceRef`, `-CommercialApprovalEvidenceRef`, `-EnterpriseAuthEvidenceRef`, reviewed `result=passed` data-flow plan/runbook/monitoring threshold/commercial snapshots through `-DataFlowStoragePlanJsonPath`, `-DataFlowStorageTransitionRunbookJsonPath`, `-MonitoringThresholdJsonPath`, `-CommercialIntegrationJsonPath`, and `-CommercialApprovalJsonPath`, explicit `-ConfirmCommercialIntegrationSnapshotReviewed`, `-ConfirmCommercialApprovalSnapshotReviewed`, and `-ConfirmMonitoringThresholdReviewed`, and reviewed `result=passed` or accepted `result=scope-out` enterprise auth smoke snapshots through `-EnterpriseAuthJsonPath`; it stores only reduced sizing/count/query-plan/runbook/alert-route/commercial/enterprise-auth status summary fields while rejecting raw SQL, raw EXPLAIN JSON, object keys, raw event messages, raw Alertmanager receiver secrets, raw identity claims, OIDC codes/states/tokens, LDAP/admin passwords, raw provider/customer/payment/price-table/contract content, and credentials. It does not execute `kubectl`, `gh`, provider APIs, storage migrations, IdP/directory login flows, or notification/payment adapters; the handoff completion timestamp must be the same as or later than the handoff start timestamp.
- `.github/workflows/manual-storage-backend-telemetry-evidence.yml`, `.github/workflows/manual-monitoring-threshold-evidence.yml`, `.github/workflows/manual-data-flow-storage-plan-evidence.yml`, `.github/workflows/manual-data-flow-storage-transition-runbook-evidence.yml`, `.github/workflows/manual-secret-rotation-evidence.yml`, `.github/workflows/manual-commercial-integration-evidence.yml`, `.github/workflows/manual-commercial-approval-evidence.yml`, and `.github/workflows/manual-operations-handoff-package.yml` wrap the corresponding evidence writers and upload `storage-backend-telemetry-evidence-<run_id>`, `monitoring-threshold-evidence-<run_id>`, `data-flow-storage-plan-evidence-<run_id>`, `data-flow-storage-transition-runbook-evidence-<run_id>`, `secret-rotation-evidence-<run_id>`, `commercial-integration-evidence-<run_id>`, `commercial-approval-evidence-<run_id>`, and `operations-handoff-package-<run_id>` artifacts for later import. The storage backend telemetry workflow supports both prepared base64 admin-info ingestion and live target `mc admin info --json` collection, then removes decoded raw input or temporary mc config before artifact upload. The monitoring threshold workflow records target Prometheus rule, Grafana dashboard, Alertmanager route, incident routing, and tenant baseline review refs only, then uploads the reduced evidence. The data-flow plan workflow accepts target sizing/retention/backfill/rollback/dashboard-cutover confirmations and optional base64 sanitized MariaDB query-plan evidence, deletes the decoded query-plan input before artifact upload, and still stores no raw SQL, raw EXPLAIN, credentials, object keys, or raw event messages. The data-flow transition runbook workflow accepts a base64 sanitized passed storage plan snapshot plus target backfill/dual-write or partition toggle/rollback/reconciliation/dashboard cutover/retention dry-run references, deletes the decoded plan input before artifact upload, and uploads only reduced runbook evidence. The commercial integration workflow accepts base64 output from `GET /api/admin/billing/payment-provider-adapter-readiness`, rejects raw provider responses, endpoint URLs, and customer payment fields, reduces the accepted snapshot to sanitized profile/status counts, deletes the decoded temporary snapshot before artifact upload, and keeps native card/bank/tax/ERP processor APIs outside the readiness gate unless concrete adapters are later implemented. The commercial approval workflow accepts base64 output from `GET /api/admin/billing/pricing-policy-proposals?status=PRICE_LIST_APPROVED`, rejects raw contract/customer/license/payment/raw price table fields, reduces the accepted snapshot to sanitized proposal approval status/reference metadata, deletes the decoded temporary snapshot before artifact upload, and excludes raw price/rate tables and raw contract text from artifacts. The operations handoff workflow accepts base64 readiness/convergence/data-flow storage plan/data-flow storage transition runbook/monitoring threshold/commercial/enterprise-auth evidence JSON snapshots, requires explicit `confirm_commercial_integration_snapshot_reviewed=true`, `confirm_commercial_approval_snapshot_reviewed=true`, and `confirm_monitoring_threshold_reviewed=true` when production evidence is required, deletes the decoded temporary files after the writer runs, and uploads only the reduced handoff package evidence.
- The data-flow workflow and writer also require `target_p95_query_latency_ms` / `TargetP95QueryLatencyMs` so partition/time-series evidence captures the target p95 query latency budget before storage handoff.
- `scripts/write-data-flow-storage-transition-runbook-evidence.ps1` records the target transition rehearsal after `.osmu-run/latest-data-flow-storage-plan.json` has `result=passed`. It requires external references and confirmations for backfill rehearsal, dual-write or partition toggle review, rollback rehearsal, row/count reconciliation, dashboard cutover review, retention dry-run review, no-object-key aggregate scope, and no-secret evidence. It stores only reduced plan summary fields and rejects raw SQL, raw EXPLAIN JSON, object keys, raw event messages, private keys, bearer tokens, and credential-shaped references.
- `scripts/write-storage-backend-telemetry-evidence.ps1` records MinIO pool/server/drive telemetry evidence from `mc admin info --json` file input or explicit `-Execute` mode. It stores summary counts, byte totals, server state, input SHA-256, and external evidence refs only; `.github/workflows/manual-storage-backend-telemetry-evidence.yml` is the CI wrapper for prepared target admin-info JSON or live target collection.
- `scripts/verify-storage-backend-telemetry-evidence.ps1` self-tests the telemetry writer with a fixture, verifies secret-like reference rejection, and confirms raw MinIO admin output is not stored.
- `scripts/write-operations-readiness-convergence.ps1` reads the latest handoff, operations readiness, operations readiness finalizer, and Kubernetes operations report sync reports, then writes `.osmu-run/latest-operations-readiness-convergence.json` plus `.md`. It does not execute `kubectl`, `gh`, workflow dispatch, finalizer, or ConfigMap sync commands; it summarizes the current bottleneck, ready decision, stage counts, finalizer gap counts, Kubernetes report sync result/readiness, local recommended command chain, and the equivalent `kubernetes-operations-report-sync-ci.yml` workflow command with optional `data_flow_storage_plan_json_base64` handoff. The workflow note reminds operators that MariaDB partition/dual-write plans must include the sanitized query-plan evidence summary. The convergence result is `ready` only after the operations readiness finalizer report exists with `result=ready` and `readinessResult=ready`, and the Kubernetes report sync evidence is `applied` with zero failed checks.
- `.github/workflows/operations-readiness-artifact-finalizer-ci.yml` is the artifact aggregation workflow for this gate. Operators provide run ids and artifact names from the storage expansion, HA/DR readiness, Kubernetes DR, IAM/RBAC, security evidence, manual storage backend telemetry, manual monitoring threshold, manual data-flow storage plan, manual data-flow storage transition runbook, manual secret rotation, manual commercial integration, manual commercial approval, enterprise auth smoke, manual operations handoff package, and optional Kubernetes operations report sync artifacts; optional `data_flow_storage_plan_json_base64` and `data_flow_storage_transition_runbook_json_base64` restore direct data-flow transition evidence into the importer path, where unsafe raw SQL/raw EXPLAIN/object key/raw event message/credential-shaped content is rejected before the operations readiness report is written and the combined artifact set is uploaded.
- The admin dashboard readiness API reads `.osmu-run/latest-operations-readiness.json`, `.osmu-run/latest-operations-evidence-plan.json`, `.osmu-run/latest-operations-evidence-plan-invocation.json`, `.osmu-run/latest-operations-invocation-unblock-plan.json`, `.osmu-run/latest-operations-dispatch-preflight.json`, `.osmu-run/latest-operations-workflow-run-ids.json`, `.osmu-run/latest-operations-artifact-collection-plan.json`, `.osmu-run/latest-operations-readiness-artifact-import.json`, `.osmu-run/latest-operations-readiness-finalize.json`, `.osmu-run/latest-operations-evidence-handoff.json`, `.osmu-run/latest-operations-handoff-package.json`, `.osmu-run/latest-storage-expansion-finalize.json`, `.osmu-run/latest-kubernetes-ha-dr-readiness.json`, `.osmu-run/latest-kubernetes-dr-finalize.json`, `.osmu-run/latest-iam-rbac-finalize.json`, `.osmu-run/latest-monitoring-threshold-evidence.json`, `.osmu-run/latest-data-flow-storage-plan.json`, `.osmu-run/latest-operations-readiness-convergence.json`, and `.osmu-run/latest-kubernetes-operations-report-sync.json` when they exist. Pending checks are surfaced as `OPERATIONS` readiness items targeting `dashboard-readiness-panel`, including evidence path and remediation command/workflow/workflow-command metadata when the source check provides it. The evidence plan is surfaced as `OPERATIONS_EVIDENCE_PLAN` with the plan path and regeneration command, and as structured `operationsEvidencePlan.actions` entries for UI execution ordering. The guarded invocation report is surfaced as `OPERATIONS_EVIDENCE_PLAN_INVOCATION` with its invocation path and command, and as `operationsEvidenceInvocation.actions` entries showing planned/blocked status, block reasons, unresolved placeholders, and dispatch commands. The invocation unblock plan is surfaced as `OPERATIONS_INVOCATION_UNBLOCK_PLAN` and as `operationsInvocationUnblockPlan`, including required confirmation flags, placeholder input mapping, repeated-placeholder warnings, blocked/planned action order lists, and copyable follow-up plan commands. The dispatch preflight report is surfaced as `OPERATIONS_DISPATCH_PREFLIGHT` and as `operationsDispatchPreflight`, including failed/warning check counts, missing required inputs, required GitHub secret names, workflow file presence, preflight checks, required input rows, and plan/execute command previews when ready. The workflow run id plan is surfaced as `OPERATIONS_WORKFLOW_RUN_ID_PLAN` and as `operationsWorkflowRunIdPlan`, including workflow query commands, missing run counts, recommended run id state, artifact collection follow-up command, and security evidence finalizer command. The artifact collection plan is surfaced as `OPERATIONS_ARTIFACT_COLLECTION_PLAN` and as `operationsArtifactCollectionPlan`, including expected artifact names, `gh run download` commands, finalizer commands, missing required artifact counts, and local import command. The readiness artifact import report is surfaced as `OPERATIONS_READINESS_ARTIFACT_IMPORT` and as `operationsReadinessArtifactImport`, including import status, selected group count, imported/failed counts, entry-level source/destination paths, and the no-secret import policy. The readiness finalizer report is surfaced as `OPERATIONS_READINESS_FINALIZER` and as `operationsReadinessFinalize`, including selected finalizer steps, final readiness result, gaps, commands, step results, and secret masking policy. The evidence handoff is surfaced as `OPERATIONS_EVIDENCE_HANDOFF` and as `operationsEvidenceHandoff`, including current bottleneck, next step command, stage summaries, blocked action count, missing workflow run count, missing required artifact count, finalizer failed count, and finalizer gap count. The handoff package is surfaced as `OPERATIONS_HANDOFF_PACKAGE` and as `operationsHandoffPackage`, including target environment/cluster/operator, confirmation flags, failed/planned/check counts, top check rows, nested readiness/convergence summaries, nested data-flow storage plan store/count/query-plan summary, nested commercial integration required/pass/payment-adapter summary, nested commercial approval version/price-list/failure summary, and no-secret policy. Storage expansion, Kubernetes HA/DR, and Kubernetes DR evidence are surfaced as `STORAGE_EXPANSION_FINALIZE`, `KUBERNETES_HA_DR_READINESS`, and `KUBERNETES_DR_FINALIZE` plus `storageExpansionFinalize`, `kubernetesHaDrReadiness`, and `kubernetesDrFinalize`, including result/status, target namespace/tenant/restore labels, failed/gap counts, top check/step rows, and no-secret policy. IAM/RBAC finalizer evidence is surfaced as `IAM_RBAC_EVIDENCE` and as `iamRbacEvidence`, including result/status, namespace/service account, backend/live auth selection flags, failed count, gap list, command names, step results, and no-secret policy. Monitoring threshold evidence is surfaced as `MONITORING_THRESHOLD_EVIDENCE` and as `monitoringThresholdEvidence`, including target labels, review window, alert mapping counts, routes, Grafana/tuning counts, confirmation flags, external evidence refs, check rows, and no-secret policy. The data-flow storage plan is surfaced as `DATA_FLOW_STORAGE_PLAN` and as `dataFlowStoragePlan`, including candidate store, target sizing, retention windows, pending check count, check rows, and OSMU analytics scope policy. The convergence report is surfaced as `OPERATIONS_READINESS_CONVERGENCE` and as `operationsReadinessConvergence`, including ready/action-required decision, current bottleneck, recommended commands, stage counts, finalizer status, Kubernetes report sync ready/result/failed count/ConfigMap target, and no-execute safety policy. Kubernetes report sync evidence is surfaced as `KUBERNETES_OPERATIONS_REPORT_SYNC` and as `kubernetesOperationsReportSync`, including namespace, ConfigMap name/key, source report hash, plan/server-dry-run/apply commands, checks, failed count, and write-safety policy.
- The surfaced `dataFlowStoragePlan` includes `targetP95QueryLatencyMs`, and the handoff package plan/runbook snapshots carry the same budget with query-plan and transition rehearsal summaries.
- The surfaced `operationsHandoffPackage.monitoringThresholdSnapshot` carries the reduced monitoring threshold handoff summary with required/mapped alert counts, route count/names, Grafana/tuning counts, confirmation flags, and failed/check counts, while excluding raw Alertmanager receiver secrets.
- The same readiness API reads `.osmu-run/latest-storage-backend-telemetry.json` as `storageBackendTelemetryEvidence` when present, exposing only result, environment/cluster/operator labels, source mode, non-secret evidence reference, admin-info SHA-256, pool/server/drive counts, online/offline state, and capacity totals. It also reads `.osmu-run/latest-storage-expansion-finalize.json`, `.osmu-run/latest-kubernetes-ha-dr-readiness.json`, and `.osmu-run/latest-kubernetes-dr-finalize.json` as structured storage/DR evidence, exposing only result/status, namespace/tenant/restore labels, selected execution flags, failed/gap counts, top check/step rows, and no-secret policy. It also reads `.osmu-run/latest-iam-rbac-finalize.json` as `iamRbacEvidence`, exposing result/status, namespace/service account, selected backend/live auth flags, failed count, sanitized gaps, command names, step results, and no-secret policy. It also reads `.osmu-run/latest-security-evidence-finalize.json`, `.osmu-run/latest-image-signing-evidence.json`, and `.osmu-run/latest-container-security-evidence.json` as `securityEvidence`, exposing finalizer result, image signing digest/signature flags, container scan/SBOM package/hash summaries, top finalizer checks, and no-secret policy. It also reads `.osmu-run/latest-secret-rotation-evidence.json` as `secretRotationEvidence`, exposing rotation window, sanitized external evidence refs, confirmation flags, core rotation coverage, failed/planned checks, rotation/check summaries, and no-secret policy. It does not expose raw `mc admin info` output, credentials, secret values, private keys, bearer tokens, kubeconfig, provider secrets, registry credentials, signing keys, raw IAM/RBAC command output, raw kubectl output, or raw SBOM files. The data-flow storage plan similarly exposes only the sanitized MariaDB query-plan evidence summary and never raw SQL, raw EXPLAIN JSON, or credentials.
- The same readiness API reads `.osmu-run/latest-monitoring-threshold-evidence.json` as `monitoringThresholdEvidence`, exposing only target labels, review window, required/mapped alert counts, routes, Grafana panel count, tuning evidence count, external evidence refs, confirmation flags, failed/check counts, top checks, and no-secret policy. It never exposes raw Alertmanager receiver secrets, webhook secrets, credentials, bearer tokens, kubeconfig, private keys, or raw customer data.
- The same readiness API reads `.osmu-run/latest-minio-bucket-cors-verification.json` as `minioBucketCorsVerification` when present. It exposes only source mode, bucket/alias labels, normalized allowed/exposed headers, max-age values, failed/planned counts, top checks, and operator commands for OSMU browser multipart upload readiness. It never exposes raw CORS XML, credentials, bearer tokens, private keys, MinIO root credentials, or object data, and it is not AWS S3 parity work.
- The frontend readiness panel highlights those `OPERATIONS` items with an operations evidence gaps summary, evidence plan summary line, invocation summary line, invocation unblock summary line, dispatch preflight summary line, workflow run id summary line, artifact collection summary line, artifact import summary line, operations finalizer summary line, evidence handoff summary line, handoff package summary line, handoff package data-flow plan/runbook/monitoring threshold/commercial/enterprise-auth snapshot summary lines, storage expansion finalizer summary line, Kubernetes HA/DR summary line, Kubernetes DR finalizer summary line, IAM/RBAC evidence summary line, security evidence summary line, secret rotation evidence summary line, data-flow storage plan summary line, storage backend telemetry summary line, monitoring threshold evidence summary line, MinIO bucket CORS summary line, convergence summary line, Kubernetes report sync summary line, evidence plan action list, invocation action list, unblock action/input list, dispatch preflight check/input/workflow rows, workflow query command list, artifact download command list, artifact import entry rows, finalizer command/step rows, handoff stage list, handoff package check list, storage expansion step/gap lists, Kubernetes HA/DR check list, Kubernetes DR step/gap lists, IAM/RBAC evidence step/gap lists, security evidence check list, secret rotation rotation/check lists, monitoring threshold check list, MinIO bucket CORS check list, data-flow storage plan check list, convergence command list, Kubernetes report sync check list, a quick `Operations` filter, and inline remediation command/workflow command/evidence details with copy controls, keeping production/B2B pending evidence visible even when the dashboard also has runtime, backup, quota, or sharing warnings.
...
- The Docker local demo mounts project `.osmu-run` into the backend container read-only. `scripts/verify-browser-e2e-local-demo.ps1` writes a Docker-local convergence fixture, passes the mounted container-relative path through `OSMU_OPERATIONS_READINESS_CONVERGENCE_REPORT_PATH`, and enables the same Browser E2E convergence check when Docker is available. Its `-EnableRealMultipartFixture` mode rebuilds the frontend with small multipart threshold and 5 MiB part size, injects the seeded demo user and writable bucket from `.osmu-run/latest-demo.json`, delays real MinIO PUT requests with Playwright before continuing them, and verifies browser pause/resume without API/storage mocks. This is browser upload operations evidence, not AWS S3 parity expansion.
- Kubernetes manifests and the Helm chart mount operations report files into the backend container at `/app/.osmu-run`. The default contract is an optional read-only `osmu-operations-reports` ConfigMap containing `latest-operations-readiness-convergence.json`, `latest-data-flow-storage-plan.json` when the plan exists, and, when promoted by the operator, `latest-kubernetes-operations-report-sync.json`; Helm can switch the same mount to a PVC with `backend.operationsReports.type=persistentVolumeClaim` for larger or frequently refreshed evidence sets.
- `scripts/sync-kubernetes-operations-reports.ps1` validates the local convergence report, records source byte count and SHA256, includes a valid local data-flow storage plan when present, records the sanitized data-flow query-plan evidence summary, and creates refresh commands/evidence for the `osmu-operations-reports` ConfigMap. MariaDB partition or dual-write plans must include the query-plan summary before ConfigMap publish, and raw SQL/raw EXPLAIN/credential-shaped summary content fails the plan. Default/`-PlanOnly` mode does not execute `kubectl`, `-ServerDryRunOnly` validates with the API server without persisting changes, and `-Apply` is required to update the ConfigMap. After a successful apply, the script writes `latest-kubernetes-operations-report-sync.json` and republishes the ConfigMap with the convergence report, optional data-flow storage plan, and sync evidence unless `-SkipEvidenceConfigMapPublish` is supplied.
- `scripts/verify-kubernetes-operations-report-mount.ps1 -Namespace <namespace>` is the read-only post-apply verifier for that delivery path. It checks the `osmu-operations-reports` ConfigMap contains expected convergence/sync keys plus the data-flow storage plan key and sanitized query-plan summary when local plan evidence exists, compares the JSON contract with local evidence when available, selects a ready backend Pod by `app.kubernetes.io/name=osmu-backend`, and reads `/app/.osmu-run/latest-operations-readiness-convergence.json`, `/app/.osmu-run/latest-kubernetes-operations-report-sync.json`, and when expected `/app/.osmu-run/latest-data-flow-storage-plan.json` through `kubectl exec cat` without reading Kubernetes Secrets.
- `.github/workflows/kubernetes-operations-report-sync-ci.yml` exposes the same sync helper as a manual workflow. It writes the convergence report, can restore optional base64-encoded `.osmu-run/latest-data-flow-storage-plan.json` evidence before sync, validates the MariaDB partition/dual-write query-plan summary contract and rejects raw SQL/raw EXPLAIN/credential-shaped summary content before running the sync helper, creates plan evidence by default, uses `OSMU_KUBECONFIG_BASE64` only when `run_live=true`, runs server dry-run before any apply, and requires both `run_live=true` and `apply=true` before the ConfigMap is updated. Its uploaded artifact includes the optional data-flow storage plan, and `scripts/import-operations-readiness-artifacts.ps1` promotes that plan back into the standard dashboard path after artifact collection only when MariaDB partition/dual-write plans include the sanitized query-plan evidence summary contract.
...
- Use `scripts/verify-operations-readiness.ps1` in local verification to ensure the readiness artifact keeps the required check list and does not silently drop an operations gate, including storage backend telemetry `result=passed`, data-flow storage transition `result=passed` with target query-plan evidence for MariaDB partition/dual-write candidates, commercial approval `result=passed`, and enterprise auth `result=passed` or accepted `result=scope-out` evidence. Use `scripts/verify-storage-backend-telemetry-evidence.ps1` to verify the target MinIO pool/node telemetry writer, summary shape, and secret-like input rejection. Use `scripts/verify-ci-workflow.ps1` to verify the dedicated storage backend telemetry and enterprise auth smoke workflow contracts, including decoded enterprise auth input cleanup. Use `scripts/verify-secret-rotation-evidence.ps1` to verify the target secret/certificate rotation evidence writer, timestamp order, and secret-like reference rejection. Use `scripts/verify-enterprise-auth-smoke-plan.ps1` to verify enterprise auth plan-only/scope-out evidence, secret redaction, and token-bearing OIDC claim/JIT input rejection. Use `scripts/verify-commercial-integration-evidence.ps1` to verify the target commercial integration evidence writer, timestamp order, payment-provider adapter readiness snapshot parsing, native processor scope boundary, credential-like reference rejection, and unsafe raw provider/customer snapshot rejection. Use `scripts/verify-commercial-approval-evidence.ps1` to verify the final pricing/licensing/legal approval evidence writer, billing pricing proposal approval snapshot parsing, raw price exclusion, unsafe raw commercial snapshot rejection, and credential-like reference rejection. Use `scripts/verify-operations-handoff-package.ps1` to verify the target operations handoff package writer, timestamp order, commercial approval reference, readiness/convergence/data-flow plan/data-flow runbook/monitoring threshold/commercial/enterprise-auth snapshot parsing, commercial integration/approval and monitoring threshold review confirmation, missing required snapshot failure, raw price-table rejection, unsafe raw identity-claim rejection, and credential-like reference/snapshot rejection. Use `scripts/verify-operations-evidence-plan.ps1` to keep the executable evidence plan schema, placeholder extraction, approval flags, and unplanned-check handling covered. Use `scripts/verify-operations-evidence-plan-invocation.ps1` to verify that the invocation helper blocks unsafe or incomplete actions and produces planned workflow commands only after required confirmations and replacements are supplied. Use `scripts/verify-operations-invocation-unblock-plan.ps1` to verify the blocked invocation to operator-input handoff. Use `scripts/verify-operations-dispatch-preflight.ps1` to verify the dispatch-ready gate before adding `-Execute`. Use `scripts/verify-operations-workflow-run-id-plan.ps1` to cover the workflow run id query/recommendation handoff, including storage backend telemetry and optional Kubernetes operations report sync run ids. Use `scripts/verify-operations-artifact-collection-plan.ps1` to verify the workflow run id to artifact name mapping and import/finalizer command generation. Use `scripts/verify-operations-readiness-artifact-import.ps1` to verify evidence promotion, including rejection of MariaDB/dual-write data-flow plans that omit the query-plan summary or include unsafe raw SQL/raw EXPLAIN/credential-shaped summary content. Use `scripts/verify-operations-evidence-handoff.ps1` to verify the top-level next-action handoff across missing, blocked, artifact-finalizer-ready, operations-finalizer-missing, operations-finalizer-pending, and readiness-ready/finalizer-missing states. Use `scripts/verify-operations-readiness-convergence.ps1` to verify the final no-execute convergence summary for missing handoff, action-required, finalizer-required, sync-required, and ready states. Use `scripts/verify-kubernetes-operations-report-sync.ps1` to verify the ConfigMap sync helper in plan, server dry-run, apply, and unsafe query-plan summary rejection paths with a fake kubectl. Use `scripts/verify-kubernetes-operations-report-mount.ps1 -Namespace <namespace>` after apply to prove the ConfigMap data and backend Pod mounted files match. Use `scripts/verify-kubernetes-operations-report-sync-live.ps1 -ApiBase <api-base> -AdminLoginId <admin> -AdminPassword <secret>` after deployment to confirm the dashboard readiness API is actually reading the applied sync evidence and the mounted data-flow storage plan; this live verifier masks credentials and stores only status, result, ConfigMap metadata, data-flow plan result/candidate/pending count, query-plan evidence summary, retry count, and non-secret polling summaries. The default dashboard poll is 6 attempts with 10 seconds between attempts; tune it with `-DashboardRetryCount` and `-DashboardRetryDelaySeconds` for slower ConfigMap volume refresh windows.
- The same artifact import verification also covers monitoring threshold promotion plus data-flow transition runbook promotion, rejecting threshold snapshots containing credential-shaped content and runbook snapshots containing raw SQL, raw EXPLAIN, object keys, raw event messages, or credential-shaped content.
...
- `scripts/finalize-storage-expansion.ps1 -RunStorageBackendTelemetryEvidence -StorageBackendTelemetryAdminInfoJsonPath <path> -StorageBackendTelemetryEnvironmentName <env> -StorageBackendTelemetryTargetCluster <cluster> -StorageBackendTelemetryOperator <operator> -StorageBackendTelemetryMinioAlias <alias> -StorageBackendTelemetryEvidenceRef <ref>` can record the same post-expansion MinIO pool/node/capacity telemetry evidence inside the storage expansion finalizer flow. The finalizer report links `.osmu-run/latest-storage-backend-telemetry.json` in both `storageBackendTelemetry` and `evidence.storageBackendTelemetry`; missing admin-info input fails before Kubernetes or backend apply work proceeds.
- `scripts/write-storage-backend-telemetry-evidence.ps1 -AdminInfoJsonPath <path> -FailIfNotPassed` captures post-expansion MinIO pool/node health and capacity evidence from `mc admin info --json` output. `.github/workflows/manual-storage-backend-telemetry-evidence.yml` runs the same writer from prepared `OSMU_MINIO_ADMIN_INFO_JSON_BASE64` or with `collection_mode=live -f minio_endpoint=<endpoint>` plus `OSMU_MINIO_ACCESS_KEY`/`OSMU_MINIO_SECRET_KEY` secrets, deletes decoded raw input or temporary mc config, and uploads only `.osmu-run/latest-storage-backend-telemetry.*`. `GET /api/admin/storage/backend-status` can use the latest passed telemetry evidence as `capacitySource=storage_backend_telemetry_evidence` when direct MinIO Prometheus metrics are unavailable in MinIO mode, and the dashboard labels that value as `used from telemetry evidence`.
- The finalizer report masks admin password values and does not write bearer tokens or secret values to evidence files.
- `.github/workflows/storage-expansion-finalizer-ci.yml` provides the same path as a manual workflow. It runs plan-only by default, requires `OSMU_KUBECONFIG_BASE64` for live Kubernetes evidence, and uploads finalizer/RBAC/dry-run evidence plus optional storage backend telemetry artifacts for review.
...
- `scripts/write-monitoring-threshold-evidence.ps1` records target Prometheus rule load, Grafana import, Alertmanager route review, incident routing review, and target tenant baseline tuning evidence as `.osmu-run/latest-monitoring-threshold-evidence.json` plus `.md` without storing receiver secrets. `.github/workflows/manual-monitoring-threshold-evidence.yml` wraps the same writer, and the artifact collection/import chain can promote the passed artifact into the standard readiness handoff path.
