...
- Pending live/security/commercial/package/storage telemetry evidence checks include structured `remediation` metadata in the JSON report and remediation command/workflow/workflow-command lines in the Markdown report when a workflow path exists. Operators can use those entries to run the exact storage expansion, storage backend telemetry, HA/DR, DR, image signing, container security, secret rotation evidence, commercial integration evidence, commercial approval evidence, operations handoff package evidence, or security finalizer path needed to close each gap.
