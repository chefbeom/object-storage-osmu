...
- `scripts/write-enterprise-auth-smoke-plan.ps1`은 실제 IdP/LDAP 연결 검증을 위한 `.osmu-run/latest-enterprise-auth-smoke.json`/Markdown evidence를 생성한다. 기본값은 plan-only라 HTTP 요청을 실행하지 않으며, `-Execute`와 필요한 credential/state가 명시된 경우에만 OIDC/LDAP smoke를 수행한다. `result=passed` evidence는 typed integer summary counts로 `passCount>0`, `failCount=0`, `blockedCount=0`, `plannedCount=0`을 증명해야 operations readiness/import/handoff promotion을 통과한다. 파일럿 계약에서 enterprise auth를 명시적으로 제외하거나 후속 단계로 미룬 경우 `-ConfirmScopeOut`, `-ScopeOutRef`, `-ScopeOutReason`으로 `result=scope-out` evidence를 남긴다. evidence에는 admin password, LDAP password, access/refresh token, OIDC authorization code/state, raw claim JSON, credential-like scope-out reference를 기록하지 않는다.
- `scripts/write-enterprise-auth-jit-rollback-evidence.ps1`은 admin-approved JIT provisioning을 production scope에 넣기 전 rollback/runbook 증거를 `.osmu-run/latest-enterprise-auth-jit-rollback-evidence.json`/Markdown으로 기록한다. 이 증거는 admin approval 유지, OIDC callback auto-JIT 비활성, JIT user disable/lock rollback, role/org/team mapping rollback, local password fallback, JIT audit review, no raw claim/no secret 확인을 요구하며 raw OIDC claim, token, LDAP/admin password, client secret, IdP/directory raw response를 저장하지 않는다.
- `scripts/write-secret-rotation-evidence.ps1`은 target 환경의 secret/certificate rotation 실행 증거를 `.osmu-run/latest-secret-rotation-evidence.json`/Markdown으로 기록한다. evidence에는 환경명, operator, timestamp, change/audit/rollout/smoke 참조와 boolean confirmation만 저장하며, 완료 시각은 시작 시각 이후여야 하고 password, token, private key, kubeconfig, database/MinIO/OIDC/LDAP/SMTP/webhook secret 값은 저장하지 않는다.
- `scripts/write-commercial-integration-evidence.ps1`은 target 환경의 notification webhook/Slack/EMAIL SMTP relay, generic/CARD/BANK/TAX/ERP payment webhook profile handoff, payment-provider adapter readiness snapshot 검증 증거를 `.osmu-run/latest-commercial-integration-evidence.json`/Markdown으로 기록한다. readiness snapshot은 `GET /api/admin/billing/payment-provider-adapter-readiness`의 sanitized status/count/profile metadata만 요약하며, evidence에는 외부 증거 참조와 boolean confirmation만 저장하고 webhook URL credential, SMTP password, provider credential, signing secret, raw provider response, 고객 결제 데이터는 저장하지 않는다.
- `scripts/write-commercial-approval-evidence.ps1`은 final pricing, terms, support SLA, license agreement, legal approval, pilot contract boundary 승인 증거와 billing pricing proposal의 commercial price-list approval snapshot을 `.osmu-run/latest-commercial-approval-evidence.json`/Markdown으로 기록한다. proposal snapshot은 status, approvedPriceList 여부, commercialApprovalReference, 승인 시각 같은 sanitized metadata만 저장하며 가격표 원문, rate 값, 계약서 원문, 고객 결제 데이터, license key, password, token, private key 값은 저장하지 않는다.
...
- `ORG_ADMIN`은 자기 조직 사용자, 조직 usage, billing pricing policy read-only 값, chargeback preview, data-flow daily rollup chargeback trend/export, threshold alerts, notification payload preview/outbox, scoped CSV export와 draft invoice CSV export만 볼 수 있다.
- `ORG_ADMIN`은 자기 조직 일반 `USER` 생성/비활성화만 가능하다.
- `ORG_ADMIN` 허용 route는 `GET/POST /api/admin/users`, `PATCH /api/admin/users/{userId}/status`, `GET /api/admin/organizations`, `GET /api/admin/organizations/usage`, `GET /api/admin/billing/pricing-policy`, `GET /api/admin/billing/chargeback-preview`, `GET /api/admin/billing/chargeback-daily-rollup`, `GET /api/admin/billing/chargeback-daily-rollup/export.csv`, `GET /api/admin/billing/chargeback-alerts`, `GET /api/admin/billing/chargeback-alert-notifications/preview`, `GET/POST /api/admin/billing/chargeback-alert-notifications/outbox`, `GET /api/admin/billing/chargeback-preview/export.csv`, `GET /api/admin/billing/chargeback-invoice-draft/export.csv`, `GET/POST /api/admin/teams`, `PUT /api/admin/teams/{teamId}/members`, `DELETE /api/admin/teams/{teamId}`로 제한한다.
- `GET/POST /api/admin/billing/chargeback-invoice-drafts`, `POST /api/admin/billing/chargeback-invoice-drafts/{invoiceId}/approve`, `POST /api/admin/billing/chargeback-invoice-drafts/{invoiceId}/finalize`, `GET /api/admin/billing/chargeback-invoices`, `POST /api/admin/billing/chargeback-invoices/{invoiceId}/payment-request`, `GET/POST /api/admin/billing/chargeback-invoices/{invoiceId}/payment-provider-handoff`, `GET /api/admin/billing/chargeback-payment-provider-handoffs`, `POST /api/admin/billing/chargeback-alert-notifications/outbox/{deliveryId}/adapter-result`, `POST /api/admin/billing/chargeback-alert-notifications/outbox/{deliveryId}/adapter-send`, `POST /api/admin/billing/chargeback-payment-provider-handoffs/{handoffId}/adapter-result`, `POST /api/admin/billing/chargeback-payment-provider-handoffs/{handoffId}/adapter-send`, `GET /api/admin/billing/chargeback-adapter-retry-worker/status`, `POST /api/admin/billing/chargeback-adapter-retry-worker/run`, `POST /api/admin/billing/chargeback-invoices/{invoiceId}/payment-record`는 청구 검토, 최종 invoice, payment workflow 상태, payment handoff outbox, adapter result/retry state를 남기므로 `ADMIN` 전용이다. payment record는 외부 결제망을 호출하지 않고 수동 reconciliation reference만 저장하며, notification generic webhook/Slack/EMAIL SMTP relay/payment native-bridge-or-webhook send/result와 retry worker는 secret 값, endpoint URL, SMTP password, native auth-header value, provider 응답 원문을 저장하지 않는다.
- Webhook, native bridge, and EMAIL SMTP relay send adapters reject `localhost`, loopback, link-local, private IPv4/IPv6, and local-domain endpoints by default before any external call. Local/dev or explicitly approved internal integrations must opt in with `osmu.billing.notification-delivery.allow-private-network=true`, `osmu.billing.notification-delivery.email.allow-private-network=true`, `osmu.billing.payment-provider.allow-private-network=true`, or `osmu.billing.payment-provider.native-api.allow-private-network=true`.
- Notification webhook/Slack/EMAIL and payment native-bridge-or-webhook handoff adapters enforce configurable outbound payload size caps before external calls: `osmu.billing.notification-delivery.max-payload-bytes`, `osmu.billing.payment-provider.max-payload-bytes`, and `osmu.billing.payment-provider.native-api.max-payload-bytes`; each cap defaults to `65536` bytes and is normalized before use.
- Generic notification webhook, payment-provider webhook handoff, and configurable native payment-provider bridge adapters can sign outbound JSON payloads with timestamped HMAC-SHA256 headers using `osmu.billing.notification-delivery.signature-secret`, `osmu.billing.payment-provider.signature-secret`, or `osmu.billing.payment-provider.native-api.signature-secret`; signing secrets are configuration-only and are not stored in outbox/history rows, audit logs, or provider response records.
