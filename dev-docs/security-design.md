...
- `GET/POST /api/admin/billing/chargeback-invoice-drafts`와 `POST /api/admin/billing/chargeback-invoice-drafts/{invoiceId}/approve`는 내부 청구 검토 기록을 남기므로 `ADMIN` 전용이다. `APPROVED_INTERNAL`도 법적 최종 invoice/payment request는 아니다.
