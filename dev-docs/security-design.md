...
- `GET/POST /api/admin/billing/chargeback-invoice-drafts`, `POST /api/admin/billing/chargeback-invoice-drafts/{invoiceId}/approve`, `POST /api/admin/billing/chargeback-invoice-drafts/{invoiceId}/finalize`, `GET /api/admin/billing/chargeback-invoices`, `POST /api/admin/billing/chargeback-invoices/{invoiceId}/payment-request`, `GET/POST /api/admin/billing/chargeback-invoices/{invoiceId}/payment-provider-handoff`, `GET /api/admin/billing/chargeback-payment-provider-handoffs`, `POST /api/admin/billing/chargeback-alert-notifications/outbox/{deliveryId}/adapter-result`, `POST /api/admin/billing/chargeback-payment-provider-handoffs/{handoffId}/adapter-result`, `GET /api/admin/billing/chargeback-adapter-retry-worker/status`, `POST /api/admin/billing/chargeback-adapter-retry-worker/run`, `POST /api/admin/billing/chargeback-invoices/{invoiceId}/payment-record`는 청구 검토, 최종 invoice, payment workflow 상태, no-send payment handoff outbox, adapter result/retry state를 남기므로 `ADMIN` 전용이다. payment record는 외부 결제망을 호출하지 않고 수동 reconciliation reference만 저장하며, notification/payment adapter result와 retry worker는 secret 값이나 provider 응답 원문을 저장하지 않는다.
- `GET/POST /api/admin/billing/pricing-policy-proposals`와 `POST /api/admin/billing/pricing-policy-proposals/{proposalId}/approve`는 내부 chargeback 가격 정책 승인 기록을 남기고 실제 계산 정책을 변경하므로 `ADMIN` 전용이다. `APPROVED_APPLIED`도 외부 고객용 확정 가격표 승인이 아니다.
