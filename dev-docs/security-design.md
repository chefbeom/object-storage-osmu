...
- `scripts/write-commercial-approval-evidence.ps1`은 final pricing, terms, support SLA, license agreement, legal approval, pilot contract boundary 승인 증거를 `.osmu-run/latest-commercial-approval-evidence.json`/Markdown으로 기록한다. evidence에는 승인 참조, approver, timestamp, boolean confirmation만 저장하며 가격표 원문, 계약서 원문, 고객 결제 데이터, license key, password, token, private key 값은 저장하지 않는다.
