...
- `ORG_ADMIN`은 자기 조직 사용자, 조직 usage, billing pricing policy read-only 값, chargeback preview, data-flow daily rollup chargeback trend, threshold alerts, notification payload preview/outbox, scoped CSV export와 draft invoice CSV export만 볼 수 있다.
...
- `ChargebackPreviewService`는 `ORG_ADMIN` 요청에서 actor의 organization id만 조회하고, user-owned bucket과 deleted/unknown bucket data-flow event는 chargeback preview와 daily rollup trend에서 제외한다.
