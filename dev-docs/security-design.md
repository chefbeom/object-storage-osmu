...
- `scripts/write-commercial-integration-evidence.ps1`은 target 환경의 notification webhook/Slack/EMAIL SMTP relay와 generic/CARD/BANK/TAX/ERP payment webhook profile handoff 검증 증거를 `.osmu-run/latest-commercial-integration-evidence.json`/Markdown으로 기록한다. evidence에는 외부 증거 참조와 boolean confirmation만 저장하며 webhook URL credential, SMTP password, provider credential, signing secret, raw provider response, 고객 결제 데이터는 저장하지 않는다.
