...
- Data-flow storage plan visibility: `dataFlowStoragePlan`이 있으면 readiness panel은 `readiness-data-flow-storage-plan-summary`, `readiness-data-flow-query-plan-evidence-summary`, `readiness-data-flow-storage-plan-checks`, `DATA_FLOW_STORAGE_PLAN` item summary로 candidate store, sizing, pending check count, query-plan evidence summary, scope policy를 표시한다. 이 표시는 partitioned/time-series 저장소가 켜졌다는 의미가 아니라 target 전환 전 evidence가 남았는지 보여주는 운영 readiness 상태다.
