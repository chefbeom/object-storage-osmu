...
- Secret rotation evidence visibility: `secretRotationEvidence`가 있으면 readiness panel은 target secret/certificate rotation 결과, rotation window, external evidence refs, confirmation flags, core rotation coverage, failed/planned check count, no-secret policy, 상위 rotation/check 상태를 표시한다. 이로써 운영자는 target 환경의 secret/certificate rotation gap을 별도 JSON 열람 없이 확인할 수 있다.
- Commercial evidence visibility: `commercialIntegrationEvidence`와 `commercialApprovalEvidence`가 있으면 readiness panel은 target integration coverage, payment-provider adapter readiness status, commercial/legal approval references, pricing proposal approval count, failed/planned check count, no-secret policy, 상위 check 상태를 표시한다. 이로써 운영자는 상업/법무 evidence JSON을 따로 열지 않고도 production/B2B readiness를 막는 commercial gap을 확인할 수 있다.
- Enterprise auth smoke visibility: `enterpriseAuthSmokeEvidence`가 있으면 readiness panel은 target OIDC/LDAP smoke 결과, 실행 모드, 필수 입력 제공 여부, pass/fail/blocked/planned count, accepted scope-out reference/reason, no-secret policy, 상위 check 상태를 표시한다. AWS S3 호환은 대체 가능한 수준으로 유지하고, paid/pilot 전환에 필요한 OSMU enterprise auth readiness gap을 우선 보여준다.
