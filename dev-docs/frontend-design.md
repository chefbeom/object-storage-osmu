...
- MinIO bucket CORS visibility: `minioBucketCorsVerification`이 있으면 readiness panel은 browser multipart upload에 필요한 CORS 검증 결과, bucket/alias, source mode, exposed header count, failure/planned count, normalized expose headers, top check 상태, no-raw-XML/no-secret scope policy를 표시한다. 이는 OSMU browser upload 운영 조건 검증이며 AWS S3 parity 확대가 아니다.
