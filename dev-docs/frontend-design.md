...
- Chargeback preview uses `getBillingPricingPolicy`, `saveBillingPricingPolicy`, `getChargebackPreview`, `getChargebackAlerts`, `getChargebackAlertNotificationPreview`, `queueChargebackAlertNotifications`, `getChargebackAlertNotificationOutbox`, `recordChargebackAlertNotificationAdapterResult`, `getChargebackAdapterRetryWorkerStatus`, `runChargebackAdapterRetryWorker`, `createChargebackInvoiceDrafts`, `getChargebackInvoiceDrafts`, `approveChargebackInvoiceDraft`, `finalizeChargebackInvoiceDraft`, `getChargebackFinalInvoices`, `requestChargebackInvoicePayment`, `queueChargebackPaymentProviderHandoff`, `getChargebackPaymentProviderHandoffs`, `recordChargebackPaymentProviderHandoffAdapterResult`, `recordChargebackInvoicePayment`, `downloadChargebackPreviewCsv`, and `downloadChargebackInvoiceDraftCsv` through `BillingChargebackPanel` in Admin; it exposes date/rate/event-limit inputs, warning/critical threshold policy inputs, notification channel/target preview inputs, payment provider/target handoff inputs, ADMIN-only policy save, scoped alert list, scoped notification payload preview, scoped notification outbox/history with block/retry controls (`chargeback-notification-adapter-block-button`, `chargeback-notification-adapter-retry-button`), adapter retry worker dry-run/run controls (`chargeback-adapter-retry-worker-refresh-button`, `chargeback-adapter-retry-worker-run-button`), scoped CSV export, draft invoice CSV export, ADMIN-only draft invoice persistence/internal approval/finalization list, final invoice payment request/handoff/paid controls, payment provider handoff outbox/history with block/retry controls (`chargeback-payment-handoff-adapter-block-button`, `chargeback-payment-handoff-adapter-retry-button`), organization cost rows, and preview totals without sending external notification or payment provider calls.
...
- BillingChargebackPanel은 `getBillingPricingPolicyProposals`, `createBillingPricingPolicyProposal`, `approveBillingPricingPolicyProposal` wrapper로 ADMIN-only internal pricing proposal queue를 표시한다. `billing-pricing-policy-proposal-button`, `billing-pricing-policy-proposal-list`, `billing-pricing-policy-proposal-approve-button`은 내부 chargeback 계산 정책 승인용이며 외부 확정 가격표 승인 UI가 아니다.
