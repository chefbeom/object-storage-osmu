...
- Storage backend telemetry visibility: `storageBackendTelemetryEvidence`가 있으면 readiness panel은 target environment/cluster/operator, pool/server/drive count, offline server count, used/total capacity, admin-info SHA-256, scope policy를 표시한다. 원본 `mc admin info` 출력과 credential은 표시하지 않는다.
