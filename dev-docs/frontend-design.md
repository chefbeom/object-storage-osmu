...
- Operations handoff package visibility: `operationsHandoffPackage`가 있으면 readiness panel은 package result, target environment/cluster/operator, failed/planned/check count, secret policy, 상위 check 상태를 표시한다. Handoff evidence가 readiness/convergence snapshot을 포함하면 result/count/sync summary도 같은 evidence payload에서 확인할 수 있다. 이로써 운영자는 최종 handoff package evidence가 runbook/troubleshooting/rollback/support/known-gap 검토까지 통과했는지 dashboard에서 바로 확인할 수 있다.
