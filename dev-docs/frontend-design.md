...
- Rule inventory uses status and target selects, a default 50/max 200 opaque cursor page, stale-response sequence protection, and `lifecycle-rule-load-more-button` for bounded append. The dashboard displays a `+` suffix when more rules exist.
- UI calls `GET/POST/DELETE /api/admin/object-lifecycle/rules`. The GET wrapper sends `status`, `targetType`, `cursor`, and `limit`.
...
- Readiness panel은 `OPERATIONS` category 항목이 있을 때 operations evidence gaps 요약과 `Operations` 빠른 필터 버튼을 표시해 operations readiness report, operations evidence plan, evidence plan invocation, invocation unblock plan, workflow run id plan, artifact import pending, operations finalizer pending, evidence handoff를 다른 warning 사이에서 바로 찾을 수 있게 한다. Pending check가 remediation metadata를 포함하면 로컬 명령, workflow path, `gh workflow run` 명령, evidence path, note를 접힌 보조 정보처럼 작은 코드/텍스트 블록으로 표시하고 command/workflow command copy 버튼을 제공해 운영자가 CLI 보고서를 열지 않아도 다음 evidence run을 바로 복사할 수 있게 한다. `OPERATIONS_EVIDENCE_PLAN` 항목이 있으면 summary 안에 plan 상태와 action count를 한 줄로 노출하고, `operationsEvidencePlan.actions` 상위 항목을 `readiness-evidence-plan-actions` list로 표시해 실행 순서, 입력 placeholder, 승인 필요 여부, kubeconfig 필요 여부, 복사 가능한 recommended command를 바로 확인할 수 있게 한다. `OPERATIONS_EVIDENCE_PLAN_INVOCATION` 항목과 `operationsEvidenceInvocation.actions`가 있으면 invocation 결과, planned/blocked/failed count, action별 block reason, unresolved placeholder, 복사 가능한 dispatch command를 보여줘 live workflow 실행 전 누락 조건을 UI에서 확인할 수 있게 한다. `OPERATIONS_INVOCATION_UNBLOCK_PLAN` 항목과 `operationsInvocationUnblockPlan.actions`가 있으면 required confirmation, placeholder input mapping, ambiguous placeholder warning, copyable confirmed/blocked-only/action plan command를 보여줘 workflow dispatch 전에 운영자가 무엇을 채워야 하는지 UI에서 정리할 수 있게 한다. `OPERATIONS_WORKFLOW_RUN_ID_PLAN` 항목과 `operationsWorkflowRunIdPlan.workflows`가 있으면 workflow query-required/ready/stale 상태, source invocation report/result, branch/query mode/limit, image signing version/commit SHA, workflow별 `gh run list` query command, GitHub REST API run-id query/copy command, artifact collection plan follow-up command, security evidence finalizer command, 그리고 `securityEvidenceFinalizerReady`/required inputs/hint rows/missing inputs/dependency note 요약을 보여줘 workflow dispatch 후 run id 수집 단계와 security finalizer의 `ImageSigningRunId` + `ContainerSecurityRunId` 의존성을 추적할 수 있게 한다. `operationsEvidenceHandoff`와 `operationsReadinessConvergence`가 있으면 run-id query mode, 실행 여부, 성공/오류/후보 run count, GitHub API token/unauthenticated 여부, input-free blocked review result/count/action orders/stale/scope 상태와 operator input values result/missing/non-ready action 요약을 한 줄로 보여줘 dispatch 이후 run id가 아직 없는 상태인지, input-free review가 최신 범위로 다시 막힘을 증명하는지, operator input values가 어떤 action에서 아직 비어 있는지, 또는 API 조회 자체가 실패한 상태인지 구분할 수 있게 한다.
- Artifact collection visibility: `operationsArtifactCollectionPlan`이 있으면 readiness panel은 collection result, invocation source/result/summary, top-level selected action order summary, ready/required/missing artifact count, Security Evidence Finalizer command, structured security finalizer input rows from `securityEvidenceFinalizerInputs[]`, Operations Readiness Artifact Finalizer command, optional direct data-flow storage plan/runbook base64 input note, optional MinIO bucket CORS input note, local import command, artifact별 `gh run download` command를 표시한다. Secret rotation, commercial integration, commercial approval, data-flow transition runbook, operations handoff package, optional MinIO bucket CORS dashboard evidence, Kubernetes operations report sync artifact가 포함되면 같은 목록에서 target evidence, approval evidence, runbook rehearsal evidence, handoff evidence, browser upload CORS evidence, deployed dashboard sync evidence 다운로드/가져오기 경로도 확인할 수 있다. 이로써 live workflow dispatch 이후 run id를 채우고 artifact import로 넘어가는 단계를 dashboard에서 바로 추적할 수 있다.
- Evidence handoff visibility: `operationsEvidenceHandoff`가 있으면 readiness panel은 현재 bottleneck result, next step code, blocked/missing count, input-free blocked review result/action orders/stale/scope summary, 복사 가능한 next command, browser dispatch checklist rows, 전체 stage 상태를 표시한다. 이로써 운영자는 Markdown 보고서를 열기 전에도 지금 당장 `invoke-operations-evidence-plan.ps1`, run id collection, artifact finalizer, artifact import 중 어느 단계로 가야 하는지 확인할 수 있다.
- Dispatch preflight visibility: when `operationsDispatchPreflight` exists, the readiness panel shows result/source counts, selected/ready/blocked action orders, confirmation requirements, failed/missing/unsafe/invalid/ambiguous/warning counts, required GitHub secrets, GitHub repository/ref context, Git ref safety summary/note, copy buttons for plan/execute/API/ready-subset/API commands, the Git ref safety note plus a push command only when the working tree is clean, preflight check rows, required input safe/valid rows, workflow file rows, and per-workflow web dispatch links before live dispatch.
- Artifact import visibility: `operationsReadinessArtifactImport`가 있으면 readiness panel은 import result/status, selected group count, imported/failed count, output directory, secret policy, selected entry 전체의 source/destination path와 detail을 표시한다. 이로써 운영자는 artifact finalizer 이후 어떤 evidence file이 promote됐고 어떤 파일이 실패했는지 dashboard에서 확인할 수 있다.
- Operations finalizer visibility: `operationsReadinessFinalize`가 있으면 readiness panel은 finalizer result/status, readiness result/summary, namespace/source namespace/restore namespace/backup timestamp context, selected step flags, evidence paths, failed/gap count, secret masking policy, 상위 command, 상위 step result를 표시한다. 이로써 운영자는 artifact import 이후 combined operations readiness finalizer가 어떤 하위 finalizer를 선택했고 마지막 readiness 재생성 결과가 무엇인지 dashboard에서 확인할 수 있다.
- Operations handoff package visibility: `operationsHandoffPackage`가 있으면 readiness panel은 package result, target environment/cluster/operator, failed/planned/check count, secret policy, 전체 check rows, target evidence refs, readiness snapshot result/count, convergence snapshot readiness/sync source/finalizer failed/finalizer gap summary와 failed/gap/sync-ready/sync-count typed invalid marker, data-flow storage plan snapshot result/store/count/query-plan summary, data-flow storage transition runbook snapshot result/plan/store/failure summary, secret rotation snapshot result/core/failure summary, monitoring threshold snapshot result/required/mapped alert/route/failure summary, commercial integration snapshot required/pass/payment-adapter summary, commercial approval snapshot version/price-list/failure summary, chargeback closeout snapshot billing-period/final-invoice/paid/reconciliation/no-raw-data summary, enterprise auth smoke/scope-out snapshot result/pass/fail/blocked summary와 cluster network access review/Helm values hardening snapshot result/count/static/confirmation summary를 표시한다. 이로써 운영자는 최종 handoff package evidence가 runbook/troubleshooting/rollback/support/known-gap 검토와 readiness/convergence/data-flow plan/runbook/secret rotation/monitoring threshold/commercial/chargeback closeout/enterprise-auth smoke snapshot 검토까지 통과했는지, 그리고 `result=ready` convergence snapshot에 숨은 finalizer 실패나 deployed-report sync source drift가 없는지 dashboard에서 바로 확인할 수 있다.
- Operations handoff package chargeback closeout visibility: operationsHandoffPackage.chargebackCloseoutSnapshot가 있으면 readiness panel은 closeout result, billing period, final invoice count, paid invoice count, scan limit, source completeness, truncation blocker count, reconciliation difference, no-raw-data flag를 handoff package snapshot summary로 표시한다.
- Operations handoff package JIT rollback visibility: operationsHandoffPackage.enterpriseAuthJitRollbackSnapshot가 있으면 readiness panel은 JIT rollback result/failure/check count, nested enterprise auth smoke result, no-raw-claims confirmation을 handoff package snapshot summary로 표시한다.
- Storage/HA/DR evidence visibility: `storageExpansionFinalize`, `kubernetesHaDrReadiness`, `kubernetesDrFinalize`가 있으면 readiness panel은 storage expansion finalizer result, execution window, service account/impersonation, backend dry-run/apply/confirm/telemetry 선택 여부, Kubernetes HA/DR namespace/kubectl/restore manifest/check 실패, DR restore namespace/backup timestamp/confirmation/mode option, command, 전체 gap rows, step 상태를 표시한다. 이로써 운영자는 target storage expansion과 Kubernetes DR evidence gap을 operations readiness report만 열지 않고 확인할 수 있다.
- IAM/RBAC evidence visibility: `iamRbacEvidence`가 있으면 readiness panel은 finalizer result/status, execution window, target namespace/service account, backend policy test/live `kubectl auth can-i` 선택 여부, failed count, run-command availability, command summary, 전체 gap rows, step result, no-secret policy를 표시한다. 이로써 운영자는 IAM/RBAC finalizer artifact를 열기 전에 app role matrix와 Kubernetes ServiceAccount 권한 evidence gap을 확인할 수 있다.
- Security evidence visibility: `securityEvidence`가 있으면 readiness panel은 security finalizer result, synthetic evidence mode, image signing result/digest/signature flags/signing mode, container scan/SBOM result, scan severity/ignore-unfixed flag, SBOM package/hash summary, source/promoted artifact refs, failed check count, no-secret policy, 상위 finalizer check 상태를 표시한다. 이로써 운영자는 signed image와 container scan/SBOM evidence gap을 workflow artifact를 직접 열기 전에 확인할 수 있다.
- Secret rotation evidence visibility: `secretRotationEvidence`가 있으면 readiness panel은 target secret/certificate rotation 결과, target environment/cluster/operator, rotation window, external evidence refs, confirmation flags, core rotation coverage, failed/planned check count, no-secret policy, 전체 rotation/check rows를 표시한다. 이로써 운영자는 target 환경의 secret/certificate rotation gap을 별도 JSON 열람 없이 확인할 수 있다.
- Commercial evidence visibility: `commercialIntegrationEvidence`와 `commercialApprovalEvidence`가 있으면 readiness panel은 target integration coverage, payment-provider adapter readiness status/reviewed flag/webhook-ready/native-ready profile count, commercial/legal approval references, approval time, pricing proposal commercial approval count, price-list approval count, confirmation flags, failed/planned check count, scope/no-secret policy, 전체 check rows를 표시한다. 이로써 운영자는 상업/법무 evidence JSON을 따로 열지 않고도 production/B2B readiness를 막는 commercial gap을 확인할 수 있다.
- Enterprise auth smoke visibility: `enterpriseAuthSmokeEvidence`가 있으면 readiness panel은 target OIDC/LDAP smoke 결과, 실행 모드, API base, 필수 입력 제공 여부, pass/fail/blocked/planned/skipped count, accepted scope-out reference/reason, no-secret policy, 전체 check rows를 표시한다. AWS S3 호환은 대체 가능한 수준으로 유지하고, paid/pilot 전환에 필요한 OSMU enterprise auth readiness gap을 우선 보여준다.
- Storage backend telemetry visibility: `storageBackendTelemetryEvidence`가 있으면 readiness panel은 target environment/cluster/operator, source mode, MinIO alias, evidence ref, pool/server/drive count, online/offline server count, used/free/total capacity와 capacity-known state, admin-info SHA-256, scope policy를 표시한다. 원본 `mc admin info` 출력과 credential은 표시하지 않는다.
- Monitoring threshold evidence visibility: `monitoringThresholdEvidence`가 있으면 readiness panel은 target environment/cluster/operator, review window, threshold target path, required/mapped/missing alert count, Alertmanager route count와 route 목록, Grafana panel count, tuning evidence count, mapping coverage 상태, external evidence refs, confirmation flags, failed/check count, no-secret policy, 전체 check rows를 표시한다. 이로써 target Prometheus/Grafana/Alertmanager/tenant baseline 검토 gap을 별도 JSON 열람 없이 확인할 수 있다.
- Cluster network/Helm hardening evidence visibility: `clusterNetworkAccessReviewEvidence`와 `helmValuesHardeningEvidence`가 있으면 readiness panel은 `readiness-cluster-network-access-review-summary`, `readiness-cluster-network-access-review-static-snapshot`, `readiness-cluster-network-access-review-checks`, `readiness-helm-values-hardening-summary`, `readiness-helm-values-hardening-static-snapshot`, `readiness-helm-values-hardening-checks` selector로 target environment/cluster/operator, review window, access/hardening refs, static NetworkPolicy/Helm snapshot fields, confirmation booleans, failed/check count, no-credential policy, 전체 check rows를 표시한다. 이로써 operations handoff package 최종화 전 Kubernetes network boundary와 Helm hardening gap을 별도 JSON 열람 없이 확인할 수 있다.
- Support escalation handoff evidence visibility: `supportEscalationHandoffEvidence`가 있으면 readiness panel은 `readiness-support-escalation-handoff-summary`, `readiness-support-escalation-handoff-review-window`, `readiness-support-escalation-handoff-evidence-refs`, `readiness-support-escalation-handoff-documents`, `readiness-support-escalation-handoff-confirmations`, `readiness-support-escalation-handoff-checks` selector로 target environment/cluster/operator, review window, support/runbook/rollback/SLA/known-gap refs, document coverage booleans, operator confirmations, failed/check count, no-secret policy, 전체 check rows를 표시한다. 이로써 operations handoff package 최종화 전 support escalation/SLA 검토 gap을 별도 JSON 열람 없이 확인할 수 있다.- MinIO bucket CORS visibility: `minioBucketCorsVerification`이 있으면 readiness panel은 browser multipart upload에 필요한 CORS 검증 결과, bucket/alias, source mode, exposed header count, failure/planned count, normalized allowed/expose headers, max-age values, operator command copy buttons, top check 상태, no-raw-XML/no-secret scope policy를 표시한다. 이는 OSMU browser upload 운영 조건 검증이며 AWS S3 parity 확대가 아니다.
- Data-flow storage plan visibility: `dataFlowStoragePlan`이 있으면 readiness panel은 `readiness-data-flow-storage-plan-summary`, `readiness-data-flow-query-plan-evidence-summary`, `readiness-data-flow-query-plan-failed-checks`, `readiness-data-flow-storage-plan-checks`, `DATA_FLOW_STORAGE_PLAN` item summary로 candidate store, sizing, pending check count, query-plan evidence result/mode/format/source/detail, failed query-plan check metadata, candidate decision, 전체 storage-plan check rows, scope policy를 표시하고, operations handoff package의 data-flow snapshot에서도 같은 candidate decision 요약을 보여준다. 이 표시는 partitioned/time-series 저장소가 켜졌다는 의미가 아니라 target 전환 전 evidence가 남았는지 보여주는 운영 readiness 상태다.
- Data-flow transition runbook visibility: `dataFlowStorageTransitionRunbook`이 있으면 readiness panel은 `readiness-data-flow-storage-transition-runbook-summary`, `readiness-data-flow-storage-transition-runbook-confirmations`, `readiness-data-flow-storage-transition-runbook-checks`, `DATA_FLOW_STORAGE_TRANSITION_RUNBOOK` item summary로 target label, storage-plan result, candidate store, target p95 latency, failed/check count, confirmation flags, 전체 check rows, no-raw-SQL/no-secret scope policy를 표시한다. 이로써 target backfill/rollback/reconciliation/dashboard cutover/retention rehearsal gap을 handoff package를 열기 전 dashboard에서 확인할 수 있다.
- Operations convergence visibility: `operationsReadinessConvergence`가 있으면 readiness panel은 ready/action-required result, current bottleneck plus browser dispatch links, `handoffBrowserDispatchDependencyNotes` fallback notes, readiness/finalizer result, finalizer failed/gap count, stage readiness, Kubernetes report sync result/source result/ready state plus stale/timestamp/freshness reason, handoff freshness line with `handoffStale` plus handoff/readiness timestamps, mirrored handoff input-free blocked review result/action orders/stale/scope summary, mirrored handoff operator input values result/missing/non-ready action summary, no-execute safety policy, 전체 recommended command rows, 그리고 `kubernetes-operations-report-sync-ci.yml` workflow command/data-flow plan input note를 표시한다. 이로써 운영자는 handoff, readiness, finalizer report를 따로 열기 전에 지금 전체 operations workflow가 ready로 수렴했는지 또는 어떤 command chain과 Actions handoff가 남았는지 dashboard에서 확인할 수 있다.
- Kubernetes operations report sync visibility: `kubernetesOperationsReportSync`가 있으면 readiness panel은 source report/result/hash, ConfigMap target, data-flow plan/runbook keys, server dry-run/apply command, workflow path, safety policy, 전체 sync check rows를 표시한다. 이로써 deployed dashboard sync evidence가 어떤 validation step에서 막혔는지 JSON report를 열지 않고 확인할 수 있다.
...
- `admin-approval-workflow-panel`은 Storage Profile `PENDING/APPROVED` 요청과 Storage Expansion `PLANNED/APPROVED` 요청을 한 큐로 묶어 승인, 반려, dry-run 계획, 적용 단계를 보여준다. 각 row는 `admin-approval-profile-*`, `admin-approval-expansion-*` 버튼으로 기존 admin API action을 재사용한다. Storage Profile queue는 기본 `OPEN`, 개별 상태/전체 필터, cursor `Load more`, stale response 무시로 큰 이력에서도 열린 작업을 우선 표시한다.
- StoragePage의 Storage Profile history는 선택 버킷만 `limit=5` cursor page로 읽고 `storage-profile-history-load-more-button`으로 이어 붙인다. 관리자 승인 큐와 별도 배열/커서/요청 순번을 사용해 라우트 전환이나 버킷 변경의 늦은 응답이 다른 목록을 덮지 않는다.
- QuotaPolicyPanel loads 50-policy cursor pages with a Load more control, key-based deduplication, and sequence guards; quota history stays on the first page while an appended inventory request is in flight.- IdentityAdminPanel loads users and teams with independent cursor state, `Load more` actions, team organization filtering, and sequence guards so an older response cannot replace the current page.
...
- Docker local demo frontend image도 `VITE_MULTIPART_UPLOAD_THRESHOLD_BYTES`와 `VITE_MULTIPART_UPLOAD_PART_SIZE_BYTES` build arg를 받는다. `verify-browser-e2e-local-demo.ps1 -EnableRealMultipartFixture`는 threshold 1 MiB, part size 5 MiB, concurrency 1로 rebuild하고 real MinIO PUT을 지연시켜 pause/resume을 검증한다.
...
- The Admin page exposes a dedicated Storage Layout panel for JBOD and RAID0/1/5/6/10-like PVC intent plans.
- The panel creates a plan with StorageClass, server count, volumes per server, PVC size, and a reason; it lists theoretical raw and usable capacity separately from real cluster capacity.
- Plans can be approved or rejected, then simulated. Simulation shows the PVC topology, StorageClass, planned MinIO pool, and a YAML preview while retaining a strict no-cluster-mutation boundary.
- The UI does not offer RAID2/3/4/7/8/9 and does not claim that a MinIO or CSI backend has been changed.