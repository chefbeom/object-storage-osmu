...
- Secret rotation evidence visibility: `secretRotationEvidence`가 있으면 readiness panel은 target secret/certificate rotation 결과, target environment/cluster/operator, rotation window, external evidence refs, confirmation flags, core rotation coverage, failed/planned check count, no-secret policy, 상위 rotation/check 상태를 표시한다. 이로써 운영자는 target 환경의 secret/certificate rotation gap을 별도 JSON 열람 없이 확인할 수 있다.
- Commercial evidence visibility: `commercialIntegrationEvidence`와 `commercialApprovalEvidence`가 있으면 readiness panel은 target integration coverage, payment-provider adapter readiness status/reviewed flag/webhook-ready/native-ready profile count, commercial/legal approval references, approval time, pricing proposal commercial approval count, price-list approval count, confirmation flags, failed/planned check count, scope/no-secret policy, 상위 check 상태를 표시한다. 이로써 운영자는 상업/법무 evidence JSON을 따로 열지 않고도 production/B2B readiness를 막는 commercial gap을 확인할 수 있다.
- Enterprise auth smoke visibility: `enterpriseAuthSmokeEvidence`가 있으면 readiness panel은 target OIDC/LDAP smoke 결과, 실행 모드, API base, 필수 입력 제공 여부, pass/fail/blocked/planned/skipped count, accepted scope-out reference/reason, no-secret policy, 상위 check 상태를 표시한다. AWS S3 호환은 대체 가능한 수준으로 유지하고, paid/pilot 전환에 필요한 OSMU enterprise auth readiness gap을 우선 보여준다.
