...
- Storage/HA/DR evidence visibility: `storageExpansionFinalize`, `kubernetesHaDrReadiness`, `kubernetesDrFinalize`가 있으면 readiness panel은 storage expansion finalizer result, backend dry-run/apply/telemetry 선택 여부, Kubernetes HA/DR check 실패, DR restore namespace/backup timestamp/confirmation/gap/step 상태를 표시한다. 이로써 운영자는 target storage expansion과 Kubernetes DR evidence gap을 operations readiness report만 열지 않고 확인할 수 있다.
- IAM/RBAC evidence visibility: `iamRbacEvidence`가 있으면 readiness panel은 finalizer result/status, target namespace/service account, backend policy test/live `kubectl auth can-i` 선택 여부, failed count, gap list, command summary, step result, no-secret policy를 표시한다. 이로써 운영자는 IAM/RBAC finalizer artifact를 열기 전에 app role matrix와 Kubernetes ServiceAccount 권한 evidence gap을 확인할 수 있다.
- Security evidence visibility: `securityEvidence`가 있으면 readiness panel은 security finalizer result, image signing result/digest/signature flags, container scan/SBOM result, SBOM package/hash summary, failed check count, no-secret policy, 상위 finalizer check 상태를 표시한다. 이로써 운영자는 signed image와 container scan/SBOM evidence gap을 workflow artifact를 직접 열기 전에 확인할 수 있다.
