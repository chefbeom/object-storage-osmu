...
Add-Check "Release checklist local durable decision current" "documentation" ((Test-Contains $releaseChecklist "GO for local durable MVP demo") -and (Test-Contains $releaseChecklist 'operations readiness finalizer report exists with `result=ready`, `readinessResult=ready`') -and (Test-Contains $releaseChecklist "typed boolean confirmations plus strict typed operations readiness/convergence snapshots") -and (Test-Contains $releaseChecklist "rejects string booleans, missing integer counts") -and (Test-NotContains $releaseChecklist "NO-GO until every durable gate above passes")) "release checklist durable, convergence, and typed handoff gate checked" $releaseChecklist.path
