...
Assert-True (@($report.inputRows | Where-Object { $_.workflowInput -eq "review_started_at" }).Count -eq 1) "expected review_started_at row"
...
Assert-True (@($report.inputRows | Where-Object { $_.workflowInput -eq "observed_p99_query_latency_ms" -and $_.placeholder -eq "<ms>" }).Count -eq 1) "expected p99 ms row"
