...
Assert-True ($missingReport.artifactCount -eq 7) "Expected seven inferred artifacts."
Assert-True ($missingReport.requiredArtifactCount -eq 5) "Expected five required readiness/convergence artifacts."
Assert-True ($missingReport.missingRequiredArtifactCount -eq 5) "Expected five missing required artifacts."
...
& powershell -NoProfile -ExecutionPolicy Bypass -File $scriptPath `
    ...
    -KubernetesOperationsReportSyncRunId "107" `
...
Assert-True ($readyReport.readyArtifactCount -eq 7) "Expected all artifacts to be concrete."
...
Assert-Contains $readyMarkdown "kubernetes_operations_report_sync_run_id=107" "ready collection markdown"
...
Assert-Contains $readyMarkdown "gh run download 107 -n kubernetes-operations-report-sync-107" "ready collection markdown"
