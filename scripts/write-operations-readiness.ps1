...
$operationsHandoffPackageRemediation = New-Remediation `
    ...
    "gh workflow run manual-operations-handoff-package.yml -f environment_name=<env> -f target_cluster=<cluster> -f operator=<operator> -f handoff_started_at=<iso-time> -f handoff_completed_at=<iso-time> -f change_approval_ref=<change-id> -f deployment_evidence_ref=<ref> -f operations_readiness_ref=<ref> -f operations_convergence_ref=<ref> -f operations_readiness_json_base64=<base64-json> -f operations_convergence_json_base64=<base64-json> -f secret_rotation_evidence_ref=<ref> -f commercial_integration_evidence_ref=<ref> -f commercial_approval_evidence_ref=<ref> -f enterprise_auth_evidence_ref=<ref> -f backup_restore_evidence_ref=<ref> -f ha_dr_evidence_ref=<ref> -f monitoring_evidence_ref=<ref> -f security_evidence_ref=<ref> -f iam_rbac_evidence_ref=<ref> -f runbook_review_ref=<ref> -f troubleshooting_review_ref=<ref> -f support_escalation_ref=<ref> -f support_sla_ref=<ref> -f known_gaps_ref=<ref> -f confirm_runbook_reviewed=true -f confirm_troubleshooting_reviewed=true -f confirm_rollback_reviewed=true -f confirm_support_escalation_reviewed=true -f confirm_known_gaps_accepted=true -f confirm_operations_readiness_snapshot_reviewed=true -f confirm_operations_convergence_snapshot_reviewed=true -f confirm_no_secret_values=true -f require_production_evidence=true -f require_operations_snapshot_evidence=true -f fail_if_not_passed=true" `
    "Run at pilot or production handoff after the operator has reviewed runbook, troubleshooting, rollback, support escalation, known gaps, commercial approval, target evidence, .osmu-run/latest-operations-readiness.json, and .osmu-run/latest-operations-readiness-convergence.json. HandoffCompletedAt must be the same as or later than HandoffStartedAt. The readiness/convergence JSON can be passed as base64 to the manual workflow and is reduced to sanitized result/count/sync summary fields. The package stores references, booleans, and reduced snapshot summaries only; do not pass passwords, bearer tokens, kubeconfig values, private keys, provider credentials, raw provider responses, raw remediation commands containing credentials, or customer payment data."
