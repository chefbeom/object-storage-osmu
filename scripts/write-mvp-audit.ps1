...
$auditLines = @(
    ...
    (Status-Line $realS3ClientPassed "Real S3 client smoke" "aws=$($optionalGates.awsCliAvailable), boto3=$($optionalGates.boto3Available), mc=$($optionalGates.mcAvailable), dockerizedMc=$($optionalGates.dockerizedMcAvailable)"),
    ...
    (Evidence-Line $(if ($realS3ClientPassed) { "PASS" } else { "PENDING" }) "Real S3 clients" "TC-S3-AUTH-003" "requires aws, Python+boto3, host mc, or Dockerized mc and -RequireS3Client"),
