...
Assert-Contains $content '- Chargeback preview: API, persistent pricing policy, ADMIN-only pricing policy proposal/internal approval, Admin billing panel, scoped threshold alerts, scoped notification payload preview, scoped notification outbox/history, scoped preview CSV export, draft invoice CSV export, ADMIN-only draft invoice persistence/internal approval, and final invoice/payment state workflow implemented; final external commercial price-list approval, external payment provider integration, actual external notification delivery adapters, retry worker, and secret storage remain pending.' "Commercial readiness draft"
