...
Assert-Contains $content 'Internal chargeback preview and invoice workflow: `GET/PUT /api/admin/billing/pricing-policy`, `GET/POST /api/admin/billing/pricing-policy-proposals`, `POST /api/admin/billing/pricing-policy-proposals/{proposalId}/approve`, `POST /api/admin/billing/pricing-policy-proposals/{proposalId}/commercial-approval`, `GET /api/admin/billing/chargeback-preview`, `GET /api/admin/billing/chargeback-alerts`, `GET /api/admin/billing/chargeback-alert-notifications/preview`, `GET/POST /api/admin/billing/chargeback-alert-notifications/outbox`, `POST /api/admin/billing/chargeback-alert-notifications/outbox/{deliveryId}/adapter-result`, `GET /api/admin/billing/chargeback-adapter-retry-worker/status`, `POST /api/admin/billing/chargeback-adapter-retry-worker/run`, `GET/POST /api/admin/billing/chargeback-invoice-drafts`, `POST /api/admin/billing/chargeback-invoice-drafts/{invoiceId}/approve`, `POST /api/admin/billing/chargeback-invoice-drafts/{invoiceId}/finalize`, `GET /api/admin/billing/chargeback-invoices`, `POST /api/admin/billing/chargeback-invoices/{invoiceId}/payment-request`, `GET/POST /api/admin/billing/chargeback-invoices/{invoiceId}/payment-provider-handoff`, `GET /api/admin/billing/chargeback-payment-provider-handoffs`, `POST /api/admin/billing/chargeback-payment-provider-handoffs/{handoffId}/adapter-result`, `POST /api/admin/billing/chargeback-invoices/{invoiceId}/payment-record`, `GET /api/admin/billing/chargeback-preview/export.csv`, `GET /api/admin/billing/chargeback-invoice-draft/export.csv`, and the Admin billing panel can model organization storage, ingress, egress, internal copy, operation costs, warning/critical threshold alerts, ADMIN-only pricing policy proposal/internal approval, commercial price-list approval reference, scoped notification payload preview, scoped notification outbox/history with adapter retry state, adapter retry worker dry-run/run controls, scoped CSV export, draft invoice CSV export, ADMIN-only draft invoice persistence/internal approval, final invoice creation, payment request/paid state tracking, and no-send payment provider handoff outbox/history with adapter retry state from current usage and data-flow events; it does not send external payment provider or notification adapter calls.' "Commercial readiness draft"
...
Assert-Contains $content '- Chargeback preview: API, persistent pricing policy, ADMIN-only pricing policy proposal/internal approval, commercial price-list approval reference, Admin billing panel, scoped threshold alerts, scoped notification payload preview, scoped notification outbox/history with adapter result retry state, adapter retry worker dry-run/run controls, scoped preview CSV export, draft invoice CSV export, ADMIN-only draft invoice persistence/internal approval, final invoice/payment state workflow, and no-send payment provider handoff outbox/history with adapter result retry state implemented; actual payment provider adapter execution, actual external notification delivery adapters, external-send retry execution, and secret storage remain pending.' "Commercial readiness draft"
