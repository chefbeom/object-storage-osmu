...
Assert-Contains $content '- Chargeback preview: API, persistent pricing policy, ADMIN-only pricing policy proposal/internal approval, Admin billing panel, scoped threshold alerts, scoped notification payload preview, scoped notification outbox/history with adapter result retry state, adapter retry worker dry-run/run controls, scoped preview CSV export, draft invoice CSV export, ADMIN-only draft invoice persistence/internal approval, final invoice/payment state workflow, and no-send payment provider handoff outbox/history with adapter result retry state implemented; final external commercial price-list approval, actual payment provider adapter execution, actual external notification delivery adapters, external-send retry execution, and secret storage remain pending.' "Commercial readiness draft"
