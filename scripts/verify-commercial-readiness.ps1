...
Assert-Contains $content 'Internal chargeback preview: `GET/PUT /api/admin/billing/pricing-policy`, `GET /api/admin/billing/chargeback-preview`, `GET /api/admin/billing/chargeback-alerts`, `GET /api/admin/billing/chargeback-preview/export.csv`, `GET /api/admin/billing/chargeback-invoice-draft/export.csv`, and the Admin billing panel can model organization storage, ingress, egress, internal copy, operation costs, warning/critical threshold alerts, scoped CSV export, and draft invoice CSV export from current usage and data-flow events; this is not a final invoice or approved commercial price list.' "Commercial readiness draft"
...
Assert-Contains $content '- Chargeback preview: API, persistent pricing policy, Admin billing panel, scoped threshold alerts, scoped preview CSV export, and draft invoice CSV export implemented; finalized invoice approval/persistence, approved pricing workflow, and external alert notification routes remain pending.' "Commercial readiness draft"
