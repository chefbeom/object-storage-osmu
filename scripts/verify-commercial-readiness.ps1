...
Assert-Contains $content 'payment provider handoff outbox/history with configured generic/CARD/BANK/TAX/ERP webhook profile handoff send and adapter result retry state implemented' "Commercial readiness draft"
Assert-Contains $content 'native card/bank/tax/ERP provider API adapters, target secret/certificate rotation `result=passed` evidence, and real integration verification remain pending.' "Commercial readiness draft"
