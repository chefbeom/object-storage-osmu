...
Assert-Contains $content '- Chargeback preview: API, persistent pricing policy, ADMIN-only pricing policy proposal/internal approval, commercial price-list approval reference, Admin billing panel, scoped threshold alerts, scoped notification payload preview, scoped notification outbox/history with adapter result retry state and configured webhook send, adapter retry worker dry-run/run controls for notification webhook retry and no-send payment blocking, scoped preview CSV export, draft invoice CSV export, ADMIN-only draft invoice persistence/internal approval, final invoice/payment state workflow, and no-send payment provider handoff outbox/history with adapter result retry state implemented; actual payment provider adapter execution, email/Slack-specific notification adapters, broader external-send hardening, and production secret rotation evidence remain pending.' "Commercial readiness draft"
