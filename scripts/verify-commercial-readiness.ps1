...
Assert-Contains $content 'Internal chargeback preview: `GET/PUT /api/admin/billing/pricing-policy`, `GET /api/admin/billing/chargeback-preview`, `GET /api/admin/billing/chargeback-alerts`, `GET /api/admin/billing/chargeback-preview/export.csv`, and the Admin billing panel can model organization storage, ingress, egress, internal copy, operation costs, warning/critical threshold alerts, and scoped CSV export from current usage and data-flow events; this is not a final invoice or approved commercial price list.' "Commercial readiness draft"
...
Assert-Contains $content '- Chargeback preview: API, persistent pricing policy, Admin billing panel, scoped threshold alerts, and scoped preview CSV export implemented; finalized invoice export, approved pricing workflow, and external alert notification routes remain pending.' "Commercial readiness draft"
