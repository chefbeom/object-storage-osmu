...
Assert-Contains $content "Real S3 client gate passes with AWS CLI, boto3, AWS SDK JavaScript, host MinIO Client, or Dockerized MinIO Client." "Commercial readiness draft"
...
Assert-Contains $content 'Operations readiness artifact import report is generated as `.osmu-run/latest-operations-readiness-artifact-import.json` when evidence is assembled from prior workflow artifacts.' "Commercial readiness draft"
Assert-Contains $content 'Operations readiness finalizer report is generated as `.osmu-run/latest-operations-readiness-finalize.json` and the underlying operations readiness result is `ready`.' "Commercial readiness draft"
