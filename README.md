...
OSMU(Object Storage Management Utility)는 기업 내부망 또는 전용 인프라에서 대용량 파일, 이미지, 영상, 로그, 데이터셋을 버킷 단위로 관리하기 위한 사설 오브젝트 스토리지 관리 플랫폼입니다. MinIO를 실제 오브젝트 저장소로 사용하고, Spring Boot 백엔드가 인증, 권한, 메타데이터, 감사 로그, quota, 운영 readiness, data-flow monitoring을 담당합니다. Vue 기반 웹 포털은 관리자와 개발자가 브라우저에서 버킷, 오브젝트, Access Key, 운영 상태를 다룰 수 있게 합니다.
...
- Operations: Kubernetes/Helm draft, monitoring artifact, backup/restore drill, storage expansion runner, secret rotation/commercial integration/commercial approval/chargeback closeout/operations handoff package evidence writer와 manual evidence workflow, data-flow storage transition plan/query-retention budget/runbook evidence writer, security evidence writer/finalizer, operations readiness finalizer와 verifier 제공. Handoff package는 readiness/convergence/data-flow storage plan/query-retention budget/runbook/secret rotation/commercial/chargeback closeout/enterprise auth smoke/JIT rollback/monitoring threshold JSON을 원문 저장 없이 result/count/sync/query-plan/query-retention/runbook/core-rotation/commercial/chargeback-closeout/enterprise-auth/jit-rollback/alert-route summary로 축약한다.
- 남은 큰 축: 실제 운영 클러스터 evidence, GitHub-hosted durable gate evidence, dedicated partitioned/time-series 저장소, target secret rotation/monitoring threshold/commercial integration/commercial approval/chargeback closeout/enterprise auth smoke/JIT rollback과 readiness/convergence/data-flow storage plan/query-retention budget/runbook/monitoring threshold snapshot을 포함한 운영 handoff package `result=passed` evidence 확보이다. Data-flow는 daily/materialized rollup, monthly aggregate JSON/CSV, monthly aggregate store refresh/read/export, storage readiness/row count 상태 표면과 storage plan/query-retention budget/backfill/dual-write/rollback runbook evidence writer까지 진행됐고, S3 client smoke는 대체성 회귀 검증으로 유지한다.
...
- 현재 billing/chargeback은 내부 pricing policy/proposal, 상업 가격표 승인 참조 기록, preview/export, data-flow daily rollup 기반 일별 chargeback 추세와 CSV export, threshold alert, notification/payment outbox, notification webhook/Slack/EMAIL SMTP relay adapter send, payment provider CARD/BANK/TAX/ERP native API bridge 우선 handoff send와 generic/CARD/BANK/TAX/ERP webhook fallback/readiness, private/local webhook/native URL과 SMTP relay host 기본 차단, outbound payload size cap, generic notification/payment webhook HMAC signature header 옵션, invoice/payment workflow, adapter retry 상태 기록, notification/payment adapter retry worker dry-run/run까지 제공한다.
- 아직 남은 범위는 target 환경의 secret/certificate rotation, commercial integration, commercial approval, enterprise auth smoke/JIT rollback, operations handoff package `result=passed` evidence 확보이다. Native payment provider adapter SPI와 composite dispatch 경로에 더해 CARD/BANK/TAX/ERP별 설정 기반 native API bridge를 제공하지만, vendor별 고정 SDK/스키마 구현과 target credential/certificate 증빙은 운영 계약/설정에 따라 남긴다. Commercial integration evidence는 현재 대체 가능한 webhook/profile/native-bridge readiness snapshot을 증빙하는 수준으로 둔다. Commercial approval evidence는 외부 승인 참조와 내부 pricing proposal의 `PRICE_LIST_APPROVED` 상태 snapshot을 묶되, 가격표 원문/계약 원문은 저장하지 않는다. Chargeback closeout evidence는 target billing period의 invoice/payment/reconciliation 상태를 sanitized count/flag로 닫고, Operations handoff package는 commercial snapshot만이 아니라 chargeback closeout, target data-flow storage plan/query-retention budget/runbook, secret rotation, enterprise auth smoke/scope-out/JIT rollback, monitoring threshold snapshot도 함께 묶어 정산 마감, analytics 저장소 전환 준비 상태, query/retention 운영 예산 준수 상태, 핵심 secret/certificate rotation 상태, enterprise auth handoff 상태, 임계값 운영 준비 상태를 닫는다.
...
| `infra/monitoring` | Prometheus rules, Grafana dashboard, Alertmanager threshold target draft | Actuator metrics |
...
├─ infra/monitoring/         Prometheus rules, Grafana dashboard, threshold target draft
...
`verify-local.ps1`는 encoding hygiene, durable MVP finalize PlanOnly 보존 검증, MVP completion/preflight handoff 검증, root README/dev-doc/infra script/workflow reference guard, Flyway migration version, `scripts/write-migration-rollback-plan.ps1`/`scripts/verify-migration-rollback-plan.ps1` 기반 rollback plan, `scripts/verify-metadata-index-coverage.ps1` 기반 metadata/data-flow/audit/storage expansion/chargeback retry 주요 index prefix, `scripts/write-mariadb-query-plan-evidence.ps1`/`scripts/verify-mariadb-query-plan-evidence.ps1` 기반 MariaDB EXPLAIN evidence contract, `scripts/verify-object-list-query-pushdown.ps1` 기반 object search/filter SQL pushdown 회귀를 확인한다.
`verify-local.ps1` also refreshes the latest operations evidence chain after evidence-writing self-tests so `.osmu-run/latest-operations-evidence-handoff.json` and `.osmu-run/latest-operations-readiness-convergence.json` stay fresh against `.osmu-run/latest-operations-readiness.json`. The wrapper asserts the refreshed report order, `staleReportCount=0`, `handoffStale=false`, and selected-action scope continuity through invocation, dispatch preflight, workflow run-id, artifact collection, and handoff reports. It also reads latest JSON evidence inputs and parses `scripts/*.ps1` through explicit UTF-8 `ReadAllText`, so Windows PowerShell default codepage changes do not mask parse or contract drift.
...
Enterprise auth는 `scripts/write-enterprise-auth-smoke-plan.ps1` 또는 `.github/workflows/enterprise-auth-smoke-ci.yml`로 OIDC authorization/callback, LDAP bind/search login, claim preview/JIT approval, audit log 확인 계획을 `.osmu-run/latest-enterprise-auth-smoke.json`과 Markdown으로 남깁니다. JIT rollback/runbook 증빙은 `scripts/write-enterprise-auth-jit-rollback-evidence.ps1` 또는 `.github/workflows/manual-enterprise-auth-jit-rollback-evidence.yml`로 `.osmu-run/latest-enterprise-auth-jit-rollback-evidence.json`과 Markdown을 생성합니다. 기본 plan-only 모드는 HTTP 요청을 실행하지 않으며, 실제 IdP/LDAP smoke는 운영자가 `-Execute` 또는 workflow `run_live=true`와 필요한 credential/state를 명시한 경우에만 수행합니다. evidence에는 admin password, LDAP password, token, OIDC code/state, raw claim JSON을 기록하지 않습니다.
...
- MinIO pool/node telemetry는 evidence writer, storage expansion finalizer 옵션, backend status fallback, dashboard label, manual workflow의 prepared/live collection mode까지 연결했다. Monitoring threshold evidence도 Alertmanager/Grafana/Prometheus target baseline writer, manual workflow, artifact import chain에 연결했다. 남은 단계는 target workflow run에서 `result=passed` evidence를 확보하는 것이다.
- MinIO bucket CORS live verification, explicit bucket versioning management API, Docker local demo real MinIO browser multipart pause/resume fixture는 연결했다. 남은 단계는 실제 local/hosted run evidence를 durable gate에 반영하는 것이다.
- 관리자/감사자/조직 관리자 워크플로우 보강, 실제 IdP/LDAP pilot smoke 실행과 `.osmu-run/latest-enterprise-auth-smoke.json` evidence 확보.
- data-flow daily rollup, materialized daily rollup store, monthly aggregate JSON/CSV, monthly aggregate store 기반 장기 analytics와 storage readiness/row count를 운영 화면에서 제공한다. `scripts/write-data-flow-storage-plan.ps1`와 `.github/workflows/manual-data-flow-storage-plan-evidence.yml`로 target partition/time-series 전환 전 sizing/query-window/target p95 query latency/backfill/rollback/dashboard cutover/no-object-key aggregate policy evidence를 먼저 확보하고, MariaDB partition/dual-write 후보는 `scripts/write-mariadb-query-plan-evidence.ps1`가 만든 `.osmu-run/latest-mariadb-query-plan-evidence.json` 통과 요약을 `-QueryPlanEvidenceJsonPath` 또는 workflow `query_plan_evidence_json_base64`로 연결한다. `scripts/write-data-flow-storage-transition-runbook-evidence.ps1`와 `.github/workflows/manual-data-flow-storage-transition-runbook-evidence.yml`는 통과한 storage plan snapshot에 backfill 리허설, dual-write/partition toggle 검토, rollback 리허설, reconciliation, dashboard cutover, retention dry-run 참조를 묶고 raw SQL/EXPLAIN, object key, secret 값을 거부한다. dashboard readiness와 operations artifact import chain은 최신 plan의 candidate store/pending check/scope policy, sanitized query-plan evidence 요약, transition runbook rehearsal evidence를 보여준다.
- tenant billing/chargeback: preview API, admin billing panel, data-flow daily rollup 기반 일별 chargeback trend와 CSV export, pricing policy 저장과 proposal/internal approval/commercial price-list reference, warning/critical threshold alert, alert notification preview/outbox/webhook/Slack/EMAIL SMTP relay send/adapter retry state, private/local webhook URL과 SMTP relay host 기본 차단, outbound payload size cap, generic notification/payment webhook HMAC signature header 옵션, payment provider CARD/BANK/TAX/ERP native API bridge 우선 handoff와 generic/CARD/BANK/TAX/ERP webhook fallback/readiness, native payment provider adapter SPI/composite dispatch, 비용 리포트 CSV export, draft invoice CSV export/persistence/internal approval, final invoice/payment state workflow, payment provider handoff outbox/native-bridge-or-webhook send/adapter retry state, notification/payment adapter retry worker를 기반으로 payment-provider adapter readiness snapshot 포함 target commercial integration evidence와 pricing proposal commercial approval snapshot 포함 final commercial approval evidence를 보강. vendor별 고정 SDK/스키마 구현과 target native endpoint credential/certificate 증빙은 이후 vendor 계약/운영 secret 준비 뒤 붙인다.
- S3 대체성 유지: host `aws`/`mc`, boto3, AWS SDK smoke에서 실제 사용 흐름이 깨지는 경우만 우선 보강하고, AWS 세부 parity 추적은 제품 영향이 확인될 때만 수행한다.
- 운영 패키징: demo notes, release notes, troubleshooting, runbook, commercial approval evidence, chargeback closeout evidence, readiness/convergence/data-flow storage plan/query-retention budget/runbook/secret rotation/enterprise auth smoke/JIT rollback/monitoring threshold snapshot 포함 operations handoff package target evidence 보강.
