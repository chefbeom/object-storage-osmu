...
- 오브젝트: 업로드, 다운로드, 목록, 검색, prefix 탐색, tag, soft delete, restore, purge, multipart threshold/part-size/retry tuning.
...
- Storage backend status: `GET /api/admin/storage/backend-status`와 dashboard health widget이 object storage health, access-key provisioner health, bucket metadata usage를 표시한다. 현재 MVP는 `minioAdminMetricsEnabled=false`이며, 직접 MinIO Admin capacity telemetry는 후속 운영 고도화 범위로 둔다.
