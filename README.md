...
- 현재 billing/chargeback은 내부 pricing policy/proposal, 상업 가격표 승인 참조 기록, preview/export, threshold alert, notification/payment outbox, notification webhook/Slack adapter send, payment provider webhook handoff send, invoice/payment workflow, adapter retry 상태 기록, notification/payment webhook retry worker dry-run/run까지 제공한다.
- 아직 남은 범위는 card/bank/tax/ERP별 provider adapter, email 전용 알림 adapter, 외부 전송 운영 hardening, production secret rotation evidence이다.
